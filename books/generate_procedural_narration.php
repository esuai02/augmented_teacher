...
if (empty($secret_key)) {
    echo json_encode(['success' => false, 'error' => 'API 키가 설정되지 않았습니다. (generate_procedural_narration.php)']);
    exit();
}
...
$requestData = [
    ...
    'messages' => [
        ...
        [
            'role' => 'user',
            'content' => "다음 수학 문제와 풀이를 절차기억 형성용 듣기평가 나레이션으로 변환해주세요:\n\n" . $inputText
        ]
    ],
    'temperature' => 0.7,
    'max_tokens' => 3000
    ...
    if(isset($result['choices'][0]['message']['content'])) {
        ...
        try {
            ...
            if($existing) {
                ...
                error_log("절차기억 나레이션 DB 업데이트 완료 - ID: " . $existing->id);
            } else {
                // 신규 삽입
                $record = new stdClass();
                $record->type = 'pmemory';
                ...
                error_log("절차기억 나레이션 DB 신규 저장 완료 - ID: " . $insertId);
            }
            ...
            error_log("절차기억 나레이션 DB 저장 오류: " . $e->getMessage());
            ...
            echo json_encode([
                ...
                'narration' => $narration,
                'sectionCount' => $atCount + 1,
                'message' => "절차기억 나레이션 생성 완료! (총 " . ($atCount + 1) . "개 구간) - DB 저장 실패",
                'saved_to_db' => false,
