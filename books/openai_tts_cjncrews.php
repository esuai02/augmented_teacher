...
if (empty($secret_key)) {
    die('오류: API 키가 설정되지 않았습니다. (openai_tts_cjncrews.php)');
}
...
<body>
  ...
  <div class="container">
    ...
    <div class="dropdown-container">
      ...
      <select id="speakerNameA">
          ...
          <option value="박사님"  selected>박사님</option>
    ...
    <div class="dropdown-container">
      ...
      <select id="speakerB">
          ...
          <option value="echo">Echo (남성)</option>
          <option value="fable">Fable (남성)</option>
          <option value="onyx"  selected>Onyx (남성)</option>
    ...
    <?php
        echo '<table align="left">
                <tr> 
                  ...
                  </td> <td><a href="https://chatgpt.com/g/g-fFLnnjprZ-nareisyeon-saengseonggi" target="_blank">나레이션 생성기</a></td>
                </tr>
              </table>';
    ?>
  ...
  <script>
      ...
      input.onchange = e => {
        ...
        $.ajax({
          ...
          success: function (data, status, xhr) {
            ...
            object = parsed_data;
            if (object) {
              // 파일 업로드 성공 시, 바로가기 링크 표시
              if(object.success){
                // PHP 파일에서 반환된 URL은 "/audiofiles/파일명" 형식이므로 전체 경로 계산
                var fullUrl = "https://mathking.kr/moodle/local/augmented_teacher/LLM" + object.url;
                var audioLinkDiv = document.getElementById("audio-link");
                audioLinkDiv.innerHTML = '<a href="'+ fullUrl +'" target="_blank">'+ fullUrl +'</a>';
              }
              // 추가적인 오디오 객체 처리 로직이 필요한 경우 구현
            }
          }
        });
      }
    ...
    function playNotificationSound() {
      ...
      document.getElementById("save_button") && document.getElementById("save_button").click();
    }
  </script>
</body>
</html>
