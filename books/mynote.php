      ...
      if($cnttext->audiourl !== NULL || $cnttext->audiourl2 !== NULL) {
        ...
        if($isListeningTest && $sectionData) {
          ...
          <style>
            ...
            /* 순서 조정: 자막(1) → 진행 버튼(2) → 오디오 플레이어(3) */
            .listening-body .listening-text-display {
              order: 1;
            }

            .listening-body #subtitle-container {
              order: 1;
            }
            ...
            .auto-play-toggle[data-mode="continuous"] {
              background: linear-gradient(135deg, #a8e063 0%, #56ab2f 100%); /* 연속 재생: 녹색 */
            ...
            /* 전체 재생 모드일 때는 autoPlayToggle 버튼 숨김 */
            body[data-audio-mode="full"] #autoPlayToggle {
              display: none !important;
            }
          ...
          <div class="listening-test-container minimized" id="listeningContainer">
            <div class="listening-header">
              <div style="display: flex; align-items: center; gap: 8px;">
                <button id="autoPlayToggle" class="auto-play-toggle" data-mode="step" type="button" onclick="event.stopPropagation(); toggleAutoPlay();" title="구간 자동 이어듣기">
          ...
          <script>
            ...
            document.addEventListener("DOMContentLoaded", function() {
              ...
              restoreAutoPlayMode(); // 자동 재생 모드 복원
            });

            // 자동 재생 모드 토글 (구간별 오디오 연속 재생)
            window.toggleAutoPlay = function() {
              window.isAutoPlayMode = !window.isAutoPlayMode;

              const toggleBtn = document.getElementById("autoPlayToggle");
              const iconSpan = document.getElementById("autoPlayIcon");

              // 요소 존재 확인
              if (!toggleBtn || !iconSpan) {
                console.error("[Auto Play] File: mynote.php, Line: " + (new Error().lineNumber || "unknown") + ", Toggle button elements not found");
                return;
              }

              if (window.isAutoPlayMode) {
                // 연속 재생 모드 활성화
                toggleBtn.setAttribute("data-mode", "continuous");
                iconSpan.textContent = "auto";
                console.log("[Auto Play] File: mynote.php, Continuous mode enabled");
              } else {
                // 단계별 모드 활성화
                toggleBtn.setAttribute("data-mode", "step");
                iconSpan.textContent = "auto";
                console.log("[Auto Play] File: mynote.php, Step-by-step mode enabled");
              }

              // localStorage에 사용자 설정 저장
              localStorage.setItem("mynote_autoPlayMode", window.isAutoPlayMode);
            };

            // 페이지 로드 시 자동 재생 모드 복원
            function restoreAutoPlayMode() {
              const toggleBtn = document.getElementById("autoPlayToggle");
              const iconSpan = document.getElementById("autoPlayIcon");

              if (!toggleBtn || !iconSpan) {
                console.log("[Auto Play] File: mynote.php, Initialization: Toggle button not found (may be in different section)");
                return;
              }

              // 항상 비활성화 상태로 초기화 (localStorage 무시)
              window.isAutoPlayMode = false;

              // UI 초기 상태 설정 (항상 비활성화)
              toggleBtn.setAttribute("data-mode", "step");
              iconSpan.textContent = "auto";
              console.log("[Auto Play] File: mynote.php, Initialized with step mode (always disabled on load)");
            }
            ...
            */
            ...
            (function() {
              ...
              // 자동 재생 모드를 전역으로 노출
              if (typeof window.isAutoPlayMode === "undefined") {
                window.isAutoPlayMode = false;
              }
              ...
              audioPlayer2.addEventListener("ended", function() {
                console.log("[Auto Play] File: mynote.php, Current section ended, isAutoPlayMode:", window.isAutoPlayMode);

                if(currentSection < sectionCount - 1) {
                  // 다음 구간이 있으면 다음 버튼 활성화
                  nextBtn.disabled = false;

                  // 연속 재생 모드: 자동으로 다음 구간 재생
                  if (window.isAutoPlayMode) {
                    console.log("[Auto Play] File: mynote.php, Playing next section:", currentSection + 1);
                    setTimeout(() => {
                      try {
                        switchToSection(currentSection + 1);
                      } catch (error) {
                        console.error("[Auto Play] File: mynote.php, Error playing next section:", error);
                      }
                    }, 100); // 짧은 딜레이로 자연스러운 전환
                  }
                } else {
                  // 마지막 구간 완료
                  nextBtn.disabled = true;
                  console.log("[Auto Play] File: mynote.php, All sections completed");
              ...
              if(replayBtn) {
                replayBtn.addEventListener("click", function(e) {
                  ...
                  const url = "https://mathking.kr/moodle/local/augmented_teacher/books/drillingmath.php?userid=" + userid + "&cid=" + contentsId +
          ...
          <div class="listening-test-container minimized" id="listeningContainer" onclick="handleContainerClick(event)">
            ...
            <div class="listening-body">
              <audio id="audioPlayer2" controls src="'.$audioUrlToUse.'" style="display: block; width: 90%; height: 30px; margin: 10px auto 12px auto; border-radius: 8px;">
              </audio>
        ...
        <script>
          (function() {
            ...
            // 페이지 로드 시 속도 복원 및 자막 초기화
            document.addEventListener("DOMContentLoaded", function() {
              restorePlaybackSpeed();

              // 자막 초기 상태 적용 (내용은 PHP에서 이미 삽입됨)
              const subtitleContainer = document.getElementById("subtitle-container");
              if (subtitleContainer) {
                subtitleContainer.style.display = window.subtitlesVisible ? "block" : "none";
              }
...
<head>
  ...
  <link rel="stylesheet" href="music_modal.css">
  <script src="https://cdn.jsdelivr.net/npm/jquery@3.6.1/dist/jquery.slim.min.js"></script>
  <script src="https://cdn.jsdelivr.net/npm/popper.js@1.16.1/dist/umd/popper.min.js"></script>
  <script src="https://cdn.jsdelivr.net/npm/bootstrap@4.6.2/dist/js/bootstrap.bundle.min.js"></script>
  <script src="music_modal.js"></script>
</head>
<body>
...
if(strpos($topictitle, '특강')!==true && $npage==11111) {
  ...
  echo '</div>
        ...
        <td valign="top" width='.$width2.'%> 
          <br><br>
          <table id="contentslist">'.$contentslist.$contentslist2.$contentslist3.'<tr><td><br></td></tr>'.$attemptquiz.'</table>
          <br>
          <table>
            <tr>
              <td>
          ...
          <table width=100%>
            <tr>
              <td width=100%>'.$youtubecontents.'<br><br> '.$tutorasacode.' '.$imageupload.' <a href="https://chatgpt.com/g/g-69169d09e5f08191b50b32b86208cf14" target="_blank">#</a> '.$musicupload.' '.$promptedit.'
...
var imageUploadBtn = document.getElementById("image_upload");
if (imageUploadBtn) {
...
// 절차기억 나레이션 생성 함수 (improveprompt.php 프롬프트 사용)
function generateDialogNarration(contentsid) {
  ...
  // AJAX 요청으로 절차기억 나레이션 생성 (generate_pmemory_narration.php 사용)
  $.ajax({
    url: "generate_pmemory_narration.php",
    type: "POST",
    dataType: "json",
