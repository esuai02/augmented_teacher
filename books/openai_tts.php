...
// openai_tts.php는 conversation 타입만 처리 (pmemory는 openai_tts_pmemory.php에서 처리)
if($type==NULL)$type='conversation';
if($type!='conversation') {
    echo '이 파일은 conversation 타입만 처리합니다. pmemory 타입은 openai_tts_pmemory.php를 사용하세요.';
    exit();
}
...
<div class="container">
    ...
    <?php
    // 기존 오디오 URL 확인 - conversation 타입만 조회 (audiourl 필드 사용)
    // pmemory 타입은 audiourl2 필드를 사용하므로 완전히 분리됨
    $existingAudio = null;

    if ($contentstype == 2) {
        // question 테이블에서 조회 (conversation 타입의 audiourl만)
        $existingAudio = $DB->get_record_sql(
            "SELECT audiourl FROM {question} WHERE id = ?",
            array($contentsid)
        );
    } else {
        // icontent_pages 테이블에서 조회 (conversation 타입의 audiourl만)
        // pmemory 타입은 audiourl2를 사용하므로 여기서는 조회되지 않음
        $existingAudio = $DB->get_record_sql(
    ...
    if ($existingAudio && !empty($existingAudio->audiourl)) {
        echo '<script>
        // 페이지 로드 시 기존 오디오 표시 (캐시 무효화)
        window.addEventListener("DOMContentLoaded", function() {
            const audioSource = document.getElementById("audio-source");
            const audioControl = document.getElementById("audio-control");
            const audioPlayer = document.getElementById("audio-player");

            // 기존 소스 제거하여 브라우저 캐시 무효화
            audioSource.src = "";
            audioControl.load();
            
            // 캐시 버스팅을 위해 타임스탬프 추가
            const baseUrl = "' . $existingAudio->audiourl . '";
            const cacheBuster = baseUrl.indexOf("?") > -1 ? "&" : "?";
            audioSource.src = baseUrl + cacheBuster + "v=" + Date.now();
            audioControl.load();
            audioPlayer.style.display = "block";

            console.log("기존 오디오 파일 로드됨: " + audioSource.src);
        });
        </script>';
    }
    ?>
...
function uploadAudioToServer(audioData) {
    ...
    // openai_tts.php는 conversation 타입만 처리
    ...
    $.ajax({
        ...
        data: {
            ...
            contentsid: contentsid,
            contentstype: contentstype,
            type: type  // 명시적으로 conversation 타입 전달
        },
        dataType: 'json',
        success: function(response) {
            if (response.success) {
                ...
                // 업로드된 파일로 오디오 소스 업데이트 (캐시 무효화)
                const audioControl = document.getElementById('audio-control');
                const audioSource = document.getElementById('audio-source');
                // 기존 소스 제거하여 브라우저 캐시 무효화
                audioSource.src = '';
                audioControl.load();
                // 새 파일로 업데이트 (캐시 버스팅을 위해 타임스탬프 추가)
                const cacheBuster = response.audioUrl.indexOf('?') > -1 ? '&' : '?';
                audioSource.src = response.audioUrl + cacheBuster + 'v=' + Date.now();
                audioControl.load();
