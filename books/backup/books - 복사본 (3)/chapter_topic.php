...
// 시작: 아코디언 컨테이너 (하나가 열리면 나머지 닫힘)
$topiclist .= '<div class="accordion" id="accordionTopic">';

// 기존 코드 바로 위에 firstUnuncheckedFound 변수를 초기화합니다.
if (!isset($firstUnuncheckedFound)) { $firstUncheckedFound = false; }

foreach($result as $value) {
    $chkitemid = $value['id']; 
    // 기존 체크박스 상태는 유지 (체크박스 input 은 그대로 둡니다)
    $chkitem = $DB->get_record_sql("SELECT usertimestamp FROM mdl_checklist_check 
        WHERE item='$chkitemid' AND userid='$studentid' 
        ORDER BY id DESC LIMIT 1");
    $checkstatus = ($chkitem && $chkitem->usertimestamp > 1) ? "checked" : "";
    ...
    $setgoal = '<span onclick="setGoal(\''.$thismenutext.'\');" style="cursor:pointer;">➕</span>';
    ...
    if(strpos($linkurl, 'icontent') !== false) {
        ...
        elseif(strpos($displaytext, '유형') !== false) {
            ...
            <a href="https://mathking.kr/moodle/local/augmented_teacher/students/KeyPatternsGame.php?&cid='.$cid.'&cntid='.$cntid.'&studentid='.$studentid.'">
                <img loading="lazy" style="margin-bottom:7px;" src="https://mathking.kr/moodle/local/augmented_teacher/IMAGES/topic.png" width="20">
            </a> 
    ...
    elseif(strpos($displaytext, '정복') !== false && $learningstyle !== '도제') {
        $nchk++;
        $quizid = $params['id'];
        $defaulttalk = '<img loading="lazy" src="https://mathking.kr/moodle/local/augmented_teacher/IMAGES/exercise.png" width="60%">
        ...
        $todoitem = '
        <img loading="lazy" style="margin-bottom:7px;" src="https://mathking.kr/moodle/local/augmented_teacher/IMAGES/topic2.png" width="20"> 
    ...
    elseif(strpos($displaytext, '마무리') !== false && $learningstyle !== '도제') {
        $nchk++;
        $quizid = $params['id'];
        $defaulttalk = '<img loading="lazy" src="https://mathking.kr/moodle/local/augmented_teacher/IMAGES/ilovemath.png" width="60%">
        ...
        $todoitem = '
        <img loading="lazy" style="margin-bottom:7px;" src="https://mathking.kr/moodle/local/augmented_teacher/IMAGES/topic3.png" width="20"> 
    ...
    if(strpos($displaytext, '도약') !== false && isset($cntid)) {
        $personaBtn = '<span style="font-size:28px;cursor:pointer;" onclick="openPersonaPopup(\''.$getimg->id.'\', \''.$studentid.'\');">🎭</span> &nbsp;&nbsp;';
    }
    
    // Wizard Card 구성: 체크박스, 제목, 진단/플러스 버튼 모두 보존
    if (!$firstUncheckedFound && $checkstatus != "checked") {
        $currentCollapseClass = "collapse show";
        $currentAriaExpanded = "true";
        $firstUncheckedFound = true;
    } else {
        $currentCollapseClass = "collapse";
        $currentAriaExpanded = "false";
    }
    $topiclist .= '
    <div class="card wizard-step" style="font-size:16;">
        <div class="card-header" id="heading-'.$chkitemid.'" style="cursor:pointer;">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <input type="checkbox" name="checkAccount" '.$checkstatus.' 
                        onClick="CheckProgress(2,\''.$studentid.'\',\''.$chkitemid.'\', this.checked)" style="margin-right:8px;"/>
                    <a data-toggle="collapse" data-target="#collapse-'.$chkitemid.'" data-parent="#accordionTopic" aria-expanded="'.$currentAriaExpanded.'" aria-controls="collapse-'.$chkitemid.'" style="cursor:pointer; font-size:18px; color:#4287f5; text-decoration:none;">
                        '.$displaytext.'
                    </a>
        ...
        <div id="collapse-'.$chkitemid.'" class="'.$currentCollapseClass.' card-body" aria-labelledby="heading-'.$chkitemid.'" data-parent="#accordionTopic">
            '.$scriptontopic.'<br>
            '.$todoitem.'
        </div>
    </div>';
}
$topiclist .= '</div>'; // 아코디언 종료
...
if($role !== 'student') {
    $alt42game = '
        <a href="https://mathking.kr/moodle/local/augmented_teacher/alt42/games/dashboard.php?cid='.$cid.'&studentid='.$studentid.'&title='.$dashbordtitle.'" target="_blank">
            <img style="margin-bottom:7px;" src="https://mathking.kr/Contents/IMAGES/joystick.png" width="35">
        </a>&nbsp;&nbsp;
        <a style="font-size:28px;" href="https://mathking.kr/moodle/local/augmented_teacher/whiteboard/board_onair.php?userid='.$studentid.'&mode=1" accesskey="w">
            🎙️
        </a>';
} else {
    $alt42game = '';
...
<div class="progress" style="background-color:#bdbdbd; height:15px;">
    <div class="progress-bar progress-bar-striped bg-'.$bgtype.'" role="progressbar" style="width: '.$progressfilled.'%; height: 15px;" aria-valuenow="'.$progressfilled.'" aria-valuemin="0" aria-valuemax="100">
    </div>
</div>
';
...
<div class="container mt-3">
    ...
    '.$progressbar.'
    <small>※ 제목을 클릭하여 해당 토픽을 확인하세요.</small>
    <br><br>
    
    <!-- 단계별 카드 (Wizard Step) 아코디언 -->
    '.$topiclist.'
