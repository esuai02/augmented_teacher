...
foreach($result as $value)
{
  $title=$value['title']; 
  $npage=$value['pagenum']; 
  if($value['audiourl']!=NULL || $value['audiourl2']!=NULL) $audioicon=' 🎧';
  else $audioicon=''; 
  ...
  {    
    ...
    elseif(strpos($title, '유형')!== false) {
      $contentslist2.='<tr><td><a href="https://mathking.kr/moodle/local/augmented_teacher/books/mynote.php?dmn='.$domain.'&cid='.$cid.'&nch='.$nch.'&cmid='.$cmid.'&quizid='.$quizid.'&page='.$npage.'&studentid='.$studentid.'">'.$headimg.' '.$title.'</a>'.$audioicon.'</td></tr>'; 
    }
    elseif(strpos($title, '복습')!== false) {
      $contentslist3.='<tr><td><a href="https://mathking.kr/moodle/local/augmented_teacher/books/mynote.php?dmn='.$domain.'&cid='.$cid.'&nch='.$nch.'&cmid='.$cmid.'&quizid='.$quizid.'&page='.$npage.'&studentid='.$studentid.'"><img src="https://mathking.kr/Contents/IMAGES/restore.png" width=15> '.$title.'</a>'.$audioicon.' <input type="checkbox"  onclick="changecheckbox(1,'.$studentid.','.$mid.', this.checked)"/></td></tr>'; 
    }
    else {
      $contentslist.='<tr><td><a href="https://mathking.kr/moodle/local/augmented_teacher/books/mynote.php?dmn='.$domain.'&cid='.$cid.'&nch='.$nch.'&cmid='.$cmid.'&quizid='.$quizid.'&page='.$npage.'&studentid='.$studentid.'">'.$headimg.' '.$title.'</a>'.$audioicon.'</td></tr>';
    }
  ...
  elseif($npage==$nthispage)
  {
    ...
    if($cnttext->audiourl !== NULL || $cnttext->audiourl2 !== NULL) {
      ...
      <style>
        ...
        .audio-player-wrapper:hover .audio-tooltip {
          opacity: 1;
          visibility: visible;
        }
      </style>
          ...
          <tr>
            <td style="padding: 2px; background: transparent !important;">
              <div class="audio-player-wrapper" id="audioWrapper1">
                ...
                <span class="audio-tooltip">수업 엿듣기</span>
              </div>
            </td>
            <td style="padding: 2px; background: transparent !important;">
              <div class="speed-slider-wrapper" id="speedWrapper1">
                <input type="range" id="speedSlider" min="0.8" max="1.8" step="0.1" value="1.0" style="width:80px;height:30px;">
              </div>
            </td>
            <td style="padding: 2px; white-space: nowrap; background: transparent !important;">
              <div class="review-count-wrapper" id="reviewWrapper1">
                ('.$thisboard->nreview.')
              </div>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding: 2px; background: transparent !important;">
              <div class="audio-player-wrapper" id="audioWrapper2">
                ...
                <span class="audio-tooltip">절차기억 연습하기</span>
              </div>
            </td>
            <td style="padding: 2px; background: transparent !important;">
              <div class="speed-slider-wrapper" id="speedWrapper2">
                <input type="range" id="speedSlider2" min="0.8" max="1.8" step="0.1" value="1.0" style="width:80px;height:30px;">
              </div>
            </td>
            <td style="padding: 2px; white-space: nowrap; background: transparent !important;">
              <div class="review-count-wrapper" id="reviewWrapper2">
                ('.$thisboard->nreview.')
              </div>
            </td>
          </tr>
      ...
      <script>
        ...
        if(audioPlayer) {
          ...
          audioPlayer.addEventListener("pause", function() {
            // Disable blur if no other audio is playing
            if(!audioPlayer2 || audioPlayer2.paused) {
              disableBlur();
            }
          });
  ...
  else
  {
    ...
    elseif(strpos($title, '복습')!== false) {
      $contentslist3.='<tr><td><a href="https://mathking.kr/moodle/local/augmented_teacher/books/mynote.php?dmn='.$domain.'&cid='.$cid.'&nch='.$nch.'&cmid='.$cmid.'&quizid='.$quizid.'&page='.$npage.'&studentid='.$studentid.'"><span  type="button"  onClick="'.$presetfunction.'(\''.$contentsid.'\')"><img src="https://mathking.kr/Contents/IMAGES/restore.png" width=15></span> '.$title.'</a>'.$audioicon.' <input type="checkbox"  onclick="changecheckbox(1,'.$studentid.','.$mid.', this.checked)"/></td></tr>'; 
    }
    else {
      $contentslist.='<tr><td><a href="https://mathking.kr/moodle/local/augmented_teacher/books/mynote.php?dmn='.$domain.'&cid='.$cid.'&nch='.$nch.'&cmid='.$cmid.'&quizid='.$quizid.'&page='.$npage.'&studentid='.$studentid.'">'.$headimg.' '.$title.'</a>'.$audioicon.'</td></tr>'; 
    }
...
if(strpos($topictitle, '특강')!==true && $npage==11111) {
  ...
  if(!empty($audiocnt)) {
    echo '<div style="position: absolute; top: 10px; right: 10px; z-index: 10; background: white; padding: 10px; border-radius: 5px; box-shadow: 0 2px 5px rgba(0,0,0,0.2);">'.$audiocnt.'</div>';
  }
  echo '</div>';
} else {
  echo '<div style="position: relative;">
