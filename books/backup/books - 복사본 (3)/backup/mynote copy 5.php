...
foreach($result as $value)
{
  ...
  if($value['audiourl']!=NULL || $value['audiourl2']!=NULL) {
    ...
    $audioicon=' <span class="regenerate-audio-icon" data-contentsid="'.$contentsid.'" onclick="event.preventDefault(); event.stopPropagation(); regenerateClassroomAudio(\''.$contentsid.'\');" style="cursor:pointer; font-size:1.2em;" title="수업 엿듣기 재생성">🎧</span>';
    ...
    if($role !== 'student') {
      ...
      $flagicon=' <span class="generate-dialog-icon" data-contentsid="'.$contentsid.'" onclick="event.preventDefault(); event.stopPropagation(); handleFlagNarration(\''.$contentsid.'\');" style="cursor:pointer; font-size:1.2em;" title="'.$title.'">'.$icon.$playCount.'</span>';
    }
  } else {
    ...
    if($role !== 'student') {
      ...
      $flagicon=' <span class="generate-dialog-icon" data-contentsid="'.$contentsid.'" onclick="event.preventDefault(); event.stopPropagation(); handleFlagNarration(\''.$contentsid.'\');" style="cursor:pointer; font-size:1.2em;" title="절차기억 나레이션 생성">🟡</span>';
    }
  }
