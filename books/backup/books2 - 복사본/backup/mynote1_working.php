...
foreach($result as $value)
{
  ...
  if($value['audiourl']!=NULL || $value['audiourl2']!=NULL) {
    if($thisboard->nreview != NULL && $thisboard->nreview > 0) {
      $audioicon=' 🎧('.$thisboard->nreview.')';
    } else {
      $audioicon=' 🎧';
    }
  } else {
    // 오디오가 없을 때 클릭 가능한 헤드폰 아이콘 생성
    $audioicon=' <span class="generate-audio-icon" data-contentsid="'.$contentsid.'" onclick="event.preventDefault(); event.stopPropagation(); handleAudioGeneration(\''.$contentsid.'\');" style="cursor:pointer; color:#007bff; font-size:1.2em;" title="나레이션 생성">🎧</span>';
  }
  ...
  elseif($npage==$nthispage)
  {
    ...
    if(strpos($title, '유형')!= false) {
      $contentslist2.='<tr style="background-color:lightpink;"><td><span type="button" onClick="'.$presetfunction.'(\''.$contentsid.'\')">'.$headimg.'</span><b> '.$title.'</b> '.$audioicon.'</td></tr>'.$HippocampusCnt;
    }
    elseif(strpos($title, '복습')!== false) {
      ...
      $contentslist.='<tr style="background-color:lightblue;"><td><span type="button" onClick="'.$presetfunction.'(\''.$contentsid.'\')">'.$headimg.'</span><b> '.$title.'</b> '.$audioicon.'</td></tr>'.$HippocampusCnt;
    }
