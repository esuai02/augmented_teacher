      ...
      if($cnttext->audiourl !== NULL || $cnttext->audiourl2 !== NULL) {
        ...
        if($isListeningTest && $sectionData) {
          ...
          <style>
            ...
            /* Replay Section Button - 우측 삼각형 오른쪽 끝에 배치 */
            .replay-section-btn {
              background: rgba(255,255,255,0.2);
          ...
          $audiocnt .= '
                ...
                <button class="replay-section-btn" id="replaySectionBtn" title="현재 부분 다시듣기">🔄</button>
              </div>
          ...
          <script>
            ...
            */
            ...
            (function() {
              ...
              // Progress dots 클릭 이벤트 - 현재 구간 클릭 시 drillingmath.php 새탭 열기
              ...
              progressDots.forEach((dot, index) => {
                dot.addEventListener("click", function() {
                  // 현재 재생 중인 구간을 클릭한 경우: drillingmath.php 새탭으로 열기
                  if(index === currentSection) {
                    console.log("[Dot Click] File: mynote2.php, Line: 1189, Open drillingmath.php for cid:", contentsId, "section:", index);

                    // 해당 구간의 자막 텍스트 가져오기
                    const sectionText = textSectionsData[index] || "";
                    ...
                    const url = "https://mathking.kr/moodle/local/augmented_teacher/books/drillingmath.php?cid=" + contentsId +
                                "&ctype=1&section=" + index +
                                "&subtitle=" + encodeURIComponent(sectionText);

                    console.log("[Dot Click] File: mynote2.php, Line: 1199, URL:", url);
                    window.open(url, "_blank");
                    return;
                  }

                  // 다른 구간을 클릭한 경우: 해당 구간으로 이동
                  console.log("[Dot Click] File: mynote2.php, Line: 1205, From:", currentSection, "To:", index);
                  switchToSection(index);
                });
              });

              // Replay 버튼 클릭 이벤트 - 현재 부분 다시듣기
              const replayBtn = document.getElementById("replaySectionBtn");
              if(replayBtn) {
                replayBtn.addEventListener("click", function(e) {
                  e.stopPropagation(); // 이벤트 버블링 방지
                  console.log("[Replay Button] File: mynote2.php, Line: 1202, Replay current section:", currentSection);
                  ...
                  if(audioPlayer2) {
                    ...
                    if (savedSpeed) {
                      audioPlayer2.playbackRate = parseFloat(savedSpeed);
                      console.log("[Replay] File: mynote2.php, Line: 1211, Speed restored:", savedSpeed);
                    }

                    audioPlayer2.play().catch(e => {
                      console.error("[Audio Error] File: mynote2.php, Line: 1215, Replay error:", e);
                    });
                  }
                });
              }
...
(function() {
  try {
    ...
    const serverAudioMode = \'<?php echo $audioModePreference; ?>\';
    ...
    window.toggleAudioMode = function() {
      ...
      // 모드 전환 시 자동 재생하지 않음 (shouldPlay = false)
      applyAudioMode(newMode, false);
      ...
      if(mode === \'full\') {
        ...
        if(fullAudioPlayer && sectionAudioPlayer) {
          ...
          if(fullSrc) {
            ...
            console.log(\'[Audio Toggle] Switched to full audio:\', fullSrc);
          }
        }
