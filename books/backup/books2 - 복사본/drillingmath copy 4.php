...
async function generateDetailedThinking() {
    ...
    console.log('[drillingmath.php:generateDetailedThinking] Context length:', fullContext.length);
    ...
    try {
        ...
        data.questions.forEach((question, index) => {
            ...
            questionsHtml += `
                <button class="question-button" onclick="toggleAnswer(${questionNum}, '${question.replace(/'/g, "\\'")}')">
                    ${question}
                </button>
        ...
        console.log('[drillingmath.php:generateDetailedThinking] Generated successfully');
...
// 추가 질문 답변 토글 함수
async function toggleAnswer(questionNum, questionText) {
    ...
    if (answerContent.textContent === '답변을 생성 중입니다...') {
        try {
            ...
            console.log('[drillingmath.php:toggleAnswer] Question:', question);
            ...
            const data = await response.json();
            answerContent.innerHTML = (data.answer || '답변을 가져올 수 없습니다.').replace(/\n/g, '<br>');

            // MathJax 렌더링
            if (typeof MathJax !== 'undefined') {
                MathJax.typesetPromise([answerContent]).catch(function(err) {
