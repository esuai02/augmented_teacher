...
foreach($result as $value)
	...
	if(strpos($linkurl, 'icontent')!= false)
		...
		if($role!=='student')$reviewbutton=' <a href="https://mathking.kr/moodle/local/augmented_teacher/students/contentsreview.php?userid='.$studentid.'&cntid='.$cntid.'&title=검수"target="_blank" style="margin-left:5px;" title="콘텐츠 검수">🔍</a> ';
		else $reviewbutton='';
		if(strpos($displaytext, '노트_중급')!==false)
			{
			$cmid; 
			$notetitle='지면평가'; 
			$retrieval='_<a href="https://mathking.kr/moodle/local/augmented_teacher/students/examplenote.php?userid='.$studentid.'&cntid='.$cntid.'&title='.$notetitle.'"target="_blank"><b>'.$notetitle.'</b></a>'.$reviewbutton;
