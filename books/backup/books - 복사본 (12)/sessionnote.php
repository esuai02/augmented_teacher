...
foreach($result as $value)
  ...
  if($pgtype==='quiz')
    ...
    elseif(strpos($title, '유형')!== false)$contentslist2.='<tr><td><a href="https://mathking.kr/moodle/local/augmented_teacher/books/sessionnote.php?dmn='.$domain.'&cid='.$cid.'&nch='.$nch.'&cmid='.$cmid.'&quizid='.$quizid.'&page='.$npage.'&studentid='.$studentid.'">'.$headimg.' '.$title.'</a>'.$audioicon.$attemptresult.'</td></tr>'; 
    elseif(strpos($title, '복습')!== false)$contentslist3.='<tr><td><a href="https://mathking.kr/moodle/local/augmented_teacher/books/sessionnote.php?dmn='.$domain.'&cid='.$cid.'&nch='.$nch.'&cmid='.$cmid.'&quizid='.$quizid.'&page='.$npage.'&studentid='.$studentid.'"><img src=https://mathking.kr/Contents/IMAGES/restore.png width=15> '.$title.'</a>'.$audioicon.' <input type="checkbox"  onclick="changecheckbox(1,'.$studentid.','.$mid.', this.checked)"/></td></tr>'; 
    else $contentslist.='<tr><td><a href="https://mathking.kr/moodle/local/augmented_teacher/books/sessionnote.php?dmn='.$domain.'&cid='.$cid.'&nch='.$nch.'&cmid='.$cmid.'&quizid='.$quizid.'&page='.$npage.'&studentid='.$studentid.'">'.$headimg.' '.$title.'</a>'.$audioicon.$attemptresult.'</td></tr>';
    ...
    if(strpos($nextpage->title, '유형')!= false && $quizid!=NULL)$nextlearningurl='https://mathking.kr/moodle/local/augmented_teacher/books/sessionnote.php?dmn='.$domain.'&cid='.$cid.'&nch='.$nch.'&cmid='.$cmid.'&quizid='.$quizid.'&page='.$nnextpage.'&studentid='.$studentid;
    elseif($quizid!=NULL)$nextlearningurl='https://mathking.kr/moodle/local/augmented_teacher/books/chapter_topic.php?cid='.$cid.'&nch='.$nch.'&cntid='.($cmid+1).'&studentid='.$studentid;
  ...
  elseif($npage==$nthispage)
    ...
    // if($cnttext->audiourl!==NULL)$audiocnt='<audio controls style="width:250px;height: 50px;"><source src="'.$cnttext->audiourl.'" type="audio/mpeg"></audio><hr>';
...
<hr>';
    ...
    $youtubecontents='<img loading="lazy" src=http://ojsfile.ohmynews.com/STD_IMG_FILE/2015/0307/IE001806909_STD.jpg width=200>';
    if(strpos($cnttext->reflections1,'youtube')!==false)$youtubecontents = '<a href="https://mathking.kr/moodle/local/augmented_teacher/LLM/movie.php?cntid='.$contentsid.'&cnttype=1&studentid='.$studentid.'&wboardid='.$wboardid.'&print=0" target="_blank"><img loading="lazy" src=https://mathking.kr/Contents/IMAGES/ytblogo.png width=120></a>';
    
    if(strpos($cnttext->reflections1,'\tab')!==false)$contentslink='&nbsp;&nbsp; <a style="color:white;" href="https://mathking.kr/moodle/local/augmented_teacher/LLM/anki.php?dmn='.$domain.'&cntid='.$contentsid.'&cnttype=1&studentid='.$studentid.'&wboardid='.$wboardid.'&print=0"target="_blank"><img src="https://ankiweb.net/logo.png" width=20></a>';
    ...
    //if($thisboard->wboardid==NULL && $USER->id==$studentid)$DB->execute("INSERT INTO {abessi_messages} (userid,userto,userrole,talkid,nstep,turn,status,contentstype,wboardid,contentstitle,contentsid,url,timemodified,timecreated) VALUES('$studentid','2','$role','2','0','$milestone','begintopic','1','$wboardid','inspecttopic','$contentsid','$mynoteurl','$timecreated','$timecreated')");
   ...
   // else $DB->execute("UPDATE {abessi_messages} SET timemodified='$timecreated',  turn='$milestone', active='1', contentsid='$contentsid',url='$mynoteurl' WHERE wboardid='$wboardid' ORDER BY id DESC LIMIT 1 ");
...
if($npage==1)
    ...
    if(strpos($title, '유형')!= false)$contentslist2.='<tr style="background-color:lightpink;"><td><span  type="button"  onClick="'.$presetfunction.'(\''.$contentsid.'\')">'.$headimg.'</span><b> '.$title.$audioicon.'</b> '.$attemptresult.'</td></tr>'.$HippocampusCnt;
    elseif(strpos($title, '복습')!== false)$contentslist3.='<tr><td><span  type="button"  onClick="'.$presetfunction.'(\''.$contentsid.'\')"><img src=https://mathking.kr/Contents/IMAGES/restore.png width=15></span> '.$title.$audioicon.' <input type="checkbox"  onclick="changecheckbox(1,'.$studentid.','.$mid.', this.checked)"/></td></tr>'; 
    else $contentslist.='<tr style="background-color:lightblue;"><td><span  type="button"  onClick="'.$presetfunction.'(\''.$contentsid.'\')">'.$headimg.'</span><b> '.$title.$audioicon.'</b> '.$attemptresult.'</td></tr>'.$HippocampusCnt;
    ...
    if(strpos($nextpage->title, '유형')!= false && strpos($title, '유형')!=true && $quizid!=NULL || ( strpos($title, '유형')!=true && $quizid!=NULL && $nextpage->id==NULL))
      {
      $nextlearningurl='https://mathking.kr/moodle/local/augmented_teacher/books/sessionnote.php?dmn='.$domain.'&cid='.$cid.'&nch='.$nch.'&cmid='.$cmid.'&quizid='.$quizid.'&pgtype=quiz&page='.$npage.'&studentid='.$studentid;
      $nquizpage=$npage;
      }
    elseif($nextpage->id!=NULL)$nextlearningurl='https://mathking.kr/moodle/local/augmented_teacher/books/sessionnote.php?dmn='.$domain.'&cid='.$cid.'&nch='.$nch.'&cmid='.$cmid.'&quizid='.$quizid.'&page='.$nnextpage.'&studentid='.$studentid;
    
    elseif($quizid!=NULL && strpos($title, '유형')!= false && $pgtype!=='quiz')$nextlearningurl='https://mathking.kr/moodle/local/augmented_teacher/books/chapter_topic.php?cid='.$cid.'&nch='.$nch.'&cntid='.($cmid+1).'&studentid='.$studentid;    
    else $nextlearningurl='https://mathking.kr/moodle/local/augmented_teacher/books/chapter_topic.php?cid='.$cid.'&nch='.$nch.'&cntid='.($cmid+1).'&studentid='.$studentid;
    //if($USER->id==2)
    $rule='<a style="text-decoration:none;color:white;" href="'.$nextlearningurl.'"><button class="stylish-button">NEXT</button></a>';
		}
	else
    ...
    if($learningstyle==='도제' && strpos($title, '대표')!==false)echo '';
    elseif(strpos($title, '유형')!= false)$contentslist2.='<tr><td><a href="https://mathking.kr/moodle/local/augmented_teacher/books/sessionnote.php?dmn='.$domain.'&cid='.$cid.'&nch='.$nch.'&cmid='.$cmid.'&quizid='.$quizid.'&page='.$npage.'&studentid='.$studentid.'">'.$headimg.' '.$title.'</a>'.$audioicon.$attemptresult.'</td></tr>'; 
    elseif(strpos($title, '복습')!== false)$contentslist3.='<tr><td><a href="https://mathking.kr/moodle/local/augmented_teacher/books/sessionnote.php?dmn='.$domain.'&cid='.$cid.'&nch='.$nch.'&cmid='.$cmid.'&quizid='.$quizid.'&page='.$npage.'&studentid='.$studentid.'"><span  type="button"  onClick="'.$presetfunction.'(\''.$contentsid.'\')"><img src=https://mathking.kr/Contents/IMAGES/restore.png width=15></span> '.$title.'</a>'.$audioicon.' <input type="checkbox"  onclick="changecheckbox(1,'.$studentid.','.$mid.', this.checked)"/></td></tr>'; 
    else $contentslist.='<tr><td><a href="https://mathking.kr/moodle/local/augmented_teacher/books/sessionnote.php?dmn='.$domain.'&cid='.$cid.'&nch='.$nch.'&cmid='.$cmid.'&quizid='.$quizid.'&page='.$npage.'&studentid='.$studentid.'">'.$headimg.' '.$title.'</a>'.$audioicon.$attemptresult.'</td></tr>'; 
		}
	}
 /*
...
if($quizid!=NULL)
  ...
  else $attemptquiz='<tr><td><a href="https://mathking.kr/moodle/local/augmented_teacher/books/sessionnote.php?dmn='.$domain.'&cid='.$cid.'&nch='.$nch.'&cmid='.$cmid.'&pgtype=quiz&quizid='.$quizid.'&page='.$npage.'&studentid='.$studentid.'">'.$headimg.' 개념체크 퀴즈</a>  <a href="https://mathking.kr/moodle/mod/quiz/view.php?id='.$quizid.'"target="_blank">(<b style="color:#E4167D;">시도</b>)</a>'.$attemptresult.'</td></tr>';
  }
...
<head>
 ...
 <body><table align=center><tr><td width='.$width1.'% valign=top>';

if(strpos($topictitle, '특강')!==true &&	$npage==11111) echo '<iframe loading="lazy"  style="border: 1px none; z-index:2; width:'.$width1.'vw; height:50vh;  margin-left:-0px; margin-top: 0px; "  src="'.$showpage.'" ></iframe>';
else echo '<iframe loading="lazy"  style="border: 1px none; z-index:2; width:'.$width1.'vw; height:100vh;  margin-left:-0px; margin-top: 0px; "  src="'.$showpage.'" ></iframe></td><td width=2%></td><td valign=top width='.$width2.'%> <br><br><table>'.$contentslist.$attemptquiz.'<tr><td><br></td></tr>'.$contentslist2.$contentslist3.'</table><br><table><tr><td><a href="https://mathking.kr/moodle/local/augmented_teacher/books/chapter_topic.php?cid='.$cid.'&nch='.$nch.'&cntid='.($cmid+1).'&studentid='.$studentid.'"><img loading="lazy" src=https://mathking.kr/Contents/MATH%20MATRIX/MATH%20images/IMG/MXBESSI1621944121001.png width=20> 목차</a>'.$singleref.$cntlink.'</td></tr>
  <tr><td align=left width=22vw style="color:#347aeb;" style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis;"><br>'.$rule.' <br><br> </td></tr></table>
  <table width=100%> <tr><td width=100%>'.$youtubecontents.'<br><br> '.$generateontology.' 기억방으로  '.$imageupload.' <a href="https://mathking.kr/moodle/local/augmented_teacher/books/mynote_full.php?'.$mynoteurl.'" target="_blank"><img src=https://mathking.kr/Contents/IMAGES/changetofull.png width=20></a></td></tr></table><hr><table><tr><td>'.$audiocnt.' </td></tr>'.$stepbystepcnt.'</table><table><tr><td><br>'.$activities.'</td></tr><tr><td><hr></td></tr></table></td></tr></table></body>';
