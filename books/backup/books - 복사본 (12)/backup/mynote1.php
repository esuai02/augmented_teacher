...
foreach($result as $value)
{
  ...
  if($value['audiourl']!=NULL || $value['audiourl2']!=NULL) {
    ...
    // audiourl2가 없을 때만 깃발 아이콘 표시
    if($value['audiourl2']==NULL) {
      $flagicon=' <span class="generate-dialog-icon" data-contentsid="'.$contentsid.'" onclick="event.preventDefault(); event.stopPropagation(); handleFlagNarration(\''.$contentsid.'\');" style="cursor:pointer; color:#ff5722; font-size:1.2em;" title="대화형 나레이션 생성">🚩</span>';
    }
  } else {
...
// 깃발 아이콘 클릭 핸들러 (대화형 나레이션) - 글로벌 스코프에 등록
window.handleFlagNarration = function(contentsid) {
  ...
  Swal.fire({
    title: "대화형 나레이션 생성",
    text: "선생님과 학생의 대화 형식으로 나레이션을 생성하시겠습니까?",
    icon: "question",
    showCancelButton: true,
    confirmButtonText: "생성",
    cancelButtonText: "취소"
...
function generateDialogNarration(contentsid) {
  ...
  Swal.fire({
    title: "대화형 나레이션 생성 중...",
    html: "선생님과 학생의 대화를 생성하고 있습니다.<br>이 작업은 약 1-2분 정도 소요됩니다.",
    allowOutsideClick: false,
    didOpen: () => {
      Swal.showLoading();
    }
  ...
  $.ajax({
    ...
    success: function(response) {
      if (response.success) {
        Swal.fire({
          icon: "success",
          title: "대화형 나레이션 생성 완료!",
          html: "대화형 나레이션과 음성이 성공적으로 생성되었습니다.<br>페이지를 새로고침합니다.",
          timer: 3000,
        ...
        Swal.fire({
          icon: "error",
          title: "생성 실패",
          text: response.message || "대화형 나레이션 생성 중 오류가 발생했습니다."
        });
      }
    },
    error: function(xhr, status, error) {
      console.error("대화형 나레이션 생성 실패:", error);
      console.error("응답 상태:", xhr.status);
      console.error("응답 텍스트:", xhr.responseText);

      let errorMessage = "대화형 나레이션 생성 중 오류가 발생했습니다.";
