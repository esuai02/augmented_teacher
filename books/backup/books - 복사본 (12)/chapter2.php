...
foreach($result as $value)
	...
	if(strpos($linkurl, 'icontent')!= false)
		...
		if(($chkitem->usertimestamp==0 || $chkitem->usertimestamp==NULL) && $topicchosen==0)
			...
			if($gmset->id!=NULL && $gmset->timemodified < time()-1200)
			{
				echo '<script>
		...
		if(strpos($displaytext, '도약')!= false )
			...
			if($USER->id==2)	
				{
				$nanki++;
				$getanki=$DB->get_record_sql("SELECT id FROM mdl_abessi_ankiquiz WHERE contentstype LIKE '1' AND contentsid LIKE '$ankicntid' ORDER BY id DESC LIMIT 1");
				if($nanki==1)$ankiquizid1=$getanki->id;
				elseif($nanki==2)$ankiquizid2=$getanki->id;
				elseif($nanki==3)$ankiquizid3=$getanki->id;
				elseif($nanki==4)$ankiquizid4=$getanki->id;
				elseif($nanki==5)$ankiquizid5=$getanki->id;
				elseif($nanki==6)$ankiquizid6=$getanki->id;
				elseif($nanki==7)$ankiquizid7=$getanki->id;
				elseif($nanki==8)$ankiquizid8=$getanki->id;
				elseif($nanki==9)$ankiquizid9=$getanki->id;
				elseif($nanki==10)$ankiquizid10=$getanki->id;
				elseif($nanki==11)$ankiquizid11=$getanki->id;
				elseif($nanki==12)$ankiquizid12=$getanki->id;
				}
			*/
			}
	...
	elseif(strpos($displaytext, '정복')!= false && $learningstyle!=='도제')
		...
		if($USER->id==2)
			...
			$threadexist=$DB->get_record_sql("SELECT * FROM mdl_abessi_ankithread where threadid='$quizid' ORDER BY id DESC LIMIT 1 "); 
			$record = new stdClass();
			$record->type = 'quiz';
			$record->threadid = $quizid;
			$record->userid = $USER->id;
			$record->title = $threadtitle;
			if($ankiquizid1!=NULL)$record->quiz1 = $ankiquizid1;
			if($ankiquizid2!=NULL)$record->quiz2 = $ankiquizid2;
			if($ankiquizid3!=NULL)$record->quiz3 = $ankiquizid3;
			if($ankiquizid4!=NULL)$record->quiz4 = $ankiquizid4;
			if($ankiquizid5!=NULL)$record->quiz5 = $ankiquizid5;
			if($ankiquizid6!=NULL)$record->quiz6 = $ankiquizid6;
			if($ankiquizid7!=NULL)$record->quiz7 = $ankiquizid7;
			if($ankiquizid8!=NULL)$record->quiz8 = $ankiquizid8;
			if($ankiquizid9!=NULL)$record->quiz9 = $ankiquizid9;
			if($ankiquizid10!=NULL)$record->quiz10 = $ankiquizid10;
			if($ankiquizid11!=NULL)$record->quiz11 = $ankiquizid11;
			if($ankiquizid12!=NULL)$record->quiz12 = $ankiquizid12;
			
			$record->timemodified = $timecreated;
			$record->timecreated = $timecreated;
			
			if($threadexist->id==NULL)$DB->insert_record('abessi_ankithread', $record);
			else 
				{
				$record->id=$threadexist->id;
...
<a href="https://mathking.kr/moodle/local/augmented_teacher/books/chapter.php?'.$modeinfo.'&cid=95&nch=1&studentid='.$studentid.'&type=init">초등 3-1</a> |
<a href="https://mathking.kr/moodle/local/augmented_teacher/books/chapter.php?'.$modeinfo.'&cid=96&nch=1&studentid='.$studentid.'&type=init">초등 3-2</a> |
