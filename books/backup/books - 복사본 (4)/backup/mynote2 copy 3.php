  ...
  elseif($npage==$nthispage)
  {
    ...
    if($cnttext->audiourl !== NULL || $cnttext->audiourl2 !== NULL) {
      ...
      <style>
        ...
        body[data-audio-mode="full"] #listeningNextBtn {
          display: none !important;
        }
      ...
      if($cnttext->audiourl2 !== NULL) {
        ...
        if($isListeningTest && $sectionData) {
          ...
          <div class="listening-test-container minimized" id="listeningContainer">
            <div class="listening-header">
              <div class="listening-progress" id="listeningProgress">
                ...
                <button id="toggleSubtitleBtn" style="margin-left:5px;cursor:pointer;background:none;border:none;font-size:0.85em;padding:0;" title="자막 표시/숨김">📄</button>
              </div>
              <div style="display: flex; align-items: center; gap: 8px;">
                ...
                <div id="audioModeToggleContainer" style="display: none; align-items: center; gap: 4px;">
                  <button id="audioModeToggle"
                          ...
                          data-mode="section"
                          type="button"
                          onclick="simpleToggleAudio(); return false;"
                          title="오디오 재생 모드 전환">
                    <span id="audioModeLabel">전체</span>
                  </button>
                </div>
          ...
          // Progress dots 생성
          $audiocnt .= '
              <div style="display: flex; align-items: center; justify-content: center; gap: 12px; margin: 15px 0 10px 0;">
                <button id="prevSectionBtn" class="section-nav-btn" title="이전 구간">◀</button>
          ...
          $audiocnt .= '
                ...
                <button id="nextSectionBtn" class="section-nav-btn" title="다음 구간">▶</button>
              </div>
            </div>
          </div>
            ...
            (function() {
              ...
              audioPlayer2.addEventListener("ended", function() {
                if(currentSection < sectionCount - 1) {
                  // 다음 구간 자동 이동 (선택적: 수동 이동을 원하면 주석 처리)
              ...
              if(toggleSubtitleBtn) {
                toggleSubtitleBtn.addEventListener("click", function() {
                  const textDisplay = document.querySelector(".listening-text-display");
                  const allTexts = document.querySelectorAll(".listening-text");

                  if(textDisplay && textDisplay.classList.contains("active")) {
                    // 현재 표시 중이면 숨기기
                    ...
                    if(currentMode === "full") {
                      ...
                      const currentText = document.getElementById("listeningText" + (currentSectionIndex + 1));
                      if(currentText) {
...
if(strpos($topictitle, '특강')!==true && $npage==11111) {
  ...
  echo '</div>
        ...
        <td valign="top" width='.$width2.'%> 
          ...
          <table width=100%>
            <tr>
              <td width=100%>'.$youtubecontents.'<br><br> '.$tutorasacode.' '.$imageupload.' '.$musicupload.'
...
// 간단한 오디오 모드 토글 (전체 재작성)
(function() {
  console.log("[Simple Audio Toggle] Initializing...");

  // 페이지 로드 완료 대기
  window.addEventListener(\'load\', function() {
    console.log("[Simple Audio Toggle] Page loaded, setting up button");

    const button = document.getElementById(\'audioModeToggle\');
    const label = document.getElementById(\'audioModeLabel\');
    const fullAudio = document.getElementById(\'audioPlayerFull\');
    const sectionAudio = document.getElementById(\'audioPlayer2\');

    // 버튼과 오디오가 모두 있는지 확인
    if(!button || !fullAudio || !sectionAudio) {
      console.log("[Simple Audio Toggle] Required elements not found");
      return;
    }

    console.log("[Simple Audio Toggle] All elements found, attaching listener");
    ...
    function applyAudioMode(mode, shouldPlay) {
      ...
      if(mode === \'full\') {
        // 전체 모드: audiourl 재생, 라벨은 "나누어" (다음 액션)
        label.textContent = \'나누어\';

        // Hide section texts and show reflections0
        const allTexts = document.querySelectorAll(\'.listening-text\');
        allTexts.forEach(text => {
          text.classList.remove(\'active\');
        });

        // Show reflections0 in subtitle area
        const textDisplay = document.querySelector(\'.listening-text-display\');
        if(textDisplay) {
          const reflections0Text = \''.(isset($cnttext->reflections0) ? addslashes($cnttext->reflections0) : '').'\';
          textDisplay.innerHTML = \'<div style="white-space: pre-wrap;">\' + reflections0Text + \'</div>\';
          textDisplay.classList.add(\'active\');
        }

        // Switch audioPlayer2 to full narration audio (audiourl)
        if(fullAudioPlayer && sectionAudioPlayer) {
          const fullSrc = fullAudioPlayer.getAttribute(\'data-audiourl\');
          if(fullSrc) {
            sectionAudioPlayer.src = fullSrc;
            sectionAudioPlayer.load();
            console.log(\'[Audio Toggle] Switched to AUDIOURL (full narration):\', fullSrc);
          }
        }
        ...
        // 구간별 모드: sectionFiles 재생, 라벨은 "전체" (다음 액션)
        label.textContent = \'전체\';

        // Hide reflections0 display
        const textDisplay = document.querySelector(\'.listening-text-display\');
        if(textDisplay) {
          textDisplay.classList.remove(\'active\');
        }

        // Hide all text sections first
        const allTexts = document.querySelectorAll(\'.listening-text\');
        allTexts.forEach(text => {
          text.classList.remove(\'active\');
        });

        // Show only current section text
        const currentSectionIndex = window.currentSection || 0;
        const currentText = document.getElementById(\'listeningText\' + (currentSectionIndex + 1));
        if(currentText) {
          currentText.classList.add(\'active\');
        ...
        // Switch audioPlayer2 back to current section audio (sectionFiles)
...
// ===== 간단한 오디오 토글 (재작성) =====
// inline onclick으로 직접 실행 - 가장 안전한 방법
window.simpleToggleAudio = function() {
  console.log("[NEW] Toggle clicked!");

  const label = document.getElementById(\'audioModeLabel\');
  const fullAudio = document.getElementById(\'audioPlayerFull\');
  const sectionAudio = document.getElementById(\'audioPlayer2\');

  if(!label || !fullAudio || !sectionAudio) {
    console.error("[NEW] Elements missing!");
    return;
  }

  // 현재 모드 확인 (라벨 텍스트로 판단)
  const isCurrentlySection = (label.textContent === \'전체\');

  if(isCurrentlySection) {
    // "전체" 라벨 보임 → 클릭 → 전체 모드로 전환 (audiourl 재생)
    console.log("[NEW] Switching to FULL mode");
    label.textContent = \'나누어\';

    const audiourl = fullAudio.getAttribute(\'data-audiourl\');
    if(audiourl) {
      sectionAudio.pause();
      sectionAudio.src = audiourl;
      sectionAudio.load();
      sectionAudio.play();
      console.log("[NEW] Playing audiourl:", audiourl);
    }

  } else {
    // "나누어" 라벨 보임 → 클릭 → 구간별 모드로 전환
    console.log("[NEW] Switching to SECTION mode");
    label.textContent = \'전체\';

    if(window.sectionFiles && window.currentSection !== undefined) {
      const sectionSrc = window.sectionFiles[window.currentSection];
      if(sectionSrc) {
        sectionAudio.pause();
        sectionAudio.src = sectionSrc;
        sectionAudio.load();
        sectionAudio.play();
        console.log("[NEW] Playing section:", sectionSrc);
      }
    }
  }
};

console.log("[NEW Audio Toggle] Function registered");
</script>';
?>
