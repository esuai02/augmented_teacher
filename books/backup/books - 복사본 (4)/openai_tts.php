...
if($contentstype==1)
    {
        $thisboard=$DB->get_record_sql("SELECT * FROM mdl_abessi_messages where contentsid='$contentsid' AND contentstype='$contentstype' AND url IS NOT NULL ORDER BY id DESC LIMIT 1 ");
        echo '<table align=left><tr><td><a href="https://mathking.kr/moodle/local/augmented_teacher/books/mynote.php?'.$thisboard->url.'"target="_blank">노트보기</a> | <a href="https://chatgpt.com/g/g-fFLnnjprZ-jeonmun-nareisyeon-saengseongjangci"target="_blank">대화생성</a> | </td><td><button id="audio_upload" type="button" class="" data-toggle="collapse" data-target="#demo" accesskey="q">⬆️</button> <button id="save_button" class="custom-button green" onclick="saveText()">저장</button></td></tr></table>';
    }
else 
    {   
