<?php
/////////////////////////////// 서버 파일 배포 스크립트 ///////////////////////////////
// 이 파일을 서버에 업로드하고 한 번만 실행하세요.
// URL: https://mathking.kr/moodle/local/augmented_teacher/books/deploy_classroom_narration.php

$targetFile = __DIR__ . '/generate_classroom_narration.php';
...
try {
    ...
    if (!file_exists($configFile)) {
        throw new Exception("[generate_classroom_narration.php:25] API 설정 파일을 찾을 수 없습니다: " . $configFile);
    }
    ...
    if ($contentsid <= 0) {
        throw new Exception("[generate_classroom_narration.php:49] 유효하지 않은 콘텐츠 ID");
    }

    // 로그인 체크
    require_login();

    // 응답 초기화 (array() 문법 사용 - PHP 5.4 호환)
    $response = array(
        'success' => false,
        'message' => '',
        'step' => 'init'
    ...
    // 콘텐츠 가져오기
    $cnttext = $DB->get_record('icontent_pages', array('id' => $contentsid));

    if (!$cnttext) {
        throw new Exception("[generate_classroom_narration.php:66] 콘텐츠를 찾을 수 없습니다");
    }
...
$maintext";
    ...
    if (!defined('OPENAI_API_KEY') || empty(OPENAI_API_KEY)) {
        throw new Exception("[generate_classroom_narration.php:134] API 키가 설정되지 않았습니다");
    }
    ...
    if ($curlError) {
        throw new Exception("[generate_classroom_narration.php:168] API 호출 오류: " . $curlError);
    }

    if ($httpCode !== 200) {
        $errorData = json_decode($apiResponse, true);
        $errorMsg = isset($errorData['error']['message']) ? $errorData['error']['message'] : "HTTP $httpCode";
        throw new Exception("[generate_classroom_narration.php:174] GPT API 오류: " . $errorMsg);
    }

    $apiData = json_decode($apiResponse, true);
    if (!isset($apiData['choices'][0]['message']['content'])) {
        throw new Exception("[generate_classroom_narration.php:179] API 응답 형식 오류");
    }
    ...
    // DB에 나레이션 저장
    $existingRecord = $DB->get_record('abrainalignment_gptresults', array(
        'type' => 'conversation',
    ...
    if ($httpCode !== 200) {
        throw new Exception("[generate_classroom_narration.php:244] TTS API 오류: HTTP " . $httpCode);
    }
    ...
    if (!file_exists($uploadPath)) {
        if (!mkdir($uploadPath, 0755, true)) {
            throw new Exception("[generate_classroom_narration.php:251] 디렉토리 생성 실패: " . $uploadPath);
        }
    }
    ...
    if (file_put_contents($filepath, $audioResponse) === false) {
        throw new Exception("[generate_classroom_narration.php:259] 오디오 파일 저장 실패: " . $filepath);
    }
    ...
    if ($DB->update_record('icontent_pages', $updateData)) {
        ...
        throw new Exception("[generate_classroom_narration.php:284] DB 업데이트 실패");
    }
...
echo "✅ 파일 배포 완료!<br><br>";
echo "생성된 파일: $targetFile<br>";
echo "파일 크기: " . filesize($targetFile) . " bytes<br>";
echo "파일 권한: " . substr(sprintf('%o', fileperms($targetFile)), -4) . "<br><br>";
echo "이제 이 파일(deploy_classroom_narration.php)은 삭제하셔도 됩니다.<br>";
echo "수업 엿듣기 기능을 테스트해보세요!";
?>
