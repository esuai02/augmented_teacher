            ...
            foreach($cntpages as $page) {
                ...
                if(!empty($ctext)) {
                    ...
                    foreach($imageTags as $imageTag) {
                        ...
                        // 조건 확인 - MATRIX, MATH, imgur 중 하나라도 포함되면
...
for($nch=1;$nch<=$chapnum;$nch++)
{
	...
	else $chapterlist.='<tr><td>'.$nch.'</td><td><a href="https://mathking.kr/moodle/local/augmented_teacher/books/patternbank.php?'.$modeinfo.'&cid='.$cid.'&nch='.$nch.'&studentid='.$studentid.'&type=init">'.$title.'</a>'.$quizresult.'</td></tr>';
	}
...
for($nch=1;$nch<=$chapnum;$nch++) 
	...
	else $obsnotelist.='<tr><td style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis;"><span>'.$nch.' <a style="color:#a9aab0;" href="https://mathking.kr/moodle/local/augmented_teacher/books/patternbank.php?'.$modeinfo.'&cid='.$cid2.'&nch='.$nchapter.'&studentid='.$studentid.'&type=init"target="_blank">'.$title.'</a></span> <a href="https://mathking.kr/moodle/local/augmented_teacher/whiteboard/obsession.php?cid='.$cid2.'&nch='.$nchapter.'&studentid='.$studentid.'&mode=domain&domain='.$domain.'"target="_blank"><img  style="margin-bottom:8px;" src=https://mathking.kr/Contents/MATH%20MATRIX/MATH%20images/IMG/cjn1667755172.png width=20></a></td></tr>';	
	}
...
foreach($result as $value) {
    ...
    if(strpos($linkurl, 'icontent') !== false) {
        $cntid = $params['id'];
        // displaytext 확인을 위해 다양한 패턴 체크
        if(strpos($displaytext, '대표유형 노트_중급') !== false || strpos($displaytext, '노트_중급') !== false) {
            $middleCntid = $cntid;
        } elseif(strpos($displaytext, '대표유형 노트_심화') !== false || strpos($displaytext, '노트_심화') !== false) {
            $advancedCntid = $cntid;
        }
    }
}
...
if(!empty($middleImages)) {
    foreach($middleImages as $img) {
        ...
        $middleImagesHTML .= '<div style="cursor: pointer; border: 1px solid #ddd; padding: 10px; flex: 0 0 calc(50% - 10px); box-sizing: border-box;"
                onclick="window.open(\'https://mathking.kr/moodle/local/augmented_teacher/alt42/patternbank/patternbank.php?userid='.$studentid.'&wboardid='.$wboardid.'&contentsid='.$img['contentsid'].'&contentstype=1&mode=questiononly\', \'_blank\')">
                <div style="font-weight: bold; margin-bottom: 5px;">페이지 '.$img['pagenum'].'</div>
    ...
    // 디버깅 정보 추가
    if($middleCntid) {
        $middleImagesHTML = '<p>중급 노트 이미지가 없습니다. (cntid: '.$middleCntid.')</p>';
    } else {
        $middleImagesHTML = '<p>중급 노트를 찾을 수 없습니다.</p>';
    }
}
...
if(!empty($advancedImages)) {
    foreach($advancedImages as $img) {
        $wboardid = 'jnrsorksqcrark'.$img['contentsid'].'_user'.$studentid;
        $advancedImagesHTML .= '<div style="cursor: pointer; border: 1px solid #ddd; padding: 10px; flex: 0 0 calc(50% - 10px); box-sizing: border-box;"
                onclick="window.open(\'https://mathking.kr/moodle/local/augmented_teacher/alt42/patternbank/patternbank.php?userid='.$studentid.'&wboardid='.$wboardid.'&contentsid='.$img['contentsid'].'&contentstype=1&mode=questiononly\', \'_blank\')">
                <div style="font-weight: bold; margin-bottom: 5px;">페이지 '.$img['pagenum'].'</div>
                <img src="'.$img['src'].'" style="width: 100%; height: auto;">
            </div>';
    ...
    // 디버깅 정보 추가
    if($advancedCntid) {
        $advancedImagesHTML = '<p>심화 노트 이미지가 없습니다. (cntid: '.$advancedCntid.')</p>';
    } else {
        $advancedImagesHTML = '<p>심화 노트를 찾을 수 없습니다.</p>';
    }
}
...
<a href="https://mathking.kr/moodle/local/augmented_teacher/books/patternbank.php?'.$modeinfo.'&cid=73&nch=1&studentid='.$studentid.'&type=init">초등 4-1</a> |
<a href="https://mathking.kr/moodle/local/augmented_teacher/books/patternbank.php?'.$modeinfo.'&cid=74&nch=1&studentid='.$studentid.'&type=init">초등 4-2</a> |
<a href="https://mathking.kr/moodle/local/augmented_teacher/books/patternbank.php?'.$modeinfo.'&cid=75&nch=1&studentid='.$studentid.'&type=init">초등 5-1</a> |
<a href="https://mathking.kr/moodle/local/augmented_teacher/books/patternbank.php?'.$modeinfo.'&cid=76&nch=1&studentid='.$studentid.'&type=init">초등 5-2</a> |
<a href="https://mathking.kr/moodle/local/augmented_teacher/books/patternbank.php?'.$modeinfo.'&cid=78&nch=1&studentid='.$studentid.'&type=init">초등 6-1</a> |
<a href="https://mathking.kr/moodle/local/augmented_teacher/books/patternbank.php?'.$modeinfo.'&cid=79&nch=1&studentid='.$studentid.'&type=init">초등 6-2</a> |
<a href="https://mathking.kr/moodle/local/augmented_teacher/books/patternbank.php?'.$modeinfo.'&cid=66&nch=1&studentid='.$studentid.'&type=init">중 1-1</a> |
<a href="https://mathking.kr/moodle/local/augmented_teacher/books/patternbank.php?'.$modeinfo.'&cid=67&nch=1&studentid='.$studentid.'&type=init">중 1-2</a> |
<a href="https://mathking.kr/moodle/local/augmented_teacher/books/patternbank.php?'.$modeinfo.'&cid=68&nch=1&studentid='.$studentid.'&type=init">중 2-1</a> |
<a href="https://mathking.kr/moodle/local/augmented_teacher/books/patternbank.php?'.$modeinfo.'&cid=69&nch=1&studentid='.$studentid.'&type=init">중 2-2</a> |
<a href="https://mathking.kr/moodle/local/augmented_teacher/books/patternbank.php?'.$modeinfo.'&cid=71&nch=1&studentid='.$studentid.'&type=init">중 3-1</a> |
<a href="https://mathking.kr/moodle/local/augmented_teacher/books/patternbank.php?'.$modeinfo.'&cid=72&nch=1&studentid='.$studentid.'&type=init">중 3-2</a> |
<a href="https://mathking.kr/moodle/local/augmented_teacher/books/patternbank.php?'.$modeinfo.'&cid=106&nch=1&studentid='.$studentid.'&type=init">공통수학 1</a> |
<a href="https://mathking.kr/moodle/local/augmented_teacher/books/patternbank.php?'.$modeinfo.'&cid=107&nch=1&studentid='.$studentid.'&type=init">공통수학 2</a> |
<a href="https://mathking.kr/moodle/local/augmented_teacher/books/patternbank.php?'.$modeinfo.'&cid=59&nch=1&studentid='.$studentid.'&type=init">수 상</a> |
<a href="https://mathking.kr/moodle/local/augmented_teacher/books/patternbank.php?'.$modeinfo.'&cid=60&nch=1&studentid='.$studentid.'&type=init">수 하</a> |
<a href="https://mathking.kr/moodle/local/augmented_teacher/books/patternbank.php?'.$modeinfo.'&cid=61&nch=1&studentid='.$studentid.'&type=init">수 1</a> |
<a href="https://mathking.kr/moodle/local/augmented_teacher/books/patternbank.php?'.$modeinfo.'&cid=62&nch=1&studentid='.$studentid.'&type=init">수 2</a> |
<a href="https://mathking.kr/moodle/local/augmented_teacher/books/patternbank.php?'.$modeinfo.'&cid=64&nch=1&studentid='.$studentid.'&type=init">확통</a> |
<a href="https://mathking.kr/moodle/local/augmented_teacher/books/patternbank.php?'.$modeinfo.'&cid=63&nch=1&studentid='.$studentid.'&type=init">미적</a> |
<a href="https://mathking.kr/moodle/local/augmented_teacher/books/patternbank.php?'.$modeinfo.'&cid=65&nch=1&studentid='.$studentid.'&type=init">기하</a></td></tr></table> <br> </div>';
...
<body>
	...
	<div class="container">
		<div class="left-column">
			...
			<hr><a href="https://mathking.kr/moodle/local/augmented_teacher/books/patternbank.php?'.$modechange.'cid='.$cid.'&nch=1&studentid='.$studentid.'&type=init "> <img style="margin-bottom:5px;" loading="lazy"  src="https://mathking.kr/Contents/IMAGES/learningpath.png" width=20>&nbsp; <span style="font-size:16px;">'.$modetext.'</span></a> <a href="https://mathking.kr/moodle/local/augmented_teacher/teachers/synergetic.php?id='.$USER->id.'&userid='.$studentid.'"><img style="margin-bottom:10px;" src=https://mathking.kr/moodle/local/augmented_teacher/IMAGES/circulation.png width=40></a><br><br>'.$domchapters.' 
			</div>
		</div>
		<div class="right-column">
		<div class="colsection"><br>
		<table width=80%><tr><td width=5%>'.$timefolding.'</td><td><h3> &nbsp;'.$thischtitle.'</h3> </td><td>'.$todaygoal.'</td> <td width=1%><a href="'.$gpturl.'"target="_blank"><img   style="margin-bottom:0px;" src=https://mathking.kr/moodle/local/augmented_teacher/IMAGES/gpt.png width=25></a>  </td> <td align=center>'.$alt42game.'</td></tr></table>
			...
			<!-- 탭 내용 -->
			<div class="tab-content" id="myTabContent" style="padding: 20px; background-color: #fff; border: 1px solid #dee2e6; border-top: none;">
			  <!-- 중급 탭 -->
			...
			<p style="background-color:lightgrey; margin-top: 20px;">'.$subjectlist.'<br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br></p>
