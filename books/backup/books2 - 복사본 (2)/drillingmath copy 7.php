...
async function generateDetailedThinking() {
    ...
    try {
        ...
        data.questions.forEach((question, index) => {
            ...
            questionsHtml += `
                <div class="question-item">
                    <strong>질문 ${questionNum}:</strong> ${question}
                </div>
            `;
        });
        ...
        // 통합 답변 보기 버튼
        questionsHtml += `
            <button class="show-all-answers-button" onclick="toggleAllAnswers()">
                📖 답변 모두 보기
            </button>
        `;

        // 통합 답변 영역
        questionsHtml += '<div id="all-answers-section" class="all-answers-section" style="display: none;">';
...
function toggleAllAnswers() {
    const allAnswersSection = document.getElementById('all-answers-section');
    const button = document.querySelector('.show-all-answers-button');

    console.log('[drillingmath.php:toggleAllAnswers] Toggling all answers');

    // 답변 영역 토글
    if (allAnswersSection.style.display === 'none' || !allAnswersSection.style.display) {
        // 열기
        allAnswersSection.style.display = 'block';
        button.textContent = '📖 답변 숨기기';
        console.log('[drillingmath.php:toggleAllAnswers] Showing all answers');

        // MathJax 렌더링
        if (typeof MathJax !== 'undefined') {
            MathJax.typesetPromise([allAnswersSection]).catch(function(err) {
                console.error('[drillingmath.php:toggleAllAnswers] MathJax error:', err);
            });
        }
    } else {
        // 닫기
        allAnswersSection.style.display = 'none';
        button.textContent = '📖 답변 모두 보기';
        console.log('[drillingmath.php:toggleAllAnswers] Hiding all answers');
    }
}
    </script> 
</body>
