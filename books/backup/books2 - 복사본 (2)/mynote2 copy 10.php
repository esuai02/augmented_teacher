...
foreach($result as $value)
{
  ...
  elseif($npage==$nthispage)
  {
    ...
    // Extract reflections0 for full audio subtitle
    $fullAudioSubtitle = '';
    if(!empty($cnttext->reflections0)) {
      $fullAudioSubtitle = $cnttext->reflections0;
      error_log(sprintf(
        '[Full Audio Subtitle] File: %s, Line: %d, Length: %d',
        basename(__FILE__),
        __LINE__,
        ...
        if($isListeningTest && $sectionData) {
          ...
          <div class="listening-test-container minimized" id="listeningContainer">
            <div class="listening-header">
              <div style="display: flex; align-items: center; gap: 8px;">
                ...
                <button onclick="toggleSubtitles()" id="subtitleToggleBtn" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border: none; color: white; width: 28px; height: 28px; border-radius: 50%; cursor: pointer; font-size: 14px; display: flex; align-items: center; justify-content: center; transition: all 0.2s; padding: 0;" title="자막 보기/숨기기">📄</button>
              </div>
              <div style="display: flex; align-items: center; gap: 8px;">
          ...
          $audiocnt .= '
              <div id="fullAudioSubtitleContainer" style="
                ...
                color: #333;
                display: none;
              ">'.htmlspecialchars($fullAudioSubtitle).'</div>';
          ...
          <script>
            ...
            function restorePlaybackSpeed() {
              const savedSpeed = localStorage.getItem("playbackSpeed_" + contentsId);
            ...
            function cyclePlaybackSpeed() {
              ...
              // localStorage에 저장
              localStorage.setItem("playbackSpeed_" + contentsId, newSpeed.toString());
            ...
            */
            ...
            if (typeof window.subtitlesVisible === "undefined") {
              const savedSubtitleState = localStorage.getItem("subtitlesVisible_'.$contentsid.'");
            ...
            window.toggleSubtitles = function() {
              ...
              // localStorage에 저장
              localStorage.setItem("subtitlesVisible_'.$contentsid.'", window.subtitlesVisible);
            ...
            (function() {
              ...
              // Replay 버튼 클릭 이벤트 - drillingmath.php 새탭 열기
              const replayBtn = document.getElementById("replaySectionBtn");
              if(replayBtn) {
                // 단일 클릭: drillingmath.php 새탭으로 열기
                replayBtn.addEventListener("click", function(e) {
                  e.stopPropagation(); // 이벤트 버블링 방지
                  console.log("[Replay Click] File: mynote2.php, Line: 1218, Open drillingmath.php for cid:", contentsId, "section:", currentSection);

                  // 현재 구간의 자막 텍스트 가져오기
                  const sectionText = textSectionsData[currentSection] || "";

                  // URL 파라미터로 전달 (encodeURIComponent로 인코딩)
                  const url = "https://mathking.kr/moodle/local/augmented_teacher/books/drillingmath.php?cid=" + contentsId +
                              "&ctype=1&section=" + currentSection +
                              "&subtitle=" + encodeURIComponent(sectionText);

                  console.log("[Replay Click] File: mynote2.php, Line: 1227, URL:", url);
...
(function() {
  try {
    ...
    document.addEventListener(\'DOMContentLoaded\', function() {
      ...
      if (typeof window.subtitlesVisible === \'undefined\') {
        const savedSubtitleState = localStorage.getItem(\'subtitlesVisible_<?php echo $contentsid; ?>\');
        window.subtitlesVisible = savedSubtitleState !== null ? (savedSubtitleState === \'true\') : false;
    ...
    function loadAudioModePreference() {
      ...
      if(serverAudioMode && serverAudioMode !== \'section\') {
        ...
        const localMode = localStorage.getItem(\'audioMode_<?php echo $contentsid; ?>\');
        if(localMode) {
    ...
    function saveAudioModePreference(mode) {
      ...
      localStorage.setItem(\'audioMode_<?php echo $contentsid; ?>\', mode);
