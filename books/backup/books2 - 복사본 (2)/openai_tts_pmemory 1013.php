...
<div class="container">
            ...
            success: function (data, status, xhr) {
                try {
                    ...
                    if (parsed_data.success) {
                        ...
                        outputText.innerHTML += `<p style="color:gray;">파일명: ${parsed_data.url}</p>`;

                        // 업로드 성공 알림
                        playUploadNotification();
                    } else {
                        outputText.innerHTML += `<p style="color:red;">❌ 업로드 실패: ${parsed_data.error || '알 수 없는 오류'}</p>`;
                    }
                } catch (e) {
                    outputText.innerHTML += `<p style="color:red;">❌ 업로드 응답 처리 오류: ${e.message}</p>`;
    ...
    document.querySelector("#startTalk").addEventListener("click", async () => {
        ...
        for (let sentence of sentences) {
            ...
            await generateEnhancedSpeech(cleanedSentence, "alloy", isPractice);
