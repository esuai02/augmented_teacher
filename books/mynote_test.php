...
foreach($result as $value)
{
  ...
  $wboardid0='jnrsorksqcrark'.$contentsid.'_user'.$studentid;
  $wboardid='jnrsorksqcrark'.$contentsid.'_user'.$studentid.'_test';
  ...
  if($pgtype==='quiz')
  {
    ...
    elseif(strpos($title, '유형')!== false) {
      $contentslist2.='<tr><td><a href="https://mathking.kr/moodle/local/augmented_teacher/LLM/editprompt.php?cntid='.$contentsid.'&cnttype=1&studentid='.$studentid.'" target="_blank">'.$headimg.'</a> <a href="https://mathking.kr/moodle/local/augmented_teacher/books/mynote_test.php?dmn='.$domain.'&cid='.$cid.'&nch='.$nch.'&cmid='.$cmid.'&quizid='.$quizid.'&page='.$npage.'&studentid='.$studentid.'">'.$title.'</a>'.$audioicon.$flagicon.'</td></tr>';
    }
    elseif(strpos($title, '복습')!== false) {
      $contentslist3.='<tr><td><a href="https://mathking.kr/moodle/local/augmented_teacher/LLM/editprompt.php?cntid='.$contentsid.'&cnttype=1&studentid='.$studentid.'" target="_blank"><img src="https://mathking.kr/Contents/IMAGES/restore.png" width=15></a> <a href="https://mathking.kr/moodle/local/augmented_teacher/books/mynote_test.php?dmn='.$domain.'&cid='.$cid.'&nch='.$nch.'&cmid='.$cmid.'&quizid='.$quizid.'&page='.$npage.'&studentid='.$studentid.'">'.$title.'</a>'.$audioicon.$flagicon.' <input type="checkbox"  onclick="changecheckbox(1,'.$studentid.','.$mid.', this.checked)"/></td></tr>';
    }
    else {
    ...
    if(strpos($nextpage->title, '유형')!= false && $quizid!=NULL) {
      $nextlearningurl='https://mathking.kr/moodle/local/augmented_teacher/books/mynote_test.php?dmn='.$domain.'&cid='.$cid.'&nch='.$nch.'&cmid='.$cmid.'&quizid='.$quizid.'&page='.$nnextpage.'&studentid='.$studentid;
    }
    elseif($quizid!=NULL) {
  ...
  elseif($npage==$nthispage)
  {
    ...
    if($cnttext->audiourl !== NULL || $cnttext->audiourl2 !== NULL) {
      ...
      if($cnttext->audiourl !== NULL || $cnttext->audiourl2 !== NULL) {
        // 서술평가 모드 확인 (reflections3 필드에 구간 정보가 있는지 확인)
        $isListeningTest = false;
        $sectionData = null;

        // audiourl2만 사용 (서술평가 전용)
        $audioUrlToUse = $cnttext->audiourl2;

        // 서술평가 모드 확인
        if(!empty($cnttext->reflections3)) {
          error_log("DEBUG - reflections3 원본: " . $cnttext->reflections3);

          $decoded = json_decode($cnttext->reflections3, true);
          error_log("DEBUG - JSON 디코드 결과: " . print_r($decoded, true));
          if(isset($decoded['mode']) && $decoded['mode'] === 'listening_test') {
            $isListeningTest = true;
            $sectionData = $decoded;
            error_log("DEBUG - 서술평가 모드 활성화됨. 사용 URL: " . $audioUrlToUse);
          } else {
            error_log("DEBUG - 서술평가 모드 아님. mode 값: " . (isset($decoded['mode']) ? $decoded['mode'] : 'null'));
          }
        } else {
          error_log("DEBUG - reflections3 필드가 비어있음. audiourl2: " . $cnttext->audiourl2);
        }
        
        if($isListeningTest && $sectionData) {
          // 서술평가 맞춤 인터페이스 (구간별 재생)
          $sectionFiles = $sectionData['sections'];
          $textSections = $sectionData['text_sections'];
          $sectionCount = count($sectionFiles);
        } else {
          // reflections3가 없는 경우: 기본 메시지 또는 단일 파일 모드
          // 서술평가 지시사항이 생성되지 않은 상태
          error_log("DEBUG - 서술평가 데이터 없음. '절차기억 생성' 버튼 클릭 필요");

          // 단일 파일 모드로 표시 (audiourl2가 있는 경우)
          if(!empty($audioUrlToUse)) {
            $sectionFiles = array($audioUrlToUse);
            $textSections = array('서술평가 지시사항을 생성하려면 "절차기억 생성" 버튼을 클릭하세요.');
            $sectionCount = 1;
          } else {
            // audiourl2도 없는 경우 - 아무것도 표시하지 않음
            $sectionFiles = null;
            $textSections = null;
            $sectionCount = 0;
          }
        ...
        // UI 렌더링 (sectionCount > 0인 경우에만)
        if($sectionCount > 0 && $sectionFiles) {
          $audiocnt .= '
          <style>
             /* 중앙 하단 플로팅 미니 플레이어 */
          ...
          <script>
            ...
            function restorePlaybackSpeed() {
              const savedSpeed = localStorage.getItem("mynote2_playbackSpeed");
              ...
              if (!audioPlayer || !speedBtn) {
                console.error("[Speed Restore] File: mynote2.php, Line: 710, Elements not found");
                return;
              }
              ...
              if (savedSpeed) {
                ...
                if (speedIndex !== -1) {
                  ...
                  console.log("[Speed Restore] File: mynote2.php, Line: 722, Restored speed:", speed);
                }
              }
            }
            ...
            function cyclePlaybackSpeed() {
              ...
              if (!audioPlayer || !speedBtn) {
                console.error("[Speed Control] File: mynote2.php, Line: 731, Elements not found");
                return;
              }
              ...
              // localStorage에 저장
              localStorage.setItem("mynote2_playbackSpeed", newSpeed.toString());

              console.log("[Speed Control] File: mynote2.php, Line: 749, Speed changed and saved:", newSpeed);
            }
              ...
              if(container.classList.contains("minimized")) {
                ...
                if(audioPlayer && audioPlayer.paused) {
                  audioPlayer.play().catch(e => {
                    console.log("[Auto Resume] File: mynote2.php, Line: 789, Blocked:", e);
                  });
                  console.log("[Interface] File: mynote2.php, Line: 789, Player expanded - Auto resume audio");
                }
              } else {
                // 최소화: 자동 일시정지
                ...
                if(audioPlayer && !audioPlayer.paused) {
                  audioPlayer.pause();
                  console.log("[Interface] File: mynote2.php, Line: 789, Player minimized - Auto pause audio");
                }
              }
            }
            ...
            */
            ...
            if (typeof window.subtitlesVisible === "undefined") {
              const savedSubtitleState = localStorage.getItem("mynote2_subtitlesVisible");
              window.subtitlesVisible = savedSubtitleState !== null ? (savedSubtitleState === "true") : false;
              console.log("[Settings] File: mynote2.php, Line: 956, Subtitle state restored from localStorage (default: closed):", window.subtitlesVisible);
            }
            ...
            window.toggleSubtitles = function() {
              ...
              // localStorage에 저장
              localStorage.setItem("mynote2_subtitlesVisible", window.subtitlesVisible);
              console.log("[Settings] File: mynote2.php, Line: 850, Subtitle state saved to localStorage:", window.subtitlesVisible);
              ...
              if (window.subtitlesVisible) {
                // 자막 표시 모드
                console.log("[Subtitle Toggle] File: mynote2.php, Line: 843, Action: Show All");
                ...
                if (fullAudioSubtitle) {
                  ...
                  if (currentMode === "full") {
                    ...
                    console.log("[Subtitle Toggle] File: mynote2.php, Line: 870, Full Audio Subtitle: Show");
                  }
                }
              } else {
                // 자막 숨기기 모드
                console.log("[Subtitle Toggle] File: mynote2.php, Line: 843, Action: Hide All");
            ...
            (function() {
              ...
              function updateButtonStates() {
                ...
                console.log("[Button States] File: mynote2.php, Line: 931, Prev:", prevBtn.disabled, "Next:", nextBtn.disabled);
              }
              ...
              function switchToSection(newSection) {
                ...
                console.log("[Section Switch] File: mynote2.php, Line: 941, From:", currentSection, "To:", newSection);
                ...
                if(newTextDiv && window.subtitlesVisible) {
                  ...
                  console.log("[Section Switch] File: mynote2.php, Line: 1124, Subtitle shown for section:", currentSection+1);
                } else if(newTextDiv) {
                  console.log("[Section Switch] File: mynote2.php, Line: 1124, Subtitle hidden (user preference)");
                }
                ...
                audioPlayer2.addEventListener("loadeddata", function playNext() {
                  ...
                  if (savedSpeed) {
                    ...
                    console.log("[Section Switch] File: mynote2.php, Line: 1064, Speed restored:", savedSpeed);
                  }

                  audioPlayer2.play().catch(e => console.error("[Audio Error] File: mynote2.php, Line: 1069, Error:", e));
              ...
              prevBtn.addEventListener("click", function() {
                console.log("[Prev Button] File: mynote2.php, Line: 986, Section:", currentSection);
                if(currentSection > 0) {
              ...
              nextBtn.addEventListener("click", function() {
                console.log("[Next Button] File: mynote2.php, Line: 994, Section:", currentSection);
                if(currentSection < sectionCount - 1) {
                  switchToSection(currentSection + 1);
                }
              });
              ...
              progressDots.forEach((dot, index) => {
                ...
                dot.addEventListener("click", function() {
                  ...
                  if(index === currentSection) {
                    console.log("[Dot Click] File: mynote2.php, Line: 1189, Replay current section:", currentSection);
                    ...
                    if(audioPlayer2) {
                      ...
                      if (savedSpeed) {
                        audioPlayer2.playbackRate = parseFloat(savedSpeed);
                        console.log("[Dot Click] File: mynote2.php, Line: 1199, Speed restored:", savedSpeed);
                      }

                      audioPlayer2.play().catch(e => {
                        console.error("[Audio Error] File: mynote2.php, Line: 1203, Replay error:", e);
                      });
                    }
                  } else {
                    // 다른 구간을 클릭한 경우: 해당 구간으로 이동
                    console.log("[Dot Click] File: mynote2.php, Line: 1207, From:", currentSection, "To:", index);
              ...
              if(replayBtn) {
                replayBtn.addEventListener("click", function(e) {
                  ...
                  console.log("[Replay Click] File: mynote2.php, Line: 1391", {
                  ...
                  console.log("[Replay Click] File: mynote2.php, Line: 1411, Full URL:", url);
                  ...
                  setTimeout(() => {
                    ...
                    console.log("[Replay Click] File: mynote2.php, Line: 1418, Overlay opened with nstep:", nstepValue);
                  }, 50);
                });
              }
              ...
              // setTimeout(function() {
              //   audioPlayer2.play().catch(e => console.log("[Auto Play] File: mynote2.php, Line: 1026, Blocked:", e));
              // }, 500);
              console.log("[Auto Play] File: mynote2.php, Line: 1149, Auto-play disabled - user must click play button");
            })();
          </script>';
        }
      }
    ...
    $showpage='https://mathking.kr/moodle/local/augmented_teacher/whiteboard/board_topic.php?id='.$wboardid.'&contentsid='.$contentsid.'&studentid='.$studentid.'&test=on&quizid='.$quizid.'&'.$mynotecurrenturl;
    // $showpage2=$showpage;
    //howpage2=$showpage; 
    ...
    if($npage==1) { 
      $stepbystepcnt='<tr><td align=center><a href="https://mathking.kr/moodle/local/augmented_teacher/whiteboard/board_topic.php?id='.$wboardid.'&contentsid='.$contentsid.'&test=on&contentstype=1&studentid='.$studentid.'" target="_blank">'.$viewcnticon.'</a></td></tr>'; 
      $nextlearningurl='';
    }
    ...
    {
      $nextlearningurl='https://mathking.kr/moodle/local/augmented_teacher/books/mynote_test.php?dmn='.$domain.'&cid='.$cid.'&nch='.$nch.'&cmid='.$cmid.'&quizid='.$quizid.'&pgtype=quiz&page='.$npage.'&studentid='.$studentid;
      $nquizpage=$npage;
    }
            ...
            <tr>
              <td align=left width=22vw style="color:#347aeb;">
                ...
                <div style="display: flex; align-items: center; gap: 8px; flex-wrap: nowrap;">
                  '.$rule.'
                  <button id="musicPlayPauseBtn2" class="music-play-button" onclick="toggleMusicPlayPause()" style="display:none;" title="재생/일시정지">
                    🎵
                  </button>
                </div>
                <br><br>
...
function loadExistingMusic() {
  ...
  console.log("[mynote2.php:loadExistingMusic] thispageid:", thispageid, "studentid:", studentid);
  ...
  $.ajax({
    ...
    success: function(response) {
      console.log("[mynote2.php:loadExistingMusic] Response:", response);
    ...
    error: function(xhr, status, error) {
      console.log("[mynote2.php:loadExistingMusic] Error:", status);
    }
  });
}

function loadMusicFile(url) {
  console.log("[mynote2.php:loadMusicFile] URL:", url);
  ...
  if (!musicPlayer || !musicSource) {
    console.error("[mynote2.php:loadMusicFile] Elements not found");
    return;
  }
  ...
  console.log("[mynote2.php:loadMusicFile] Music loaded successfully");
}
...
function toggleMusicPlayPause() {
  ...
  if (!musicPlayer) {
    console.error("[mynote2.php:toggleMusicPlayPause] Player not found");
    return;
  }
  ...
  if (isMusicPlaying) {
    ...
    if (musicPlayBtn) musicPlayBtn.innerHTML = "🎵";
    isMusicPlaying = false;
    console.log("[mynote2.php:toggleMusicPlayPause] Paused");
  } else {
    musicPlayer.play();
    if (musicPlayBtn) musicPlayBtn.innerHTML = "⏸";
    isMusicPlaying = true;
    console.log("[mynote2.php:toggleMusicPlayPause] Playing");
  }
}
...
function uploadAudioFile()
{
  ...
  input.onchange = function(e) {
    var file = e.target.files[0];
    if (!file) {
      alert("[mynote2.php] 파일이 선택되지 않았습니다.");
      return;
    }
    ...
    if (fileExt !== "mp3" && fileExt !== "wav" && !validTypes.includes(file.type)) {
      alert("[mynote2.php] MP3 또는 WAV 파일만 업로드 가능합니다. (현재: " + fileExt + ")");
      return;
    }

    // Check file size (50MB max)
    if (file.size > 50 * 1024 * 1024) {
      alert("[mynote2.php] 파일 크기가 너무 큽니다. 최대 50MB까지 업로드 가능합니다.");
      return;
    }
    ...
    $.ajax({
      ...
      data: formData,
      success: function(response) {
        console.log("[mynote2.php] Upload response:", response);
        ...
        if (typeof response === "string") {
          try {
            ...
            console.error("[mynote2.php] JSON parse error:", e);
        ...
        if (data.success) {
          ...
          console.log("[mynote2.php:uploadAudioFile] Audio uploaded and loaded:", data.data.url);
        } else {
          Swal.fire("오류", data.message || "업로드 실패", "error");
          console.error("[mynote2.php] Upload failed:", data.message);
        }
      },
      error: function(xhr, status, error) {
        console.error("[mynote2.php] AJAX error:", status, error);
        console.error("[mynote2.php] Response:", xhr.responseText);
...
function generateDialogNarration(contentsid) {
  ...
  // AJAX 요청으로 서술평가 지시사항 생성
  $.ajax({
    url: "generate_essay_instruction.php",
    type: "POST",
    dataType: "json",
  ...
  try {
    ...
    document.addEventListener(\'DOMContentLoaded\', function() {
      ...
      if (typeof window.subtitlesVisible === \'undefined\') {
        const savedSubtitleState = localStorage.getItem(\'mynote2_subtitlesVisible\');
        window.subtitlesVisible = savedSubtitleState !== null ? (savedSubtitleState === \'true\') : false;
        console.log(\'[Settings] File: mynote2.php, Line: 3242, Subtitle state initialized from localStorage (default: closed):\', window.subtitlesVisible);
      }
    ...
    function loadAudioModePreference() {
      ...
      if(serverAudioMode && serverAudioMode !== \'section\') {
        ...
        const localMode = localStorage.getItem(\'mynote2_audioMode\');
        if(localMode) {
    ...
    window.toggleAudioMode = function() {
      console.log(\'[Audio Toggle] File: mynote2.php, Line: 3138, toggleAudioMode called, currentMode:\', currentAudioMode);

      // 플레이어를 매번 다시 가져와서 최신 상태 확인 (중복 ID 문제 해결)
      fullAudioPlayer = document.getElementById(\'audioPlayerFull\');
      sectionAudioPlayer = document.getElementById(\'audioPlayer2\');

      console.log(\'[Audio Toggle] File: mynote2.php, Line: 3143, Players found - full:\', !!fullAudioPlayer, \', section:\', !!sectionAudioPlayer);

      const newMode = (currentAudioMode === \'section\') ? \'full\' : \'section\';
      console.log(\'[Audio Toggle] File: mynote2.php, Line: 3146, Switching from\', currentAudioMode, \'to\', newMode);
      ...
      console.log(\'[Audio Toggle] File: mynote2.php, Line: 3152, shouldPlay:\', shouldPlay);
    ...
    function applyAudioMode(mode, shouldPlay) {
      ...
      if(!toggleBtn || !label) {
        console.warn(\'[Audio Toggle] File: mynote2.php, Line: 3166, Warning: Toggle button or label not found\');
        return;
      }
      ...
      if(mode === \'full\') {
        ...
        if(fullAudioPlayer && sectionAudioPlayer) {
          ...
          if(fullSrc) {
            ...
            console.log(\'[Audio Toggle] File: mynote2.php, Line: 3189, Switched to full audio:\', fullSrc);
          } else {
            console.warn(\'[Audio Toggle] File: mynote2.php, Line: 3191, Warning: Full audio source not found\');
          }
        } else {
          console.warn(\'[Audio Toggle] File: mynote2.php, Line: 3193, Warning: Audio players not found\');
        }

        // Play full audio if requested
        if(shouldPlay && sectionAudioPlayer) {
          sectionAudioPlayer.play().catch(err => {
            console.error(\'[Audio Toggle] File: mynote2.php, Line: 3198, Play failed:\', err);
          });
        }
        ...
        if (fullAudioSubtitleContainer) {
          ...
          if (window.subtitlesVisible !== false) {
            ...
            console.log(\'[Full Audio Subtitle] File: mynote2.php, Line: 2928, Action: Show\');
          } else {
            fullAudioSubtitleContainer.style.display = \'none\';
            console.log(\'[Full Audio Subtitle] File: mynote2.php, Line: 2928, Action: Keep Hidden (user toggled off)\');
          }
        }
        ...
        if (activeSubtitle) {
          ...
          console.log(\'[Listening Test Subtitle] File: mynote2.php, Line: 2939, Action: Hide\');
        }
      } else {
        ...
        if(fullAudioSubtitleContainer) {
          fullAudioSubtitleContainer.style.display = \'none\';
          console.log(\'[Full Audio Subtitle] File: mynote2.php, Line: 2986, Action: Hide\');
        }
        ...
        if (window.subtitlesVisible !== false) {
          ...
          if (subtitleToShow) {
            ...
            // 현재 구간 자막 표시
            subtitleToShow.classList.add(\'active\');
            console.log(\'[Listening Test Subtitle] File: mynote2.php, Line: 2990, Action: Show section\', currentSectionIndex);
          }
        } else {
          ...
          console.log(\'[Listening Test Subtitle] File: mynote2.php, Line: 2990, Action: Keep Hidden (user toggled off)\');
        }
      }
    ...
    function saveAudioModePreference(mode) {
      ...
      localStorage.setItem(\'mynote2_audioMode\', mode);
    ...
    window.playFullAudio = function() {
      ...
      if(!fullAudioPlayer) {
        console.error(\'[Full Audio ERROR] 파일: mynote2.php, 위치: playFullAudio\');
    ...
    window.pauseFullAudio = function() {
      ...
      if(!fullAudioPlayer) {
        console.error(\'[Full Audio ERROR] 파일: mynote2.php, 위치: pauseFullAudio\');
        return;
      }
    ...
    if(fullAudioPlayer) {
      ...
      fullAudioPlayer.addEventListener(\'error\', function(e) {
        console.error(\'[Full Audio ERROR] 파일: mynote2.php, 위치: audio player error event\');
        console.error(\'[Full Audio ERROR] Error loading audio:\', e);
      });
    }

  } catch(error) {
    console.error(\'[Audio Toggle ERROR] 파일: mynote2.php, 위치: audio toggle initialization\');
