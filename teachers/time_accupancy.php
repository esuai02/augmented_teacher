...
global $SITE, $CFG;
  ...
  //  $a->username     = 'tslee';
   // $a->userusername = 'ktm';
    ...
    $eventdata->courseid          = 12;
    $eventdata->component         = 'mod_quiz';
    $eventdata->name              = 'confirmation';
    $eventdata->notification      = 1;

    $eventdata->userfrom          = core_user::get_noreply_user();
    $eventdata->userto            = 3;//$recipient;
    $eventdata->subject           ='aaaa';//get_string('emailconfirmsubject', 'quiz', $a);
    $eventdata->fullmessage       = 'aaaa';//get_string('emailconfirmbody', 'quiz', $a);
    $eventdata->fullmessageformat = FORMAT_PLAIN;
    $eventdata->fullmessagehtml   = '';

    $eventdata->smallmessage      ='aaaa';//get_string('emailconfirmsmall', 'quiz', $a);
    $eventdata->contexturl        = 'aaaa';//$a->quizurl;
    $eventdata->contexturlname    ='aaaa';// $a->quizname;
...
$message->userfrom = 827; // If the message is 'from' a specific user you can set them here
$message->userto = 2;
...
$message->contexturl = 'aaaaa'; //(new \moodle_url('/course/'))->out(false); // A relevant URL for the notification
$message->contexturlname = 'Course list'; // Link title explaining where users get to for the contexturl
$content = 'aaaa'; //array('*' => array('header' => ' test ', 'footer' => ' test ')); // Extra content for specific processor
$message->set_additional_content('email', $content);
$messageid = message_send($message);
 */
$username= $DB->get_record_sql("SELECT lastname, firstname FROM mdl_user WHERE id='$teacherid' ");
if($role==='student') $mystudents=$DB->get_records_sql("SELECT id, lastname, firstname FROM mdl_user WHERE institution LIKE '$academy' AND   suspended=0 AND  (lastaccess> '$amonthago' OR timecreated> '$amonthago' ) AND firstname LIKE '%$tsymbol%' ORDER BY id DESC ");  
else $mystudents=$DB->get_records_sql("SELECT id, lastname, firstname FROM mdl_user WHERE institution LIKE '$academy' AND   suspended=0 AND  (lastaccess> '$amonthago' OR timecreated> '$amonthago' ) AND  (firstname LIKE '%$tsymbol%' OR firstname LIKE '%$tsymbol1%'  OR firstname LIKE '%$tsymbol2%' OR firstname LIKE  '%$tsymbol3%' ) ORDER BY id DESC ");   
  		...
  		if($var2!=NULL && $var4!=NULL &&  $var4!=0)
			...
			$n2=(int)(($minutes-530)/30);	
	 		...
	 		for($ntime=0;$ntime<$ntimemax;$ntime++)
				...
				$name[$n1][$n2+$ntime].=$stdname;
 				if($naccupancy[$n1][$n2+$ntime]>=8)$name[$n1][$n2+$ntime]='<img src="https://mathking.kr/Contents/Moodle/Visual%20arts/reddot.png" width=15> '.$naccupancy[$n1][$n2+$ntime];
				elseif($naccupancy[$n1][$n2+$ntime]<=4)$name[$n1][$n2+$ntime]='<img src="https://mathking.kr/Contents/Moodle/Visual%20arts/greendot.png" width=15> '.$naccupancy[$n1][$n2+$ntime];
				else $name[$n1][$n2+$ntime]='<img src="https://mathking.kr/Contents/Moodle/Visual%20arts/bluedot.png" width=15> '.$naccupancy[$n1][$n2+$ntime];
		  	 	}
			}
		}
	}
												...
												<div class="invoice-item">
													<div class="table-responsive">
														<table class="table table-striped">
															<thead>
																<tr>
<thead><tr><td></td><td></td><td></td><td></td><td></td><td align=right>총 '.$size.'명</td><td></td><td><a href="https://mathking.kr/moodle/local/augmented_teacher/teachers/timetable.php?id='.$teacherid.'&tb=7">시간표</a></td><td><a href="https://mathking.kr/moodle/local/augmented_teacher/teachers/time_accu_detail.php?id='.$teacherid.'&tb=7">점유명단</a></td></tr></thead>
															<tbody>
...
<tr style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis;" ><td>~10시</td><td>'.$name[1][0].$usrname[1][0].'</td><td>'.$name[2][0].$usrname[2][0].'</td><td>'.$name[3][0].$usrname[3][0].'</td><td>'.$name[4][0].$usrname[4][0].'</td><td>'.$name[5][0].$usrname[5][0].'</td><td>~10시</td><td>'.$name[6][0].$usrname[6][0].'</td><td>'.$name[7][0].$usrname[7][0].'</td></tr>															
