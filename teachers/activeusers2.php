...
$teacher3=$DB->get_record_sql("SELECT data AS symbol FROM mdl_user_info_data where userid='$teacherid' AND fieldid='76' "); 
...
$mystudents=$DB->get_records_sql("SELECT id, lastname, firstname FROM mdl_user WHERE  lastaccess >'$hoursago1'  AND  (firstname LIKE '%$tsymbol%' OR firstname LIKE '%$tsymbol2%'  OR firstname LIKE '%$tsymbol3%' ) ORDER BY id DESC ");  //
...
foreach($userlist as $user)
	...
	if($timeafter<7200)  // active user 들에 대한 정보만 가지고 오기
		...
		*/
		...
		$handwriting=$DB->get_records_sql("SELECT * FROM mdl_abessi_messages WHERE  userid='$studentid' AND (status LIKE 'ask' OR status LIKE 'studentreply')  ORDER BY tlaststroke DESC LIMIT 3");
		...
		foreach($result as $value) 
			...
			if($status==='weekly' || $status==='examplan' || $status==='summary')
				{
				$wbtype='board_brainstorm.php';
				$questiontext='<img src="https://c.tenor.com/UyX1ahhXlh0AAAAC/study-assignment.gif" width=100>▶';
				}
 
			$wblist1.='&nbsp;&nbsp; <a href="https://mathking.kr/moodle/local/augmented_teacher/whiteboard/'.$wbtype.'?id='.$encryption_id.'" target="_blank"><div class="tooltip2">'.$attemptstep.' '.$imgstatus.'<span class="tooltiptext2"><table style="" align=center><tr><td align=center>'.$tlaststroketext.' | '.$timemodifiedtext.' 업데이트 <hr>'.$questiontext.'</td></tr></table></span></div></a> | ';
 		 	}
		// 질문 하나 가지고 오기
		...
		$guideme=$DB->get_record_sql("SELECT * FROM mdl_abessi_messages WHERE teacher_check NOT LIKE 2 AND userid='$studentid'  AND tlaststroke > '$timedelayed' AND tlaststroke < '$timedelayed' AND (status LIKE 'attempt' OR status LIKE 'begin' OR status LIKE 'exam') ORDER BY tlaststroke ASC LIMIT 1");
		if($guideme->id!=NULL)
			{
 			$contentsid=$guideme->contentsid;
			if($guideme->contentstype==2)
				{ 
				$qtext = $DB->get_record_sql("SELECT questiontext FROM mdl_question WHERE id='$contentsid' ");
				$htmlDom = new DOMDocument; @$htmlDom->loadHTML($qtext->questiontext); $imageTags = $htmlDom->getElementsByTagName('img'); $extractedImages = array();
			...
			$cognitivekick.='<td><a href="https://mathking.kr/moodle/local/augmented_teacher/whiteboard/'.$wbtype.'?id='.$guideme->wboardid.'" target="_blank"><div class="tooltip2"> <b>'.$lastname.'</b>의 질문<span class="tooltiptext2"><table style="" align=center><tr><td align=center>'.$tlaststroketext.' | '.$timemodifiedtext.' 업데이트 <hr>'.$questiontext.'</td></tr></table></span></div></a> &nbsp;</td>';
			}
...
foreach($result2 as $value2) 
	...
	if($finalresult->state==='complete')
		...
		if(strpos($rightanswer,$ans)===false)		
			...
			$assess.=' <a href="https://mathking.kr/moodle/local/augmented_teacher/students/today.php?id='.$userid.'&tb=604800" target="_blank" >'.$stdname.'</a> &nbsp;<a href="https://mathking.kr/moodle/local/augmented_teacher/whiteboard/board.php?id='.$atmptboard.' "target="_blank">(<img src=https://mathking.kr/Contents/Moodle/Visual%20arts/pill3.png width=15> '.$nslot.'번)</a> | ';
			}
		}
	}
