...
echo ' 		<div class="main-panel">
	...
	$quizattempts = $DB->get_records_sql("SELECT *,mdl_quiz_attempts.ratio AS ratio1, mdl_quiz_attempts.comment AS comment, mdl_quiz.sumgrades AS tgrades, mdl_quiz.course AS courseid FROM mdl_quiz  LEFT JOIN mdl_quiz_attempts ON  mdl_quiz.id=mdl_quiz_attempts.quiz  
	...
	$quizlist3='<table width=90%><tr><th></th><th></th><th></th><th></th><th></th></tr>';
	unset($value); 	
	foreach(array_reverse($quizresult) as $value)
		{
		$studentid=$value['userid'];
		$thisstudent=$DB->get_record_sql("SELECT  *  FROM mdl_user WHERE id='$studentid' ");

		
		if(strpos($thisstudent->firstname, $tsymbol)!== false || strpos($thisstudent->firstname, $tsymbol1)!== false || strpos($thisstudent->firstname, $tsymbol2)!== false || strpos($thisstudent->firstname, $tsymbol3)!== false)
		{ 
		...
		if($quizgrade<90 && $thisstudent->id!=NULL && $value['comment']==NULL && $value['courseid']!=239 )  
		{
		$comment='';
		...
		if($ratio1==0 && $Qnum2==0) $imgtoday='https://mathking.kr/Contents/MATH%20MATRIX/MATH%20images/IMG/bessi21_1645446736.png';
		...
		$imgstatus=$imgstatus.'<img style="margin-bottom:6px;" src="'.$imgtoday.'" width=20>';	
		if(strpos($quizname, '내신')!== false) 
			{
			$inspect='결과분석 '.$imgstatus;
			if($value['state']===inprogress){$inspect='시도중 ...';}
