...
if($tbegin==NULL)$handwriting=$DB->get_records_sql("SELECT * FROM mdl_abessi_messages WHERE userid='$studentid' AND active='1' AND timemodified > '$hoursago'   ORDER BY timecreated DESC LIMIT 100");
else $handwriting=$DB->get_records_sql("SELECT * FROM mdl_abessi_messages WHERE userid='$studentid' AND active='1' AND timemodified > '$tbegin' AND timemodified < '$tend'   ORDER BY timecreated DESC LIMIT 100");
...
foreach($result as $value) 
	...
	if(strpos($wboardid, 'jnrsorksqcrark')!== false)
		...
		$imagegrid.='<div class="column1"><a href="https://mathking.kr/moodle/local/augmented_teacher/books/mynote.php?'.$noteurl.'"target="_blank"><img loading="lazy" src="'.$imgSrc.'" style="width:100%"></a><table align=right><tr><th>'.$value['status'].'</th></tr></table></div>';
		}
	else
		{
		...
		$imagegrid.='<div class="column2"><a href="https://mathking.kr/moodle/local/augmented_teacher/whiteboard/board_onair.php?mode=1&userid='.$studentid.'&wboardid='.$wboardid.'"target="_blank"><img loading="lazy" src="'.$imgSrc.'" style="width:100%"></a><table align=right><tr><th>'.$value['status'].'</th></tr></table></div>';
		} 
