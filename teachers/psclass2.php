...
$info=$DB->get_record_sql("SELECT data FROM mdl_user_info_data where userid='$USER->id' AND fieldid='46' "); 
$academy=$info->data;
	...
	foreach($result as $user)
		...
		$lastaccess=time()-$lastaction;
 		...
 		if($lastaccess<10800 || $var2!=NULL  && $var4>0 && (time()- $schedule->timecreated)<86400000)
			...
			자율모드 이상 해제 방법 : 일시복구 (30분) - 연속 3개 정답(로켓상태), 마지막 테스트 80점 넘기기, 발표활동. 영구복구 - 평점복구, 주간성취 복구
			...
			*/
			...
			if(($lastaccess<10800||$npresent==$n2+1||$npresent==$n2+2||$npresent==$n2+3||$npresent==$n2+4||$npresent==$n2+5|$npresent==$n2+6||$npresent==$n2+7||$npresent==$n2+8||$npresent==$n2+9||$npresent==$n2+10||$npresent==$n2) && $nday==$n1)
				...
				if($lastaccess>36000)
					...
					if($autoGradeState==='AI' && $timecreated-$indicators->tinspect>=$inspecttime && $nviewtoday<4 && $inspect->data!=NULL && $indicators->aion==1 )
						{ 
						$nviewtoday++;
						echo '<script>
							var Userid= \''.$userid.'\';
					...
					elseif($useinfo->data==NULL && $nviewtoday<4 && $inspect->data==NULL && $indicators->aion==1)
						{
						$nviewtoday++;
						echo '<script>
							var Userid= \''.$userid.'\';
...
<td align=center><button type="button"   style="background-color:#fff599;color:black;width:100%;height:40px;"><a href="https://mathking.kr/moodle/local/augmented_teacher/teachers/psclass.php?id=2&tb=7&mode=today&update=180">3분'.$upicon1.'</a> | <a href="https://mathking.kr/moodle/local/augmented_teacher/teachers/psclass.php?id=2&tb=7&mode=today&update=300">5분'.$upicon2.'</a> | <a href="https://mathking.kr/moodle/local/augmented_teacher/teachers/psclass.php?id=2&tb=7&mode=today&update=600">10분'.$upicon3.'</a> | <a href="https://mathking.kr/moodle/local/augmented_teacher/teachers/psclass.php?id=2&tb=7&mode=today&update=604800">멈춤'.$upicon4.'</a> | (총 '.$size.'명)  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;열공 (<b>'.$nenergy.'</b>) &nbsp;&nbsp;&nbsp;성장 (<b>'.$ngrowth.'</b>)
&nbsp;&nbsp;&nbsp;완결 (<b>'.$ncomplete.'</b>) &nbsp;&nbsp;&nbsp;상호작용  (<b>'.$ninteraction.'</b>)  &nbsp;&nbsp;&nbsp;<a href="https://mathking.kr/moodle/mod/checklist/view.php?id=89264&forceview=1"target="_blank">교수법</a>&nbsp;&nbsp;&nbsp;<a href="https://mathking.kr/moodle/local/augmented_teacher/teachers/softlanding.php?id='.$teacherid.'"target="_blank">신규생</a></button></td><td><button type="button"  style="background-color:#fff599;color:black;width:100%;height:40px;">체크 포인트</button></td></tr></thead>
									<tr><td style="vertical-align: top"><table width=95% align=center>';
//if($name[$n1][0]!=NULL || $nprint==1){}
echo '<tr><td width=15%><b style="color:blue;">보강</b></td><td><table width=95%>'.$showattendlog.'</table></td></tr><tr><td width=15%><hr></td><td><hr></td></tr>';
