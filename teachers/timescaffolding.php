...
$dayago=$timecreated-86400; // 24시간 전
$weekago=$timecreated-604800; // 1주일 전 
...
// 현재 사용자(선생님) 이름 가져오기
$currentTeacher = $DB->get_record_sql("SELECT lastname, firstname FROM mdl_user WHERE id='$USER->id' ");
$teacherName = ($currentTeacher->lastname ?? '').($currentTeacher->firstname ?? '');
...
{	 
    ...
    { 
        ...
        $lefttime_seconds = $value['duration'] - $timecreated; // 남은 시간을 초 단위로 계산
        if($lefttime_seconds < 0) $lefttime_seconds = 0; // 음수 방지
        $lefttime_minutes = floor($lefttime_seconds / 60); // 분
        $lefttime_secs = floor(($lefttime_seconds % 60) / 10) * 10; // 초를 10초 단위로 반올림
        if($lefttime_secs < 0) $lefttime_secs = 0;
        ...
        $duetime = '<div style="float: right; white-space: nowrap;" id="second">('.$lefttime_minutes.'분 '.$lefttime_secs.'초 남음)</div>';
        $statuscolor='#e0e0e0'; $rowheight='50px';
        echo '<script>
        (function() {
            // 고유 네임스페이스로 변수 격리 (timescaffolding.php:133)
            var timerId = \'timer_\' + \''.$studentid.'\';
            var initialSeconds = '.$lefttime_seconds.'; // 초기 남은 시간 (timescaffolding.php:134)
            if (initialSeconds < 0) initialSeconds = 0; // 음수 방지 (timescaffolding.php:135)
            var startTime = Date.now(); // 시작 시간 저장 (timescaffolding.php:136)
            var Userid = \''.$studentid.'\'; 
            var Inputtext = \''.$trackingtext.'\'; 
            var auto_refresh = null; // 타이머 참조 (timescaffolding.php:137)

            function formatTime(seconds) {
                if (seconds < 0) seconds = 0;
                var minutes = Math.floor(seconds / 60);
                var secs = Math.floor((seconds % 60) / 10) * 10; // 10초 단위로 반올림
                if (secs < 0) secs = 0;
                return {minutes: minutes, seconds: secs};
            }
            
            function getRemainingSeconds() {
                // 실제 경과 시간 기반으로 남은 시간 계산 (timescaffolding.php:148)
                var elapsedMs = Date.now() - startTime;
                var elapsedSeconds = Math.floor(elapsedMs / 1000);
                var remaining = initialSeconds - elapsedSeconds;
                return remaining > 0 ? remaining : 0;
            }
            
            function updateTitle() {
                var totalSeconds = getRemainingSeconds();
                var time = formatTime(totalSeconds);
                var timeStr = time.minutes + "분 " + time.seconds + "초";
                
                if (totalSeconds > 180) { // 3분 초과
                    document.title = "🟢수학일기(" + timeStr + ")";
                } else if (totalSeconds >= 0) { // 0초 이상 3분 이하
                    document.title = "🟡수학일기(" + timeStr + ")";
                } else { // 음수
                    document.title = "🔴수학일기(" + timeStr + ")";
                }
            ...
            function updateDisplay() {
                var totalSeconds = getRemainingSeconds();
                var time = formatTime(totalSeconds);
                var newcontent = "(" + time.minutes + "분 " + time.seconds + "초 남음)";
                if ($("#second").length) {
            ...
            function checkAndUpdate() {
                ...
                if (totalSeconds <= 180 && totalSeconds > 0 && totalSeconds % 30 === 0) {
                    alertTime(\''.$studentid.'\');
                    $.ajax({
                        url:"check.php",
                        type: "POST",
                ...
                else if(totalSeconds % 1800 === 0 && totalSeconds > 0) { // 30분마다
                    $.ajax({
                        url:"check.php",
                        type: "POST",
                        dataType:"json",
            ...
            // 10초마다 업데이트 (timescaffolding.php:229)
            auto_refresh = setInterval(checkAndUpdate, 10000);
            
            // 탭이 다시 활성화될 때 시간 재계산 (timescaffolding.php:230)
            ...
            // 페이지 포커스 시에도 업데이트 (timescaffolding.php:231)
...
$goalid=$checkgoal->id;
$headtext='    |   <a href="https://mathking.kr/moodle/local/augmented_teacher/cjnstudents/mypersonas.php?userid='.$studentid.'"target="_blank">🎭</a> | <a href="https://mathking.kr/moodle/local/augmented_teacher/students/attendancerecords.php?userid='.$studentid.'"target="_blank">출결</a>';
if($role!=='student') {
    $headtext .= '  | <a href="https://mathking.kr/moodle/local/augmented_teacher/teachers/timescaffolding.php?userid='.$studentid.'&tp=7257600">최근 3개월</a>';
}
$headtext .= '   <a href="https://mathking.kr/moodle/local/augmented_teacher/alt42/studenthome/contextual_agents/beforegoinghome/dashboard.php?userid='.$studentid.'"target="_blank">🟩귀검결과</a>';
...
// 최근 24시간 내 활동 정보 조회
try {
    $recent24hCount = $DB->count_records_sql("
        SELECT COUNT(*) 
        FROM mdl_abessi_tracking 
        WHERE userid = ? AND timecreated >= ? AND hide = 0
    ", [$studentid, $dayago]);
    
    $recent24hCompleted = $DB->count_records_sql("
        SELECT COUNT(*) 
        FROM mdl_abessi_tracking 
        WHERE userid = ? AND timecreated >= ? AND hide = 0 AND status = 'complete'
    ", [$studentid, $dayago]);
    
    $recent24hInProgress = $DB->count_records_sql("
        SELECT COUNT(*) 
        FROM mdl_abessi_tracking 
        WHERE userid = ? AND timecreated >= ? AND hide = 0 AND status = 'begin'
    ", [$studentid, $dayago]);
} catch (Exception $e) {
    error_log('Error fetching 24h activities in timescaffolding.php (line ' . __LINE__ . '): ' . $e->getMessage());
    $recent24hCount = 0;
    $recent24hCompleted = 0;
    $recent24hInProgress = 0;
}

$recent24hInfo = '';
if ($recent24hCount > 0) {
    $recent24hInfo = '<div style="background: #e3f2fd; padding: 1rem; border-radius: 0.5rem; margin-bottom: 1rem; border-left: 4px solid #2196f3;">
        <h3 style="margin-top: 0; color: #1976d2; font-size: 1.1rem;">📊 최근 24시간 활동 현황</h3>
        <p style="margin: 0.5rem 0;"><strong>총 활동:</strong> ' . $recent24hCount . '개</p>
        <p style="margin: 0.5rem 0;"><strong>완료:</strong> ' . $recent24hCompleted . '개</p>
        <p style="margin: 0.5rem 0;"><strong>진행 중:</strong> ' . $recent24hInProgress . '개</p>
    </div>';
} else {
    $recent24hInfo = '<div style="background: #fff3e0; padding: 1rem; border-radius: 0.5rem; margin-bottom: 1rem; border-left: 4px solid #ff9800;">
        <p style="margin: 0; color: #e65100;">최근 24시간 내 활동이 없습니다.</p>
    </div>';
}

// 최근 1주일 이내 선생님 메모 조회 (timelineWeek.php 참고)
$teacherNotes = $DB->get_records_sql(
    "SELECT sn.*, uid.data AS author_role 
     ...
     AND sn.created_at >= ? AND (uid.data IS NULL OR uid.data != 'student')
...
if (!empty($teacherNotes)) {
    $teacherNotesHTML = '<div style="background: #f3e5f5; padding: 1rem; border-radius: 0.5rem; margin-bottom: 1rem; border-left: 4px solid #9c27b0;">
        <h3 style="margin-top: 0; color: #7b1fa2; font-size: 1.1rem;">📝 최근 1주일 선생님 메모</h3>';
    
    foreach ($teacherNotes as $note) {
        $noteArray = (array)$note;
        $noteDate = date('Y-m-d H:i', $noteArray['created_at']);
        $noteContent = isset($noteArray['content']) ? htmlspecialchars($noteArray['content'], ENT_QUOTES, 'UTF-8') : '';
        
        // 이미지 태그가 있으면 HTML 그대로 사용
        if (strpos($noteContent, '<img') !== false) {
            $noteContent = $noteArray['content'];
        }
        
        $teacherNotesHTML .= '<div style="background: white; padding: 0.75rem; margin: 0.5rem 0; border-radius: 0.25rem; border: 1px solid #e1bee7;">
            <div style="font-size: 0.85rem; color: #666; margin-bottom: 0.25rem;">' . $noteDate . '</div>
            <div style="font-size: 0.95rem; color: #333;">' . $noteContent . '</div>
        </div>';
    }
    
    $teacherNotesHTML .= '</div>';
} else {
    $teacherNotesHTML = '<div style="background: #fff3e0; padding: 1rem; border-radius: 0.5rem; margin-bottom: 1rem; border-left: 4px solid #ff9800;">
        <p style="margin: 0; color: #e65100;">최근 1주일 내 선생님 메모가 없습니다.</p>
    </div>';
}
 
if($mode==='parental') {
    echo '<div class="top-menu" style="text-align: center; margin: 20px 0; padding: 10px 0;">
            <table align="center" style="margin: 0 auto;">
                <tr>
                    ...
                    <td><a href="https://mathking.kr/moodle/local/augmented_teacher/teachers/mathpomodoro.php?userid='.$studentid.'" class="btn btn-sm btn-danger">일지</a></td>
          ...
          <div style="text-align: right; margin-bottom: 10px;">'.$teachereval.'</div>
          '.$recent24hInfo.'
          '.$teacherNotesHTML.'
         <table align="left" width="80%" style="table-layout: auto;">
         <tr><td> </td><td width="60%"> </td><td style="width: 20px;"></td><td align="center" style="white-space: nowrap; padding: 0 12px; min-width: 60px; width: auto;">Plan</td><td align="center" style="white-space: nowrap; padding: 0 12px; min-width: 60px; width: auto;">Final</td><td align="center" style="white-space: nowrap; padding: 0 12px; min-width: 60px; width: auto;">상태</td><td style="width: 20px;"></td></tr>
    ...
    echo '<table align=center width=90%>
         ...
         <table align=center width=70%>
            <tr><td>
                ...
                <td>DMN 휴식 <input type="checkbox" id="dmnRestCheckbox" name="checkAccount" '.$status4.' onClick="Resttime(33,\''.$studentid.'\',\''.$goalid.'\', this.checked)"/> |  <a href="https://blog.naver.com/esuai02/223627321735" target="_blank">포모도르 공부법</a>  <audio controls style="margin-top:5px;width:150px;height:20px;" 
...
if($role!=='student') {
    echo '<td>
                    <button onclick="evaluateStudentAttitude(\''.$studentid.'\')"
                            style="white-space: nowrap;background-color:rgb(20, 163, 75); border: none; color: white; padding: 5px 10px;
...
echo '   <td>  &nbsp;&nbsp;&nbsp;  <a href="https://mathking.kr/moodle/local/augmented_teacher/alt42/studenthome/contextual_agents/beforegoinghome/index.php?userid='.$studentid.'"><button style="background-color:rgb(20, 163, 75); border: none; color: white; padding: 5px 10px; text-align: center; font-size: 14px; cursor: pointer; border-radius: 5px; font-weight: bold;">귀가검사</button></a>   </td>          <td align=right style="color:lightgrey;">'.$headtext.'</td>   
            </tr> 
         </table>
         ...
         <table width=100% style="table-layout: auto;">
            <tr>
                ...
                <td align="center" style="white-space: nowrap; padding: 0 12px; min-width: 60px; width: auto;">Plan</td>
                <td align="center" style="white-space: nowrap; padding: 0 12px; min-width: 60px; width: auto;">Final</td>
                <td align="center" style="white-space: nowrap; padding: 0 12px; min-width: 60px; width: auto;">상태</td>
...
function Resttime(Eventid,Userid,Goalid,Checkvalue)
{
    ...
    var TimebeforeFinish= 1000; // 큰 값으로 설정 (귀가시간 체크 비활성화)
    if(Checkvalue==true)
    {
        checkimsi = 1;
        if(Timeleft<0)
        {
            Swal.fire({
                ...
                html:
                \'<div style="width:100%;height:600px;overflow:hidden;"><iframe id="dmnRestFrame" style="border: 1px none; z-index:2; width:100%;height:100%;margin:0;padding:0;"   src="https://mathking.kr/moodle/local/augmented_teacher/students/DMN_time.php?id='.$studentid.'" ></iframe></div>\',
                allowOutsideClick: true,
                allowEscapeKey: true,
                showConfirmButton: false,
                didClose: function() {
                    // 팝업이 닫힐 때 (OK 클릭, 빈공간 클릭, ESC 등) 휴식종료와 동일하게 동작
                    var iframe = document.getElementById(\'dmnRestFrame\');
                    if(iframe && iframe.contentWindow) {
                        // iframe 내부의 휴식종료 버튼 클릭과 동일하게 동작
                        try {
                            iframe.contentWindow.postMessage(\'forceEnd\', \'*\');
                        } catch(e) {
                            console.log(\'Error in timescaffolding.php:718 - iframe message failed: \' + e.message);
                        }
                    }
                }
            }).then((result) => {
                ...
                if(iframe && iframe.contentWindow) {
                    try {
                        ...
                        console.log(\'Error in timescaffolding.php:726 - iframe message failed: \' + e.message);
                    }
                }
            });
            ...
            window.addEventListener(\'message\', function(event) {
                if(event.data === \'restEnd\' || event.data === \'feedbackShown\') {
                    // 휴식 종료 또는 피드백 표시 완료
                    console.log(\'Rest ended or feedback shown\');
                    
                    if(event.data === \'restEnd\') {
                        // 종료 클릭 시 2초 후 팝업 자동 닫기
                        setTimeout(function() {
                            Swal.close();
                            
                            // DMN 휴식 체크박스 해제
                            var dmnCheckbox = document.getElementById(\'dmnRestCheckbox\');
                            if(dmnCheckbox && dmnCheckbox.checked) {
                                dmnCheckbox.checked = false;
                                // 휴식 종료 처리 (체크 해제와 동일하게)
                                Resttime(33, \''.$studentid.'\', \''.$goalid.'\', false);
                            }
                        }, 2000);
                    } else if(event.data === \'feedbackShown\') {
                        // 피드백 표시 완료 시 체크박스만 해제 (팝업은 이미 닫힘)
                        var dmnCheckbox = document.getElementById(\'dmnRestCheckbox\');
                        if(dmnCheckbox && dmnCheckbox.checked) {
                            dmnCheckbox.checked = false;
                            // 휴식 종료 처리 (체크 해제와 동일하게)
...
.waiting-list-panel.open {
    left: 0;
}

.waiting-list-panel-header {
...
.waiting-list-panel-header .close-btn {
...
.waiting-list-panel-header .close-btn:hover {
    background: rgba(255,255,255,0.3);
}

.waiting-list-panel-content {
    height: calc(100% - 50px);
    overflow: hidden;
}

.waiting-list-panel-content iframe {
    width: 100%;
    height: 100%;
    border: none;
}

.waiting-list-toggle-btn {
    position: fixed;
    left: 0;
    bottom: 0;
    width: 40px;
...
.waiting-list-toggle-btn:hover {
    background: linear-gradient(135deg, #764ba2 0%, #667eea 100%);
}
</style>';

echo '<div class="waiting-list-panel" id="waitingListPanel">
    <div class="waiting-list-panel-header">
        <span>질문 대기 목록 ('.$teacherName.')</span>
        <button class="close-btn" onclick="toggleWaitingListPanel()" title="닫기">×</button>
    </div>
    <div class="waiting-list-panel-content">
        <iframe 
            id="waitingListIframe"
            src="https://mathking.kr/moodle/local/augmented_teacher/teachers/waitinglist.php?studentid='.$studentid.'" 
            frameborder="0"
            scrolling="yes">
        </iframe>
    </div>
</div>
<button class="waiting-list-toggle-btn" id="waitingListToggleBtn" onclick="toggleWaitingListPanel()" title="대기 목록 열기">
    대기목록
</button>
<div id="tabNotificationDot" style="position: fixed; top: 10px; right: 10px; width: 20px; height: 20px; background-color: #ff0000; border-radius: 50%; display: none; animation: blinkDot 1s infinite; z-index: 10000; box-shadow: 0 0 10px rgba(255,0,0,0.5); pointer-events: none;"></div>';
...
echo '<script>
// 대기 목록 패널 토글 함수
function toggleWaitingListPanel() {
    var panel = document.getElementById("waitingListPanel");
    var toggleBtn = document.getElementById("waitingListToggleBtn");
    
    if (panel.classList.contains("open")) {
        panel.classList.remove("open");
        toggleBtn.textContent = "대기목록";
        toggleBtn.style.left = "0";
        localStorage.setItem("waitingListPanelOpen", "false");
    } else {
        panel.classList.add("open");
        toggleBtn.textContent = "닫기";
        toggleBtn.style.left = "320px";
        localStorage.setItem("waitingListPanelOpen", "true");
    }
}

// 페이지 로드 시 상태 복원 (기본값: 열림)
document.addEventListener("DOMContentLoaded", function() {
    var panel = document.getElementById("waitingListPanel");
    var toggleBtn = document.getElementById("waitingListToggleBtn");
    var savedState = localStorage.getItem("waitingListPanelOpen");
    
    // 저장된 상태가 없거나 "true"이면 열림 (새로고침 시 자동으로 열림)
    if (savedState !== "false") {
        panel.classList.add("open");
        toggleBtn.textContent = "닫기";
        toggleBtn.style.left = "320px";
    ...
    // iframe 새로고침 및 알림 체크 함수
    var waitingListIframe = document.getElementById("waitingListIframe");
    var tabNotificationDot = document.getElementById("tabNotificationDot");
    var originalTitle = document.title;
    var titleBlinkInterval = null;
    
    function checkWaitingListPosition() {
        try {
            // iframe이 로드되었는지 확인
            if (waitingListIframe.contentWindow && waitingListIframe.contentWindow.currentStudentPosition !== undefined) {
                var position = waitingListIframe.contentWindow.currentStudentPosition;
                
                // 앞에 한 명만 있을 때 (순서가 2번째일 때) 알림 표시
                if (position === 2) {
                    // 빨간색 원 표시
                    tabNotificationDot.style.display = "block";
                    ...
                    // 알림 숨김
                    tabNotificationDot.style.display = "none";
                    if (titleBlinkInterval) {
            ...
            // iframe 접근 오류 (같은 도메인이지만 보안 정책으로 인한 오류 가능)
            console.log("iframe 접근 오류 (timescaffolding.php): " + e.message);
        }
    }
    
    // iframe 로드 완료 후 체크
    waitingListIframe.addEventListener("load", function() {
        setTimeout(checkWaitingListPosition, 500); // iframe 내부 스크립트 실행 대기
    });
    
    // 초기 체크
    setTimeout(checkWaitingListPosition, 1000);
    
    // 60초마다 iframe 새로고침 및 체크
    setInterval(function() {
        var currentSrc = waitingListIframe.src;
        waitingListIframe.src = currentSrc; // iframe 새로고침
        setTimeout(checkWaitingListPosition, 1000); // 새로고침 후 체크
    }, 60000);
    ...
    document.addEventListener("visibilitychange", function() {
        if (!document.hidden) {
            // 페이지가 보일 때 즉시 체크
            setTimeout(checkWaitingListPosition, 500);
        } else {
            // 페이지가 숨겨질 때 타이틀 복원
            document.title = originalTitle;
        }
