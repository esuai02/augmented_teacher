...
$stdname = $thisuser ? $thisuser->lastname : 'Unknown User';
...
if (isset($_GET['action']) && $_GET['action'] === 'fetch_learning_activity') {
    ...
    try {
        // synergetic.php API 호출을 위한 내부 데이터 조회 (직접 DB 접근)
        if ($timeBegin == null) {
            $handwriting = $DB->get_records_sql("SELECT * FROM mdl_abessi_messages WHERE userid='$userid' AND active='1' AND timemodified > '$hoursago' ORDER BY timemodified DESC LIMIT 100");
        } else {
            $handwriting = $DB->get_records_sql("SELECT * FROM mdl_abessi_messages WHERE userid='$userid' AND active='1' AND timemodified > '$timeBegin' AND timemodified < '$timeEnd' ORDER BY timemodified DESC LIMIT 100");
        }
...
// OpenAI 기반 고급 학습 분석 핸들러
if (isset($_GET['action']) && $_GET['action'] === 'analyze_with_openai') {
    header('Content-Type: application/json');
    
    $userid = intval($_GET['userid'] ?? $studentid);
    $timeBegin = intval($_GET['tb'] ?? ($timecreated - 43200));
    $timeEnd = intval($_GET['te'] ?? $timecreated);
    
    try {
        // 캐시 테이블 생성 확인 (최초 실행 시) - 함수 존재 여부 확인
        if (function_exists('createAnalysisCacheTable')) {
            createAnalysisCacheTable();
        }
        
        // 먼저 기본 활동 데이터 조회
        if ($timeBegin == null) {
            $handwriting = $DB->get_records_sql("SELECT * FROM mdl_abessi_messages WHERE userid='$userid' AND active='1' AND timemodified > '$hoursago' ORDER BY timemodified DESC LIMIT 20");
        } else {
            $handwriting = $DB->get_records_sql("SELECT * FROM mdl_abessi_messages WHERE userid='$userid' AND active='1' AND timemodified > '$timeBegin' AND timemodified < '$timeEnd' ORDER BY timemodified DESC LIMIT 20");
        }
        
        // 학생 컨텍스트 정보 수집
        $studentInfo = $DB->get_record_sql("SELECT firstname, lastname FROM mdl_user WHERE id='$userid'");
        $studentContext = $DB->get_record_sql("SELECT text FROM mdl_abessi_tracking WHERE userid='$userid' AND type LIKE 'context' ORDER BY id DESC LIMIT 1");
        
        $contextInfo = $studentInfo->firstname . ' ' . $studentInfo->lastname;
        if ($studentContext) {
            $contextInfo .= ' - ' . $studentContext->text;
        }
        ...
        foreach ($handwriting as $activity) {
            $totalTime += $activity->usedtime;
            $totalStrokes += $activity->nstroke;
            if ($activity->feedback > 0) $feedbackCount++;
            
            $activitySummary[] = [
                'title' => $activity->contentstitle,
                'time' => round($activity->usedtime / 60, 1),
                'strokes' => $activity->nstroke,
                'feedback' => $activity->feedback,
                'status' => $activity->status
            ];
        }
        ...
        $analysisData = [
            'total_activities' => count($handwriting),
            'total_time_minutes' => round($totalTime / 60, 1),
            'avg_time_per_activity' => count($handwriting) > 0 ? round($totalTime / 60 / count($handwriting), 1) : 0,
        ...
        // 캐시 확인 먼저 수행
        $cacheResult = getCachedAnalysis($userid, $analysisData, 'openai_enhanced');
        
        if ($cacheResult['success'] && $cacheResult['cache_hit']) {
            // 캐시된 결과 사용
            $cachedData = $cacheResult['data'];
            $response = [
                'success' => true,
                'data' => [
                    'basic_stats' => $analysisData,
                    'ai_analysis' => $cachedData['analysis_result'] ?? '캐시된 분석 결과',
                    'analysis_type' => 'openai_enhanced_cached',
            ...
            // 캐시 미스 - OpenAI로 새로운 분석 실행
            $aiAnalysis = analyzeCompletionScore($analysisData, $contextInfo);
            
            if ($aiAnalysis) {
                // 분석 성공 - 캐시에 저장
                $cacheResult = setCachedAnalysis($userid, $analysisData, $aiAnalysis, null, 'openai_enhanced');
                ...
                $response = [
                    ...
                    'data' => [
                        ...
                        'analysis_type' => 'openai_enhanced',
                        'model_used' => OPENAI_MODEL,
                        'cache_saved' => $cacheResult['success'] ?? false,
                        'cache_hit' => false
                    ],
                    'timestamp' => $timecreated
                ];
            } else {
                // OpenAI 실패 시 기본 분석으로 폴백
                $basicScore = min(100, ($totalTime / 3600) * 30 + ($feedbackCount * 20) + min(40, $totalStrokes / 10));
                
                $response = [
                    'success' => true,
                    'data' => [
                        'basic_stats' => $analysisData,
                        'ai_analysis' => "OpenAI 분석을 사용할 수 없어 기본 분석을 제공합니다.\n\n완성도 점수: " . round($basicScore) . "점\n\n기본 통계를 바탕으로 한 간단한 분석입니다.",
                        'analysis_type' => 'basic_fallback',
                        'fallback_score' => round($basicScore),
                        'cache_hit' => false
                    ],
                    'timestamp' => $timecreated
                ];
...
// 고급 학습완성도 분석 엔진 핸들러
if (isset($_GET['action']) && $_GET['action'] === 'deep_learning_analysis') {
    header('Content-Type: application/json');
    
    $userid = intval($_GET['userid'] ?? $studentid);
    $timeBegin = intval($_GET['tb'] ?? ($timecreated - 86400)); // 기본값: 24시간 전
    $timeEnd = intval($_GET['te'] ?? $timecreated);
    
    try {
        // 학습 활동 데이터 조회 (더 많은 데이터로 정확한 분석)
        if ($timeBegin == null) {
            $handwriting = $DB->get_records_sql("SELECT * FROM mdl_abessi_messages WHERE userid='$userid' AND active='1' AND timemodified > '$hoursago' ORDER BY timemodified DESC LIMIT 50");
        } else {
            $handwriting = $DB->get_records_sql("SELECT * FROM mdl_abessi_messages WHERE userid='$userid' AND active='1' AND timemodified > '$timeBegin' AND timemodified < '$timeEnd' ORDER BY timemodified DESC LIMIT 50");
        }
        
        // 학생 정보 및 컨텍스트 수집
        $studentInfo = $DB->get_record_sql("SELECT firstname, lastname FROM mdl_user WHERE id='$userid'");
        $studentContext = $DB->get_record_sql("SELECT text FROM mdl_abessi_tracking WHERE userid='$userid' AND type LIKE 'context' ORDER BY id DESC LIMIT 1");
        $recentGoals = $DB->get_records_sql("SELECT text, type FROM mdl_abessi_tracking WHERE userid='$userid' AND type IN ('today_goal', 'week_goal') ORDER BY id DESC LIMIT 5");
        ...
        // 최근 목표 추가
        if (!empty($recentGoals)) {
            $contextInfo .= ' | 최근 목표: ';
            $goals = [];
            foreach ($recentGoals as $goal) {
                $goals[] = $goal->type . ':' . $goal->text;
            }
            $contextInfo .= implode(', ', array_slice($goals, 0, 3));
        }
        
        // 학습 패턴 분석을 위한 시간대별 데이터
        $timePatterns = [];
        $contentTypes = [];
        ...
        foreach ($handwriting as $activity) {
            ...
            // 시간대 패턴 분석
            $hour = date('H', $activity->timemodified);
            $timePatterns[$hour] = ($timePatterns[$hour] ?? 0) + 1;
            
            // 콘텐츠 타입 분석
            $contentTypes[$activity->contentstype] = ($contentTypes[$activity->contentstype] ?? 0) + 1;
            ...
            $activityDetails[] = [
                ...
                'feedback' => $activity->feedback,
                'status' => $activity->status,
                'content_type' => $activity->contentstype,
                'timestamp' => $activity->timemodified
            ];
        }
        ...
        $analysisData = [
            ...
            'total_strokes' => $totalStrokes,
            'feedback_received' => $feedbackCount,
            'completion_rate' => count($handwriting) > 0 ? round(($completedCount / count($handwriting)) * 100, 1) : 0,
            'time_patterns' => $timePatterns,
            'content_types' => $contentTypes,
            'recent_activities' => array_slice($activityDetails, 0, 20) // 최근 20개
        ];
        ...
        // 고급 학습완성도 분석 엔진 실행
        $deepAnalysis = analyzeLearningCompletion($analysisData, $contextInfo, $timeRange);
        ...
        if ($deepAnalysis['success']) {
            $response = [
                ...
                'data' => [
                    ...
                    'deep_analysis' => $deepAnalysis['parsed_data'],
                    'raw_ai_response' => $deepAnalysis['raw_analysis'],
                    'token_usage' => $deepAnalysis['token_usage']
                ],
                'timestamp' => $timecreated
            ];
        } else {
            ...
            $response = [
                ...
                'data' => [
                    ...
                    'raw_data' => $analysisData,
                    'deep_analysis' => $deepAnalysis['fallback_analysis'],
                    'error_message' => $deepAnalysis['error']
                ],
                'timestamp' => $timecreated
            ];
        }
        ...
        echo json_encode([
            'success' => false,
            'error' => 'Deep learning analysis failed: ' . $e->getMessage()
        ]);
    }
...
// 고급 피드백 생성 시스템 핸들러
if (isset($_GET['action']) && $_GET['action'] === 'generate_personalized_feedback') {
    header('Content-Type: application/json');
    ...
    try {
        // 학생 프로필 정보 수집
        $studentInfo = $DB->get_record_sql("SELECT firstname, lastname FROM mdl_user WHERE id='$userid'");
        $userProfile = $DB->get_record_sql("SELECT data FROM mdl_user_info_data WHERE userid='$userid' AND fieldid='23'"); // 학년
        $learningStyle = $DB->get_record_sql("SELECT data FROM mdl_user_info_data WHERE userid='$userid' AND fieldid='24'"); // 학습 스타일
        
        // 최근 학습 목표 수집
        $recentGoals = $DB->get_records_sql("SELECT text FROM mdl_abessi_tracking WHERE userid='$userid' AND type IN ('today_goal', 'week_goal', 'context') ORDER BY id DESC LIMIT 5");
        
        // 학생 프로필 구성
        $studentProfile = [
            'name' => $studentInfo->firstname . ' ' . $studentInfo->lastname,
            'grade_level' => $userProfile->data ?? '미설정',
            'learning_style' => $learningStyle->data ?? '분석형',
            'motivation_type' => '성취지향' // 기본값
        ];
        ...
        foreach ($recentGoals as $goal) {
            if (!empty($goal->text)) {
                $learningGoals[] = $goal->text;
            }
        }
        
        // 피드백 캐시 확인 먼저 수행
        $activityDataForCache = $analysisData['basic_stats'] ?? $analysisData;
        $cacheResult = getCachedAnalysis($userid, $activityDataForCache, 'feedback_enhanced');
        
        if ($cacheResult['success'] && $cacheResult['cache_hit'] && !empty($cacheResult['data']['feedback_result'])) {
            // 캐시된 피드백 사용
            $cachedFeedback = $cacheResult['data']['feedback_result'];
            $response = [
                'success' => true,
                'parsed_feedback' => $cachedFeedback,
                'raw_feedback' => json_encode($cachedFeedback),
                'cache_hit' => true,
                'cached_at' => $cacheResult['data']['cached_at'],
                'expires_at' => $cacheResult['data']['expires_at'],
                'message' => '캐시된 피드백을 로드했습니다.'
            ];
        } else {
            // 캐시 미스 - 새로운 피드백 생성
            $feedbackResult = generateAdvancedPersonalizedFeedback($analysisData, $studentProfile, $learningGoals);
            
            if ($feedbackResult['success']) {
                // 피드백 생성 성공 - 캐시에 저장
                $cacheResult = setCachedAnalysis($userid, $activityDataForCache, null, $feedbackResult['parsed_feedback'], 'feedback_enhanced', $feedbackResult['token_usage'] ?? null);
                
                $response = [
                    'success' => true,
                    'parsed_feedback' => $feedbackResult['parsed_feedback'],
                    'raw_feedback' => $feedbackResult['raw_feedback'],
                    'token_usage' => $feedbackResult['token_usage'],
                    'cache_hit' => false,
                    'cache_saved' => $cacheResult['success'] ?? false
                ];
            } else {
                ...
                $response = [
                    ...
                    'fallback_feedback' => $feedbackResult['fallback_feedback'] ?? null,
                    'cache_hit' => false
                ];
            }
        }
        ...
        echo json_encode([
            'success' => false,
            'error' => 'Feedback generation failed: ' . $e->getMessage(),
            'fallback_feedback' => [
                'current_status' => '시스템 오류로 인해 피드백을 생성할 수 없습니다.',
                'motivation_message' => '잠시 후 다시 시도해주세요.',
                'next_focus' => '기본 학습 활동 지속'
            ]
        ]);
    }
...
// 캐시 통계 조회 핸들러
if (isset($_GET['action']) && $_GET['action'] === 'get_cache_statistics') {
    header('Content-Type: application/json');
    ...
    try {
        ...
        echo json_encode([
            'success' => false,
            'error' => 'Cache statistics failed: ' . $e->getMessage()
        ]);
    }
...
// 만료된 캐시 정리 핸들러
if (isset($_GET['action']) && $_GET['action'] === 'cleanup_expired_cache') {
    header('Content-Type: application/json');
    
    try {
        $result = cleanupExpiredCache();
...
// 사용자 캐시 무효화 핸들러
if (isset($_GET['action']) && $_GET['action'] === 'invalidate_user_cache') {
    header('Content-Type: application/json');
    ...
    try {
        ...
        echo json_encode([
            'success' => false,
            'error' => 'User cache invalidation failed: ' . $e->getMessage()
        ]);
    }
...
<div style="display: flex; justify-content: space-between; align-items: flex-start; margin: 3px auto; width: 90%;">
    ...
    echo '<div class="modern-header">
            ...
            <div class="nav-top">
                <div class="content-container"> 
                    <div class="nav-controls">
                        ...
                        <a href="https://mathking.kr/moodle/local/augmented_teacher/alt42/studenthome/index.php?userid='.$studentid.'" class="nav-btn">
                            🏠 홈
                        </a>
                        <a href="https://mathking.kr/moodle/local/augmented_teacher/students/index42.php?id='.$studentid.'" class="nav-btn">
                        👩🏻‍🎨‍ 내공부방
                        </a>


                        <a href="https://mathking.kr/moodle/local/augmented_teacher/students/today42.php?id='.$studentid.'" class="nav-btn">
                        📝 오늘
                        </a>
                        <a href="https://mathking.kr/moodle/local/augmented_teacher/students/schedule42.php?id='.$studentid.'" class="nav-btn">
                            📅 일정
                        </a>

                        <a href="https://mathking.kr/moodle/local/augmented_teacher/students/goals42.php?id='.$studentid.'" class="nav-btn">
                            🎯 목표
                        </a>

                        <a href="https://mathking.kr/moodle/local/augmented_teacher/alt42/teachingsupport/student_inbox.php?studentid='.$studentid.'" class="nav-btn">
                        📩 메세지
                        </a>
                        <a href="https://mathking.kr/moodle/local/augmented_teacher/teachers/timescaffolding42.php?userid='.$studentid.'" class="nav-btn active">
                            📅 수학일기
                        </a>
                        </div>   
                    </div>
            ...
            <div class="status-section">
                <span class="brand-text">⭐ KAISTT</span>
                <div class="status-item">
                    <input type="checkbox" name="checkAccount" '.htmlspecialchars(isset($status5) ? $status5 : '', ENT_QUOTES, 'UTF-8').' onClick="ChangeCheckBox(333,\''.$studentid.'\',\''.$goalid.'\', this.checked)" id="study-checkbox"/>
                    <label for="study-checkbox">책/프린트 공부</label>
                </div>
                <div class="status-item">
                    <input type="checkbox" name="checkAccount" '.htmlspecialchars(isset($status4) ? $status4 : '', ENT_QUOTES, 'UTF-8').' onClick="Resttime(33,\''.$studentid.'\',\''.$goalid.'\', this.checked)" id="rest-checkbox"/>
                    <label for="rest-checkbox">DMN 휴식</label>
                </div>
                <div class="status-item">
                      
                </div><table width-10%><tr><td></td></tr></table>
            </div>
        </div>
        
        <!-- jQuery UI 탭 시스템 시작 -->
        <div id="main-tabs" class="main-tabs-container">
         ...
         <table align=center width=70%><tr><td width=100%>
         <table width=100%>
            <tr>
                <td style="white-space: nowrap;">
                    <a href="https://mathking.kr/moodle/local/augmented_teacher/books/chapter42.php?cid='.$chapterlog->cid.'&cntid='.$chapterlog->cntid.'&nch='.$chapterlog->nch.'&studentid='.$studentid.'&type=init">
                ...
                <td width="60%">&nbsp; 
                    ...
                    &nbsp;&nbsp;&nbsp;&nbsp;
                    <a href="https://mathking.kr/moodle/local/augmented_teacher/students/weeklyplans.php?id='.$studentid.'&cid='.$chapterlog->cid.'&pid='.$termplan2->id.'">🟦</a>
                    <a href="https://mathking.kr/moodle/local/augmented_teacher/students/dailygoals.php?id='.$studentid.'&cid='.$chapterlog->cid.'&pid='.$wgoal->id.'">🟪</a>
                    <a href="https://mathking.kr/moodle/local/augmented_teacher/students/todayplans.php?id='.$studentid.'&cid='.$chapterlog->cid.'&pid='.$checkgoal->id.'&nch='.$chapterlog->nch.'">🟩</a>  
                    <a style="color:white;" href="https://mathking.kr/moodle/local/augmented_teacher/students/edittoday.php?id='.$studentid.'">🧑🏻</a> 
                ...
                <div class="analysis-container">
                    <div class="analysis-header">
                        ...
                        <div class="header-controls">
                            <button id="refresh-analysis" class="btn-refresh">🔄 새로고침</button>
                            <select id="analysis-timeframe" class="analysis-select">
                        ...
                        <div class="dashboard-cards">
                            <div class="dashboard-card completion-card">
                                ...
                                <div class="card-content">
                                    <span class="card-value" id="completion-rate">0%</span>
                                    <span class="card-label">학습 완성도</span>
                                </div>
                            </div>
                            <div class="dashboard-card activity-card">
                                <div class="card-icon">📝</div>
                                <div class="card-content">
                                    <span class="card-value" id="activity-count">0</span>
                                    <span class="card-label">총 활동 수</span>
                                </div>
                            </div>
                            <div class="dashboard-card time-card">
                                <div class="card-icon">⏱️</div>
                                <div class="card-content">
                                    <span class="card-value" id="study-time">0분</span>
                                    <span class="card-label">학습 시간</span>
                                </div>
                            </div>
                            <div class="dashboard-card score-card">
                                <div class="card-icon">🎯</div>
                                <div class="card-content">
                                    <span class="card-value" id="avg-score">0점</span>
                    ...
                    <div class="analysis-results" id="analysis-results" style="display:none;">
                        <div class="results-tabs">
                            ...
                            <button class="tab-btn" data-tab="progress">📈 진도</button>
                            <button class="tab-btn" data-tab="feedback">💡 피드백</button>
                            <button class="tab-btn" data-tab="recommendations">🎯 추천</button>
                        </div>
                        ...
                        <div class="tab-content" id="feedback-content">
                            <div class="ai-feedback-section">
                                ...
                                <div id="ai-feedback-content" class="feedback-content">
                                    <button id="generate-feedback-btn" class="btn-generate-feedback">🤖 AI 피드백 생성</button>
                                </div>
                            </div>
                        </div>
                    ...
                    <!-- AI 피드백 패널 -->
                    <div id="ai-feedback-panel" class="feedback-section" style="display:none; margin-top: 20px;">
                        <div class="feedback-placeholder" style="text-align: center; padding: 40px; background: #f8f9ff; border: 2px dashed #8B72E6; border-radius: 12px;">
                            <div style="font-size: 48px; margin-bottom: 15px;">🎯</div>
                            <h3 style="color: #8B72E6; margin-bottom: 10px;">개인맞춤형 피드백</h3>
                            <p style="color: #666; margin: 0;">분석 완료 후 AI 피드백이 표시됩니다.</p>
                        </div>
                    </div>
                    
                    <div class="analysis-placeholder" id="analysis-placeholder">
                        <div class="placeholder-icon">🤖</div>
                        <h3>AI 학습 분석을 시작해보세요!</h3>
                        <p>최근 학습 활동을 분석하여 개인맞춤형 피드백을 제공합니다.</p>
                        ...
                        <button id="start-analysis" class="btn-start-analysis">분석 시작하기</button>
                    </div>
                </div>
            </div>
            <!-- 학습분석 탭 종료 -->
...
function initializeTabs() {
    ...
    $('#main-tabs').tabs({
        activate: function(event, ui) {
            // 학습분석 탭이 활성화되면 고급 분석 엔진 실행 (Deep Learning Analysis)
            if (ui.newPanel.attr('id') === 'tab-analysis') {
                loadDeepAnalysisData();
            }
            // 메모장 탭이 활성화되면 메모 새로고침
            else if (ui.newPanel.attr('id') === 'tab-notes') {
                loadNotes();
            }
        },
        beforeActivate: function(event, ui) {
            // 탭 전환 전 처리
            const currentTab = ui.oldTab.attr('aria-controls');
            const newTab = ui.newTab.attr('aria-controls');
            
            console.log('탭 전환:', currentTab, '->', newTab);
            return true;
        }
    });
    
    // 탭 버튼에 아이콘 추가 (이미 HTML에 있지만 동적으로도 설정)
    $('#main-tabs .ui-tabs-nav a[href="#tab-activity"]').html('📈 활동일지');
    $('#main-tabs .ui-tabs-nav a[href="#tab-notes"]').html('📝 메모장');
    $('#main-tabs .ui-tabs-nav a[href="#tab-analysis"]').html('🤖 학습분석');
    
    // 기본적으로 첫 번째 탭(활동일지) 활성화
    $('#main-tabs').tabs('option', 'active', 0);
}

// 학습분석 데이터 로딩 함수
function loadAnalysisData() {
    console.log('학습분석 데이터를 로딩합니다...');
    
    const analysisPanel = document.getElementById('tab-analysis');
    
    // 로딩 상태 표시
    analysisPanel.innerHTML = '<div class="analysis-loading" style="padding: 40px; text-align: center;">' +
        '<div class="loading-spinner" style="margin-bottom: 20px;">' +
        '<i class="fas fa-spinner fa-spin fa-2x" style="color: #8B72E6;"></i>' +
        '</div>' +
        '<p style="color: #666; font-size: 16px;">학습 활동 데이터를 분석중입니다...</p>' +
        '<p style="color: #999; font-size: 14px;">잠시만 기다려주세요.</p>' +
        '</div>';
    
    // 시간 범위 설정 (최근 24시간)
    const currentTime = Math.floor(Date.now() / 1000);
    const dayAgo = currentTime - (24 * 60 * 60);
    
    // Synergetic API 호출
    fetchLearningActivityData(currentUserId, dayAgo, currentTime)
        .then(response => {
            if (response.success) {
                displayAnalysisResults(response.data);
            } else {
                showAnalysisError(response.error || '데이터를 불러오는데 실패했습니다.');
            }
        })
        .catch(error => {
            console.error('Analysis data loading error:', error);
            showAnalysisError('서버 연결에 실패했습니다.');
        });
}

// Synergetic API 호출 함수
function fetchLearningActivityData(userid, timeBegin, timeEnd) {
    return new Promise((resolve, reject) => {
        $.ajax({
            url: window.location.pathname + '?action=fetch_learning_activity',
            type: 'GET',
            data: {
                userid: userid,
                tb: timeBegin,
...
function displayAnalysisResults(data) {
    ...
    let analysisHtml = `
        <div class="analysis-container" style="padding: 20px;">
            <div class="analysis-header" style="margin-bottom: 30px;">
                <h3 style="color: #333; margin-bottom: 10px;">🤖 AI 학습분석 리포트</h3>
                <div class="analysis-summary" style="display: flex; gap: 20px; flex-wrap: wrap;">
                    <div class="summary-card" style="background: linear-gradient(135deg, #f8f9ff 0%, #e8ebff 100%); padding: 15px; border-radius: 8px; flex: 1; min-width: 200px;">
                        <div style="color: #8B72E6; font-weight: bold;">총 학습 활동</div>
                        <div style="font-size: 24px; font-weight: bold; color: #333;">${summary.total_activities || 0}개</div>
                    </div>
                    <div class="summary-card" style="background: linear-gradient(135deg, #fff8f0 0%, #ffebdc 100%); padding: 15px; border-radius: 8px; flex: 1; min-width: 200px;">
                        <div style="color: #ff8c42; font-weight: bold;">평균 완성도</div>
                        <div style="font-size: 24px; font-weight: bold; color: #333;">${summary.avg_completion_score || 0}점</div>
                    </div>
                </div>
            </div>
    ...
    if (activities.length === 0) {
        analysisHtml += `
                <div style="text-align: center; padding: 40px; color: #999;">
                    <i class="fas fa-chart-bar fa-3x" style="margin-bottom: 15px; opacity: 0.5;"></i>
                    <p style="font-size: 16px;">분석할 학습 활동이 없습니다.</p>
                    <p style="font-size: 14px;">학습 활동을 진행한 후 다시 확인해주세요.</p>
                </div>
        `;
    } else {
        analysisHtml += `
                <h4 style="color: #333; margin-bottom: 15px;">📊 최근 활동 분석</h4>
                <div class="activities-list" style="max-height: 400px; overflow-y: auto;">
        `;
        
        activities.forEach((activity, index) => {
            const completionColor = activity.completion_score >= 80 ? '#4CAF50' : 
                                  activity.completion_score >= 60 ? '#FF9800' : '#F44336';
            const timeAgo = Math.floor((Date.now() / 1000 - activity.timemodified) / 60);
            const timeDisplay = timeAgo < 60 ? `${timeAgo}분 전` : `${Math.floor(timeAgo / 60)}시간 전`;
            
            analysisHtml += `
                    <div class="activity-item" style="border: 1px solid #eee; border-radius: 8px; padding: 15px; margin-bottom: 10px; background: white;">
                        <div style="display: flex; justify-content: between; align-items: start; gap: 15px;">
                            <div style="flex: 1;">
                                <div style="font-weight: bold; color: #333; margin-bottom: 5px;">
                                    ${activity.contentstitle || '활동 ' + (index + 1)}
                                </div>
                                <div style="color: #666; font-size: 14px; margin-bottom: 8px;">
                                    📝 ${activity.usedtime}분 소요 | ✏️ ${activity.nstroke}회 작성 | 💬 ${activity.feedback}회 피드백
                                </div>
                                <div style="color: #999; font-size: 12px;">${timeDisplay} | ${activity.status}</div>
                            </div>
                            <div style="text-align: center; min-width: 80px;">
                                <div style="font-size: 18px; font-weight: bold; color: ${completionColor};">
                                    ${activity.completion_score}점
                                </div>
                                <div style="font-size: 11px; color: #666;">완성도</div>
                            </div>
                        </div>
                    </div>
            `;
    ...
    analysisHtml += `
            ...
            <div class="analysis-footer" style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #eee; text-align: center;">
                <button onclick="refreshAnalysis()" class="btn" style="background: #8B72E6; color: white; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer;">
                    <i class="fas fa-sync-alt"></i> 분석 새로고침
                </button>
                <p style="color: #999; font-size: 12px; margin-top: 10px;">
                    더 정확한 AI 분석 결과는 OpenAI 연동 후 제공됩니다.
                </p>
            </div>
        </div>
    `;
...
function showAnalysisError(errorMessage) {
    const analysisPanel = document.getElementById('tab-analysis');
    analysisPanel.innerHTML = '<div style="padding: 40px; text-align: center;">' +
        '<div style="color: #f44336; margin-bottom: 20px;">' +
        '<i class="fas fa-exclamation-triangle fa-3x"></i>' +
        '</div>' +
        '<p style="color: #666; font-size: 16px; margin-bottom: 10px;">분석 데이터 로딩 실패</p>' +
        '<p style="color: #999; font-size: 14px; margin-bottom: 20px;">' + errorMessage + '</p>' +
        '<button onclick="loadAnalysisData()" class="btn" style="background: #8B72E6; color: white; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer;">' +
...
function testOpenAIConnection() {
    console.log('OpenAI 연결을 테스트합니다...');
    
    return new Promise((resolve, reject) => {
        $.ajax({
            url: window.location.pathname + '?action=test_openai_connection',
            type: 'GET',
            dataType: 'json',
            timeout: 10000, // 10초 타임아웃
            success: function(response) {
                if (response.success) {
                    console.log('OpenAI 연결 성공:', response);
                    resolve(response);
                } else {
            ...
            error: function(xhr, status, error) {
                console.error('OpenAI 테스트 오류:', status, error);
                reject(new Error('연결 테스트 실패: ' + error));
            }
        });
    });
}
...
// OpenAI 기반 고급 분석 함수
function analyzeWithOpenAI(userid, timeBegin, timeEnd) {
    console.log('OpenAI 기반 고급 분석을 시작합니다...');
    
    return new Promise((resolve, reject) => {
        $.ajax({
            url: window.location.pathname + '?action=analyze_with_openai',
            type: 'GET',
            data: {
                userid: userid,
                tb: timeBegin,
            ...
            dataType: 'json',
            timeout: 45000, // 45초 타임아웃 (OpenAI 분석 시간 고려)
            success: function(response) {
                resolve(response);
            },
            error: function(xhr, status, error) {
                console.error('OpenAI 분석 오류:', status, error);
                reject(error);
            }
        });
    });
...
// 향상된 학습분석 데이터 로딩 (OpenAI 포함)
function loadEnhancedAnalysisData() {
    console.log('향상된 학습분석 데이터를 로딩합니다...');
    ...
    analysisPanel.innerHTML = `
        <div class="analysis-loading" style="padding: 40px; text-align: center;">
            <div class="loading-spinner" style="margin-bottom: 20px;">
                <i class="fas fa-brain fa-spin fa-2x" style="color: #8B72E6;"></i>
            </div>
            <p style="color: #666; font-size: 16px;">🤖 AI가 학습 데이터를 분석중입니다...</p>
            <p style="color: #999; font-size: 14px;">더 정확한 분석을 위해 잠시만 기다려주세요.</p>
            <div class="progress-bar" style="background: #f0f0f0; border-radius: 10px; overflow: hidden; margin-top: 15px; height: 8px;">
                <div class="progress-fill" style="background: linear-gradient(90deg, #8B72E6, #6B5CE7); height: 100%; width: 0%; transition: width 2s ease-in-out;"></div>
            </div>
        </div>
    `;
    ...
    // 진행률 애니메이션
    setTimeout(() => {
        const progressFill = analysisPanel.querySelector('.progress-fill');
    ...
    // OpenAI 기반 분석 실행
    analyzeWithOpenAI(currentUserId, dayAgo, currentTime)
        .then(response => {
            setTimeout(() => {
                const progressFill = analysisPanel.querySelector('.progress-fill');
            ...
            setTimeout(() => {
                if (response.success) {
                    displayEnhancedAnalysisResults(response.data);
                } else {
                    showAnalysisError(response.error || 'AI 분석에 실패했습니다.');
                }
            }, 2000);
        })
        .catch(error => {
...
function displayEnhancedAnalysisResults(data) {
    ...
    const analysisType = data.analysis_type || 'basic';
    
    // 분석 데이터를 전역 변수에 저장 (피드백 생성에 사용)
    window.lastAnalysisData = data;
    
    let analysisHtml = `
        <div class="enhanced-analysis-container" style="padding: 20px;">
            <div class="analysis-header" style="margin-bottom: 30px;">
                <h3 style="color: #333; margin-bottom: 10px; display: flex; align-items: center; gap: 10px;">
                    🤖 AI 고급 학습분석 리포트
                    <span style="background: ${analysisType === 'openai_enhanced' ? '#4CAF50' : '#FF9800'}; color: white; padding: 2px 8px; border-radius: 12px; font-size: 11px;">
                        ${analysisType === 'openai_enhanced' ? 'GPT-4 분석' : '기본 분석'}
                    </span>
                </h3>
                ...
                <div class="stats-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 15px; margin-bottom: 25px;">
                    <div class="stat-card" style="background: linear-gradient(135deg, #f8f9ff 0%, #e8ebff 100%); padding: 15px; border-radius: 8px; text-align: center;">
                        <div style="font-size: 24px; font-weight: bold; color: #8B72E6;">${basicStats.total_activities || 0}</div>
                        <div style="color: #666; font-size: 12px;">총 활동</div>
                    </div>
                    <div class="stat-card" style="background: linear-gradient(135deg, #fff8f0 0%, #ffebdc 100%); padding: 15px; border-radius: 8px; text-align: center;">
                        <div style="font-size: 24px; font-weight: bold; color: #ff8c42;">${basicStats.total_time_minutes || 0}</div>
                        <div style="color: #666; font-size: 12px;">총 학습시간(분)</div>
                    </div>
                    <div class="stat-card" style="background: linear-gradient(135deg, #f0fff4 0%, #dcffe4 100%); padding: 15px; border-radius: 8px; text-align: center;">
                        <div style="font-size: 24px; font-weight: bold; color: #4CAF50;">${basicStats.avg_time_per_activity || 0}</div>
                        <div style="color: #666; font-size: 12px;">평균 활동시간(분)</div>
                    </div>
                    <div class="stat-card" style="background: linear-gradient(135deg, #fff0f5 0%, #fce4ec 100%); padding: 15px; border-radius: 8px; text-align: center;">
                        <div style="font-size: 24px; font-weight: bold; color: #E91E63;">${basicStats.feedback_received || 0}</div>
                        <div style="color: #666; font-size: 12px;">받은 피드백</div>
                    </div>
                </div>
            </div>
            
            <div class="ai-analysis-section" style="background: white; border: 1px solid #e0e0e0; border-radius: 12px; padding: 20px; margin-bottom: 20px;">
                <h4 style="color: #333; margin-bottom: 15px; display: flex; align-items: center; gap: 8px;">
                    <i class="fas fa-robot" style="color: #8B72E6;"></i>
                    AI 분석 결과
                </h4>
                <div style="line-height: 1.6; color: #444; white-space: pre-wrap;">${aiAnalysis}</div>
            </div>
            
            <div class="analysis-actions" style="text-align: center; margin-top: 25px;">
                <button onclick="loadEnhancedAnalysisData()" class="btn" style="background: #8B72E6; color: white; border: none; padding: 12px 24px; border-radius: 6px; cursor: pointer; margin-right: 10px;">
                    <i class="fas fa-sync-alt"></i> AI 재분석
                </button>
                <button onclick="testOpenAIConnection()" class="btn" style="background: #4CAF50; color: white; border: none; padding: 12px 24px; border-radius: 6px; cursor: pointer;">
                    <i class="fas fa-wifi"></i> 연결 테스트
                </button>
            </div>
            
            ${updateAnalysisWithCacheInfo(data)}
            
            ${analysisType === 'openai_enhanced' || analysisType === 'openai_enhanced_cached' ? 
                `<p style="color: #999; font-size: 11px; text-align: center; margin-top: 15px;">
                    Powered by ${data.model_used || 'GPT-4'} | ${analysisType === 'openai_enhanced_cached' ? '캐시된 분석 결과' : '더 정확한 AI 기반 분석 결과'}
                 </p>` : 
                `<p style="color: #999; font-size: 11px; text-align: center; margin-top: 15px;">
                    OpenAI 연결 실패로 기본 분석을 표시합니다.
                 </p>`
            }
        </div>
    `;
    ...
    // 새로운 UI 구조가 있는지 확인
    const newAnalysisResults = document.getElementById('analysis-results');
    if (newAnalysisResults) {
        // 새로운 UI 구조 사용
        displayNewAnalysisUI(data, basicStats, aiAnalysis, analysisType);
    } else {
        // 기존 UI 구조 사용 (후방 호환성)
        analysisPanel.innerHTML = analysisHtml;
    }
    ...
    if (feedbackPanel) {
        ...
        // 분석이 성공한 경우에만 자동으로 피드백 생성
        if (analysisType === 'openai_enhanced' && data.parsed_analysis) {
            setTimeout(() => {
                generatePersonalizedFeedback(data);
            }, 1000); // 1초 후 피드백 생성 시작
        }
...
// 고급 학습완성도 분석 실행 함수
function runDeepLearningAnalysis(userid, timeBegin, timeEnd) {
    console.log('고급 학습완성도 분석을 시작합니다...');
    
    return new Promise((resolve, reject) => {
        $.ajax({
            url: window.location.pathname + '?action=deep_learning_analysis',
            type: 'GET',
            data: {
                userid: userid,
                tb: timeBegin,
            ...
            dataType: 'json',
            timeout: 60000, // 60초 타임아웃 (고급 분석 시간 고려)
            success: function(response) {
                resolve(response);
            },
            error: function(xhr, status, error) {
                console.error('고급 분석 오류:', status, error);
                reject(error);
            }
        });
    });
...
// 개인맞춤형 피드백 생성 함수
function generatePersonalizedFeedback(analysisData) {
    console.log('개인맞춤형 피드백 생성을 시작합니다...');
    
    if (!analysisData) {
        console.error('분석 데이터가 제공되지 않았습니다.');
        return;
    }
    ...
    return new Promise((resolve, reject) => {
        $.ajax({
            url: window.location.pathname + '?action=generate_personalized_feedback',
            type: 'POST',
            data: {
            ...
            success: function(response) {
                ...
                displayFeedbackResults(response);
                resolve(response);
            },
            error: function(xhr, status, error) {
                console.error('피드백 생성 오류:', status, error);
                displayFeedbackError(error);
                reject(error);
            }
        });
    });
...
// 피드백 결과 표시 함수
function displayFeedbackResults(feedbackData) {
    const feedbackPanel = document.getElementById('ai-feedback-panel');
    if (!feedbackPanel) {
        console.error('피드백 패널을 찾을 수 없습니다.');
        return;
    }
    
    let feedbackContent = '';
    
    if (feedbackData.success && feedbackData.parsed_feedback) {
        const feedback = feedbackData.parsed_feedback;
        
        feedbackContent = `
            <div class="feedback-container" style="background: white; border: 1px solid #e0e0e0; border-radius: 12px; padding: 20px; margin-bottom: 20px;">
                <h3 style="color: #333; margin-bottom: 15px; display: flex; align-items: center; gap: 10px;">
                    🎯 개인맞춤형 학습 피드백
                    <span style="background: #4CAF50; color: white; padding: 2px 8px; border-radius: 12px; font-size: 11px;">
                        AI 생성
                    </span>
                </h3>
                
                <div class="current-status" style="background: linear-gradient(135deg, #f8f9ff 0%, #e8ebff 100%); padding: 15px; border-radius: 8px; margin-bottom: 20px;">
                    <h4 style="color: #8B72E6; margin-bottom: 10px; display: flex; align-items: center; gap: 8px;">
                        <i class="fas fa-chart-line"></i>
                        현재 학습 상태
                    </h4>
                    <p style="color: #444; line-height: 1.6; margin: 0;">${feedback.current_status}</p>
                </div>
                
                <div class="achievements-section" style="margin-bottom: 20px;">
                    <h4 style="color: #4CAF50; margin-bottom: 10px; display: flex; align-items: center; gap: 8px;">
                        <i class="fas fa-trophy"></i>
                        주요 성취사항
                    </h4>
                    <ul style="color: #444; line-height: 1.6;">
                        ${feedback.achievements.map(achievement => `<li>${achievement}</li>`).join('')}
                    </ul>
                </div>
                
                <div class="improvement-plans" style="margin-bottom: 20px;">
                    <h4 style="color: #FF9800; margin-bottom: 10px; display: flex; align-items: center; gap: 8px;">
                        <i class="fas fa-lightbulb"></i>
                        개선 계획
                    </h4>
                    ${feedback.improvement_plans.map(plan => `
                        <div style="background: #fff8f0; border-left: 4px solid #FF9800; padding: 15px; margin-bottom: 10px; border-radius: 4px;">
                            <strong style="color: #FF9800;">${plan.area}</strong>
                            <p style="margin: 5px 0; color: #666; font-size: 14px;">${plan.current_issue}</p>
                            <ul style="margin: 10px 0 5px 20px; color: #444;">
                                ${plan.action_steps.map(step => `<li>${step}</li>`).join('')}
                            </ul>
                            <p style="margin: 5px 0; color: #4CAF50; font-size: 14px; font-weight: 500;">
                                <i class="fas fa-target"></i> ${plan.expected_outcome}
                            </p>
                        </div>
                    `).join('')}
                </div>
                
                <div class="weekly-goals" style="margin-bottom: 20px;">
                    <h4 style="color: #E91E63; margin-bottom: 10px; display: flex; align-items: center; gap: 8px;">
                        <i class="fas fa-calendar-week"></i>
                        이번 주 목표
                    </h4>
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 10px;">
                        ${feedback.weekly_goals.map((goal, index) => `
                            <div style="background: #fce4ec; padding: 12px; border-radius: 6px; border-left: 3px solid #E91E63;">
                                <span style="font-weight: 500; color: #E91E63;">${index + 1}.</span>
                                <span style="color: #444; margin-left: 5px;">${goal}</span>
                            </div>
                        `).join('')}
                    </div>
                </div>
                
                <div class="learning-tips" style="margin-bottom: 20px;">
                    <h4 style="color: #8B72E6; margin-bottom: 10px; display: flex; align-items: center; gap: 8px;">
                        <i class="fas fa-brain"></i>
                        학습 팁
                    </h4>
                    <div style="background: #f8f9ff; padding: 15px; border-radius: 8px;">
                        <ul style="color: #444; line-height: 1.8; margin: 0;">
                            ${feedback.learning_tips.map(tip => `<li>${tip}</li>`).join('')}
                        </ul>
                    </div>
                </div>
                
                <div class="motivation-message" style="background: linear-gradient(135deg, #4CAF50 0%, #45a049 100%); color: white; padding: 20px; border-radius: 12px; text-align: center; margin-bottom: 20px;">
                    <h4 style="margin-bottom: 10px; display: flex; align-items: center; justify-content: center; gap: 8px;">
                        <i class="fas fa-heart"></i>
                        응원 메시지
                    </h4>
                    <p style="margin: 0; line-height: 1.6; font-size: 16px;">${feedback.motivation_message}</p>
                </div>
                
                <div class="next-focus" style="background: #e8f5e8; border: 1px solid #4CAF50; border-radius: 8px; padding: 15px;">
                    <h4 style="color: #4CAF50; margin-bottom: 10px; display: flex; align-items: center; gap: 8px;">
                        <i class="fas fa-arrow-right"></i>
                        다음 집중 영역
                    </h4>
                    <p style="color: #333; margin: 0; font-weight: 500;">${feedback.next_focus}</p>
                    <span style="background: ${getDifficultyColor(feedback.difficulty_level)}; color: white; padding: 2px 8px; border-radius: 12px; font-size: 11px; margin-top: 10px; display: inline-block;">
                        난이도: ${feedback.difficulty_level}
                    </span>
                </div>
                
                <div class="feedback-actions" style="text-align: center; margin-top: 25px;">
                    <button onclick="generatePersonalizedFeedback(lastAnalysisData)" class="btn" style="background: #8B72E6; color: white; border: none; padding: 12px 24px; border-radius: 6px; cursor: pointer; margin-right: 10px;">
                        <i class="fas fa-sync-alt"></i> 피드백 새로고침
                    </button>
                    <button onclick="exportFeedbackReport()" class="btn" style="background: #4CAF50; color: white; border: none; padding: 12px 24px; border-radius: 6px; cursor: pointer;">
                        <i class="fas fa-download"></i> 피드백 저장
                    </button>
                </div>
                
                <p style="color: #999; font-size: 11px; text-align: center; margin-top: 15px;">
                    Powered by GPT-4 | 개인맞춤형 AI 피드백 시스템
                </p>
            </div>
        `;
        
    } else if (feedbackData.fallback_feedback) {
        // 폴백 피드백 표시
        const fallback = feedbackData.fallback_feedback;
        feedbackContent = `
            <div class="feedback-container" style="background: white; border: 1px solid #e0e0e0; border-radius: 12px; padding: 20px; margin-bottom: 20px;">
                ...
                <div style="background: #fff8f0; padding: 15px; border-radius: 8px; margin-bottom: 15px;">
                    <p style="color: #444; line-height: 1.6; margin: 0;">${fallback.current_status}</p>
                </div>
                
                <div style="background: #f0fff4; padding: 15px; border-radius: 8px; margin-bottom: 15px;">
                    <h4 style="color: #4CAF50; margin-bottom: 10px;">다음 집중 영역</h4>
                    <p style="color: #444; margin: 0;">${fallback.next_focus}</p>
                </div>
                
                <div style="background: linear-gradient(135deg, #4CAF50 0%, #45a049 100%); color: white; padding: 15px; border-radius: 8px; text-align: center;">
                    <p style="margin: 0; line-height: 1.6;">${fallback.motivation_message}</p>
                </div>
                
                <p style="color: #999; font-size: 11px; text-align: center; margin-top: 15px;">
                    AI 연결 실패로 기본 피드백을 표시합니다. 잠시 후 다시 시도해주세요.
                </p>
            </div>
        `;
    } else {
        feedbackContent = `
            <div class="error-message" style="background: #ffebee; border: 1px solid #f44336; border-radius: 8px; padding: 15px; text-align: center; color: #d32f2f;">
    ...
    feedbackPanel.innerHTML = feedbackContent;
}
...
function showFeedbackLoadingState() {
    ...
    if (feedbackPanel) {
        feedbackPanel.innerHTML = `
            <div class="loading-feedback" style="text-align: center; padding: 40px; background: white; border: 1px solid #e0e0e0; border-radius: 12px;">
                <div class="spinner" style="border: 4px solid #f3f3f3; border-top: 4px solid #8B72E6; border-radius: 50%; width: 40px; height: 40px; animation: spin 1s linear infinite; margin: 0 auto 20px;"></div>
                <h4 style="color: #8B72E6; margin-bottom: 10px;">AI 피드백 생성 중...</h4>
                <p style="color: #666; margin: 0;">학습 데이터를 분석하여 개인맞춤형 피드백을 생성하고 있습니다.</p>
            </div>
        `;
    }
}
...
function displayFeedbackError(error) {
    ...
    if (feedbackPanel) {
        feedbackPanel.innerHTML = `
            <div class="error-message" style="background: #ffebee; border: 1px solid #f44336; border-radius: 8px; padding: 20px; text-align: center; color: #d32f2f;">
                <i class="fas fa-exclamation-triangle" style="font-size: 24px; margin-bottom: 10px;"></i>
                <h4 style="margin-bottom: 10px;">피드백 생성 실패</h4>
                <p style="margin: 0; color: #666;">오류: ${error}</p>
                <button onclick="generatePersonalizedFeedback(lastAnalysisData)" class="btn" style="background: #f44336; color: white; border: none; padding: 10px 20px; border-radius: 6px; cursor: pointer; margin-top: 15px;">
...
function exportFeedbackReport() {
    alert('피드백 리포트 내보내기 기능은 추후 구현 예정입니다.');
}

// ===== 캐시 관리 시스템 JavaScript 함수들 =====

// 캐시 통계 조회
function getCacheStatistics() {
    return new Promise((resolve, reject) => {
        $.ajax({
            url: window.location.pathname + '?action=get_cache_statistics',
            type: 'GET',
            dataType: 'json',
            timeout: 10000,
            success: function(response) {
                if (response.success) {
                    ...
                    reject(new Error(response.error || '캐시 통계 조회 실패'));
                }
            },
            error: function(xhr, status, error) {
                console.error('캐시 통계 조회 오류:', status, error);
                reject(new Error('캐시 통계 조회 통신 오류: ' + error));
            }
        });
    });
}
...
function cleanupExpiredCache() {
    return new Promise((resolve, reject) => {
        $.ajax({
            url: window.location.pathname + '?action=cleanup_expired_cache',
            type: 'GET',
            dataType: 'json',
            timeout: 30000, // 30초 타임아웃
            success: function(response) {
                if (response.success) {
                    console.log('캐시 정리 완료:', response.message);
                    resolve(response);
                } else {
                    reject(new Error(response.error || '캐시 정리 실패'));
                }
            },
            error: function(xhr, status, error) {
                console.error('캐시 정리 오류:', status, error);
                reject(new Error('캐시 정리 통신 오류: ' + error));
            }
        });
    });
}
...
function invalidateUserCache(userid = null) {
    ...
    return new Promise((resolve, reject) => {
        $.ajax({
            url: window.location.pathname + '?action=invalidate_user_cache',
            type: 'GET',
            ...
            success: function(response) {
                if (response.success) {
                    console.log('사용자 캐시 무효화 완료:', response.message);
                    resolve(response);
                } else {
                    reject(new Error(response.error || '사용자 캐시 무효화 실패'));
                }
            },
            error: function(xhr, status, error) {
                console.error('사용자 캐시 무효화 오류:', status, error);
                reject(new Error('사용자 캐시 무효화 통신 오류: ' + error));
            }
        });
    });
}
...
// 캐시 통계 UI 표시
function displayCacheStatistics() {
    getCacheStatistics().then(stats => {
        const cacheStatsHtml = `
            <div class="cache-statistics" style="background: white; border: 1px solid #e0e0e0; border-radius: 12px; padding: 20px; margin: 20px 0;">
                <h4 style="color: #333; margin-bottom: 15px; display: flex; align-items: center; gap: 8px;">
                    <i class="fas fa-database" style="color: #8B72E6;"></i>
                    캐시 시스템 통계
                </h4>
                ...
                <div class="stats-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 15px; margin-bottom: 20px;">
                    <div class="stat-card" style="background: linear-gradient(135deg, #f8f9ff 0%, #e8ebff 100%); padding: 15px; border-radius: 8px; text-align: center;">
                        <div style="font-size: 24px; font-weight: bold; color: #8B72E6;">${stats.total_cache_count || 0}</div>
                        <div style="color: #666; font-size: 12px;">전체 캐시</div>
                    </div>
                    <div class="stat-card" style="background: linear-gradient(135deg, #f0fff4 0%, #dcffe4 100%); padding: 15px; border-radius: 8px; text-align: center;">
                        <div style="font-size: 24px; font-weight: bold; color: #4CAF50;">${stats.valid_cache_count || 0}</div>
                        <div style="color: #666; font-size: 12px;">유효한 캐시</div>
                    </div>
                    <div class="stat-card" style="background: linear-gradient(135deg, #fff8f0 0%, #ffebdc 100%); padding: 15px; border-radius: 8px; text-align: center;">
                        <div style="font-size: 24px; font-weight: bold; color: #FF9800;">${stats.expired_cache_count || 0}</div>
                        <div style="color: #666; font-size: 12px;">만료된 캐시</div>
                    </div>
                    <div class="stat-card" style="background: linear-gradient(135deg, #fff0f5 0%, #fce4ec 100%); padding: 15px; border-radius: 8px; text-align: center;">
                        <div style="font-size: 24px; font-weight: bold; color: #E91E63;">${stats.cache_efficiency || 0}%</div>
                        <div style="color: #666; font-size: 12px;">캐시 효율성</div>
                    </div>
                </div>
                
                <div class="cache-actions" style="text-align: center; margin-top: 20px;">
                    <button onclick="cleanupExpiredCache().then(() => displayCacheStatistics())" class="btn" style="background: #FF9800; color: white; border: none; padding: 10px 20px; border-radius: 6px; cursor: pointer; margin-right: 10px;">
                        <i class="fas fa-broom"></i> 만료 캐시 정리
                    </button>
                    <button onclick="invalidateUserCache().then(() => displayCacheStatistics())" class="btn" style="background: #f44336; color: white; border: none; padding: 10px 20px; border-radius: 6px; cursor: pointer;">
                        <i class="fas fa-user-times"></i> 내 캐시 무효화
                    </button>
                </div>
                
                <p style="color: #999; font-size: 11px; text-align: center; margin-top: 15px;">
                    캐시 시스템을 통해 OpenAI API 호출 비용을 절감하고 응답 속도를 향상시킵니다.
                </p>
            </div>
        `;
        
        // 기존 캐시 통계 섹션이 있으면 교체, 없으면 분석 탭에 추가
        const existingStats = document.querySelector('.cache-statistics');
        if (existingStats) {
            existingStats.outerHTML = cacheStatsHtml;
        } else {
            const analysisTab = document.getElementById('tab-analysis');
            if (analysisTab) {
                analysisTab.insertAdjacentHTML('beforeend', cacheStatsHtml);
            }
        }
    }).catch(error => {
        console.error('캐시 통계 표시 오류:', error);
        ShowMessage('캐시 통계를 불러올 수 없습니다: ' + error.message);
    });
}

// 분석 결과에 캐시 상태 표시
function updateAnalysisWithCacheInfo(data) {
    if (data.cache_hit) {
        const cacheInfo = `
            <div class="cache-info" style="background: #e8f5e8; border: 1px solid #4CAF50; border-radius: 6px; padding: 10px; margin: 10px 0; font-size: 12px; color: #2e7d32;">
                <i class="fas fa-bolt" style="color: #4CAF50;"></i>
                캐시된 결과를 사용했습니다 (${data.cached_at ? new Date(data.cached_at).toLocaleString() : '시간 정보 없음'})
            </div>
        `;
        return cacheInfo;
    } else if (data.cache_saved) {
        const cacheInfo = `
            <div class="cache-info" style="background: #e3f2fd; border: 1px solid #2196F3; border-radius: 6px; padding: 10px; margin: 10px 0; font-size: 12px; color: #1565c0;">
                <i class="fas fa-save" style="color: #2196F3;"></i>
                새로운 분석 결과가 캐시에 저장되었습니다
            </div>
        `;
        return cacheInfo;
    }
...
function exportAnalysisReport() {
    alert('분석 리포트 내보내기 기능은 추후 구현 예정입니다.');
}

// 탭 시스템에서 Deep Analysis 사용하도록 업데이트
// 기존의 loadAnalysisData를 loadDeepAnalysisData로 교체

// Deep Analysis 함수 (탭에서 호출)
function loadDeepAnalysisData() {
    console.log('Deep Analysis 데이터를 로딩합니다...');
    
    // 먼저 기본 분석 실행
    loadAnalysisData();
    
    // OpenAI 연결 확인 후 향상된 분석 실행
    testOpenAIConnection()
        .then(() => {
            console.log('OpenAI 연결 확인됨. 향상된 분석을 시작합니다.');
            return loadEnhancedAnalysisData();
        })
        .catch(error => {
            console.log('OpenAI 연결 실패. 기본 분석을 유지합니다.', error);
        });
}

// 분석 UI 이벤트 핸들러 초기화
function initializeAnalysisEventHandlers() {
    // 분석 시작 버튼
    $(document).on('click', '#start-analysis', function() {
        console.log('분석 시작 버튼이 클릭되었습니다.');
        $('#analysis-placeholder').hide();
        loadDeepAnalysisData();
    });
    
    // 분석 새로고침 버튼
    $(document).on('click', '#refresh-analysis', function() {
        console.log('분석 새로고침 버튼이 클릭되었습니다.');
        loadDeepAnalysisData();
    });
    
    // AI 피드백 생성 버튼 (동적 생성되는 버튼)
    $(document).on('click', '.btn-generate-feedback', function() {
        const userid = $(this).data('userid') || currentUserId;
        console.log('AI 피드백 생성 버튼이 클릭되었습니다. 사용자 ID:', userid);
    ...
    $(document).on('click', '.tab-btn', function() {
        ...
        console.log('분석 결과 탭 전환:', targetTab);
    ...
    // 시간 범위 선택 변경
    $(document).on('change', '#analysis-timeframe', function() {
        const timeframe = $(this).val();
        console.log('분석 시간 범위 변경:', timeframe + '일');
...
$(window).on('load', function() {
    ...
    // 분석 이벤트 핸들러 초기화
    initializeAnalysisEventHandlers();
    
    console.log('학습분석 시스템이 초기화되었습니다.');
});
...
function drawProgressChart() {
    ...
    // 샘플 데이터 (실제로는 분석된 데이터 사용)
    const data = [
        { date: '3일전', completion: 65 },
        { date: '2일전', completion: 72 },
        { date: '1일전', completion: 68 },
        { date: '오늘', completion: 85 }
    ];
    ...
    data.forEach((item, index) => {
        ...
        const barHeight = (item.completion / maxValue) * chartHeight;
        const y = canvas.height - padding - barHeight;
        
        // 바 그리기
        const gradient = ctx.createLinearGradient(0, y, 0, y + barHeight);
        gradient.addColorStop(0, '#8B72E6');
        gradient.addColorStop(1, '#6B5CE7');
        ...
        ctx.fillText(item.completion + '%', x + barWidth / 2, y - 5);
...
function updateAnalysisDashboard(data) {
    ...
    if (completionRate) {
        completionRate.textContent = (summary.avg_completion_score || 0) + '%';
    }
    
    // 활동 수 업데이트
    const activityCount = document.getElementById('activity-count');
    if (activityCount) {
        activityCount.textContent = summary.total_activities || 0;
    }
    
    // 학습 시간 업데이트
    const studyTime = document.getElementById('study-time');
    if (studyTime) {
        const totalTime = summary.total_study_time || 0;
        studyTime.textContent = totalTime + '분';
    }
    
    // 평균 점수 업데이트
    const avgScore = document.getElementById('avg-score');
    if (avgScore) {
        avgScore.textContent = (summary.avg_score || 0) + '점';
    }
}

// 새로운 UI 구조에 분석 데이터 표시
function displayNewAnalysisUI(data, basicStats, aiAnalysis, analysisType) {
    // 대시보드 업데이트 (기본 통계를 새 형식으로 변환)
    const dashboardData = {
        summary: {
            avg_completion_score: Math.round((basicStats.total_activities > 0 ? 
                (basicStats.total_time_minutes / basicStats.total_activities) * 10 : 0)),
            total_activities: basicStats.total_activities || 0,
            total_study_time: basicStats.total_time_minutes || 0,
            avg_score: basicStats.avg_time_per_activity || 0
        }
    };
    
    updateAnalysisDashboard(dashboardData);
    ...
    if (analysisResults) {
        ...
        if (scoreNumber) {
            scoreNumber.textContent = dashboardData.summary.avg_completion_score;
        }
        
        // AI 분석 결과를 피드백 탭에 표시
        const feedbackContent = document.getElementById('ai-feedback-content');
        if (feedbackContent && aiAnalysis) {
            feedbackContent.innerHTML = `
                <div class="ai-analysis-result" style="background: #f8f9ff; padding: 20px; border-radius: 8px; margin-bottom: 15px;">
                    <div style="font-weight: bold; color: #8B72E6; margin-bottom: 10px;">
                        ${analysisType === 'openai_enhanced' ? '🤖 GPT-4 AI 분석' : '📊 기본 분석'}
                    </div>
                    <div style="line-height: 1.6; white-space: pre-wrap;">${aiAnalysis}</div>
                </div>
                <button id="generate-feedback-btn" class="btn-generate-feedback" data-userid="${data.userid || window.currentUserId}">
                    🤖 추가 피드백 생성
                </button>
            `;
        }
        
        // 추천 활동을 추천 탭에 표시 (AI 분석에서 추출)
        const recommendationsContent = analysisResults.querySelector('#recommendations-content .content');
        if (recommendationsContent) {
            let recommendations = [];
            if (data.parsed_analysis && data.parsed_analysis.recommendations) {
                recommendations = data.parsed_analysis.recommendations;
            } else {
                // 기본 추천사항
                recommendations = [
                    '정기적인 학습 패턴 유지하기',
                    '피드백을 적극 활용하여 학습 개선하기',
                    '어려운 문제는 충분한 시간을 두고 해결하기'
                ];
            }
...
// 새로운 UI에 맞게 분석 결과 표시 함수
function displayNewAnalysisResults(data) {
    const analysisPanel = document.getElementById('tab-analysis');
    const activities = data.activities || [];
    const summary = data.summary || {};
    ...
    if (analysisResults) {
        ...
        const scoreNumber = analysisResults.querySelector('.score-number');
        if (scoreNumber) {
            scoreNumber.textContent = summary.avg_completion_score || 0;
        }
        
        // 강점 섹션 업데이트
        const strengthsContent = analysisResults.querySelector('.strengths-section .content');
        if (strengthsContent && summary.strengths) {
        ...
        // 개선점 섹션 업데이트
        const improvementsContent = analysisResults.querySelector('.improvements-section .content');
        if (improvementsContent && summary.improvements) {
            improvementsContent.innerHTML = '<ul>' + 
                summary.improvements.map(item => '<li>' + item + '</li>').join('') + 
                '</ul>';
        }
        
        // 추천 활동 섹션 업데이트
        const recommendationsContent = analysisResults.querySelector('.recommendations-section .content');
        if (recommendationsContent && summary.recommendations) {
            recommendationsContent.innerHTML = '<ul>' + 
...
// Chart.js CDN과 JavaScript는 여기서 출력
echo '<!-- Chart.js CDN -->
...
/* 모던 헤더 스타일 */
.modern-header {
    background: linear-gradient(135deg, #8B72E6 0%, #6B5CE7 100%);
    padding: 12px 20px;
...
.ui-tabs-nav li.ui-state-active a {
    background: linear-gradient(135deg, #8B72E6 0%, #6B5CE7 100%) !important;
...
{		 
    var CurrentTrackingId= \'' . $currenttrackingid . '\';
