...
echo '<div style="display:none;"><iframe  src="https://mathking.kr/moodle/local/augmented_teacher/teachers/pscmarrival.php?userid='.$USER->id.'"></iframe></div>';
...
if($mode==='today')
	...
	foreach($result as $user)
		...
		if($indicators->mngrpick!=1)continue;
		$var='start'.$n1;
		$var2=$schedule->$var;
		$var3='duration'.$n1;
		$var4=$schedule->$var3;
		...
		$engagement2 = $DB->get_record_sql("SELECT * FROM  mdl_logstore_standard_log WHERE userid='$userid' AND courseid NOT LIKE '239' AND component NOT LIKE 'core' AND  component NOT LIKE 'local_webhooks'  ORDER BY id DESC LIMIT 1 ");  // mathkinglog		 
		...
		$lastaccess=time()-$lastaction;
 		...
 		if($lastaccess<10800 || $var2!=NULL  && $var4>0 && (time()- $schedule->timecreated)<86400000)
			...
			elseif($useinfo->data==='자율')
				...
				if($cognitivekick->id!=NULL)$cogkick1.='<tr><td><span style="color:black;">자율</span></td><td width=10%></td> <td> <a href="https://mathking.kr/moodle/local/augmented_teacher/students/today.php?id='.$userid.'&tb=604800" target="_blank">'.$studentname.'</a></td><td> <a href="https://mathking.kr/moodle/local/augmented_teacher/whiteboard/board_onair.php?userid='.$userid.'&mode=2&wboardid='.$cowb->wboardid.'" target="_blank">생각계단 설정</a></td></tr>';
				}
			...
			if(($lastaccess<10800||$npresent==$n2+1||$npresent==$n2+2||$npresent==$n2+3||$npresent==$n2+4||$npresent==$n2+5|$npresent==$n2+6||$npresent==$n2+7||$npresent==$n2+8||$npresent==$n2+9||$npresent==$n2+10||$npresent==$n2) && $nday==$n1)
				...
				if($lastaccess>36000)
					{
					if($tgoal >43200 )$name[$n1][$n2].='<tr style="white-space: nowrap; text-overflow: ellipsis;" height=30><td ><a href="https://mathking.kr/moodle/local/augmented_teacher/students/today.php?id='.$userid.'&tb=604800 " > <img src="https://mathking.kr/Contents/MATH%20MATRIX/MATH%20images/IMG/bessi21_1638961977.png" width=30>'.$studentname.'</a><span onClick="directMessage(\''.$userid.'\')" > <img src="https://mathking.kr/Contents/MATH%20MATRIX/MATH%20images/IMG/MXBESSI1627200629001.png" width=20></span></td><td width=7%>('.$indicators->ngrowth.')</td><td  width=40% align=right>'.$imgtoday.'</td><td width=5%><input type="checkbox" name="checkAccount"  '.$checkstatus.' onClick="ChangeCheckBox(8,\''.$userid.'\',this.checked)"/>'.$insimg.'</td></tr>';
					else 
						{
						$name[$n1][$n2].='<tr style="white-space: nowrap; text-overflow: ellipsis;" height=30><td ><span type="button"  onClick="secondbrain(\''.$userid.'\',\''.$studentname.'\')">'.$laststatus.'</span><a href="https://mathking.kr/moodle/local/augmented_teacher/students/today.php?id='.$userid.'&tb=604800 " >'.$studentname.'</a><span onClick="directMessage(\''.$userid.'\')" > <img src="https://mathking.kr/Contents/MATH%20MATRIX/MATH%20images/IMG/MXBESSI1627200629001.png" width=20></span></td><td  width=7%>('.$indicators->ngrowth.')</td><td  width=40% align=right>'.$imgtoday.'</td><td width=5%><input type="checkbox" name="checkAccount"  '.$checkstatus.' onClick="ChangeCheckBox(8,\''.$userid.'\',this.checked)"/>'.$insimg.'</td></tr>';
						}
					}
				else 
					{
					...
					if($autoGradeState==='AI' && $timecreated-$indicators->tinspect>=$inspecttime &&  $inspect->data!=NULL && $indicators->aion==1 )
						{ 
					...
					elseif($useinfo->data==NULL &&  $inspect->data==NULL && $indicators->aion==1)
						{
						$currenturl='id='.$userid;
						$DB->execute("INSERT INTO {abessi_chat} (mode,userid,userto,sender,contextid,currenturl,mark,t_trigger) VALUES('initiate','$userid','$USER->id','$USER->id','https://mathking.kr/moodle/user/editadvanced.php','$currenturl','1','$timecreated')");				
						}
					}
				}
			else  
				{ 
				 $checkstatus='';
				 $name[$n1][$n2].='<tr style="white-space: nowrap; text-overflow: ellipsis;"height=30><td><span type="button"  onClick="secondbrain(\''.$userid.'\',\''.$studentname.'\')"><img src="https://mathking.kr/Contents/MATH%20MATRIX/MATH%20images/IMG/bessi21_1646637882.png" width=30></span><a href="https://mathking.kr/moodle/local/augmented_teacher/students/today.php?id='.$userid.'&tb=604800 " >'.$studentname.'</a><span onClick="directMessage(\''.$userid.'\')" > <img src="https://mathking.kr/Contents/MATH%20MATRIX/MATH%20images/IMG/MXBESSI1627200629001.png" width=20></span></td><td width=7%>('.$indicators->ngrowth.')</td><td  width=40% align=right>'.$imgtoday.'</td><td width=5%><input type="checkbox"  name="checkAccount"  '.$checkstatus.' onClick="ChangeCheckBox(8,\''.$userid.'\',this.checked)"/>'.$insimg.'</td></tr>';
 				}
			}
		elseif($var4!=0&&(time()- $schedule->timecreated)<86400000)
			{	
			$name[$n1][29].='<tr style="white-space: nowrap; text-overflow: ellipsis;" height=30><td></td><td><a href="https://mathking.kr/moodle/local/augmented_teacher/students/today.php?id='.$userid.'&tb=604800 " >'.$studentname.'</a><span onClick="directMessage(\''.$userid.'\')" > <img src="https://mathking.kr/Contents/MATH%20MATRIX/MATH%20images/IMG/MXBESSI1627200629001.png" width=20></span></td><td width=7%>('.$indicators->ngrowth.')</td><td  width=40% align=right>'.$imgtoday.'</td><td width=5%><input type="checkbox"  name="checkAccount"  '.$checkstatus.' onClick="ChangeCheckBox(8,\''.$userid.'\',this.checked)"/>'.$insimg.'</td></tr>';
			}
		elseif($lastaccess<43200)
			...
			$onlineusers.='<td><a href="https://mathking.kr/moodle/local/augmented_teacher/students/today.php?id='.$userid.'&tb=604800 " >&nbsp;&nbsp;'.$studentname.' </a>&nbsp;&nbsp; <input type="checkbox"  name="checkAccount"  '.$checkstatus.' onClick="ChangeCheckBox(8,\''.$userid.'\',this.checked)"/>'.$insimg.'</td>';
...
foreach($result3 as $value3) 
	...
	$wboardlist.='<tr ><td>'.$abtest->chapter.'</td><td  style="vertical-align: top;"><a href="https://mathking.kr/moodle/local/augmented_teacher/whiteboard/board.php?id='.$value3['studentnote'].'" target="_blank">'.$drillingtext.'</a></td>  <td  style="vertical-align: top; ">'.$creatorname.'</td><td  style="vertical-align: top;">'.date("m/d",$value3['timecreated']).'</td></tr>';
	}
$contentsusage= '<table width=100% align=center><tr ><td width=20%>단원</td><td width=70%>DrillingText for AB_TEST</td><td>T</td><td>시간</td></tr>'.$wboardlist.'</table><hr style="height:2px;border-width:0;color:gray;background-color:gray">';
...
echo '</tbody></table></td><td></td><td width=35% style="vertical-align: top">'.$actionpoint.'</td><td width=2%></td><td style="vertical-align: top">'.$asklist.'</table><hr style="height:2px;border-width:0;color:gray;background-color:gray">'.$feedbackreview.'<hr style="height:2px;border-width:0;color:gray;background-color:gray">'.$contentsusage.'</td></tr></table><hr><table><tr><td>&nbsp;&nbsp;&nbsp; # 일정 외 접속자 '.$onlineusers.' </td> </tr></table><br> 
									</div>
								</div>
							</div>	
