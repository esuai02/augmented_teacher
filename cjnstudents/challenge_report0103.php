...
if (empty($openai_api_key)) {
    error_log('[challenge_report0103.php:' . __LINE__ . '] OPENAI_API_KEY 환경변수가 설정되지 않았습니다.');
}
...
if($checkgoal->id==NULL)
  {
  echo '<br><br><br><br><br><br><table align=center><tr><td>먼저 오늘목표를 설정해 주세요. <hr> <a href="https://mathking.kr/moodle/local/augmented_teacher/students/edittoday.php?id='.$userid.'">목표 입력하기</a></td></tr></table>';
  exit();
  }
...
';
 ...
 exit();
    ...
    $DB->execute("UPDATE {abessi_tracking} SET nwboard='$nwb'  WHERE id='$trackingid'   ORDER BY id DESC LIMIT 1");  

    $preparegpt=$sessiontext.' # 구체적인 학습내용들 : '.$cntprompt.' |  이해도 평가 체크리스트 와 목표 중간점검 체크리스트 생성';
...
$rolea='';
$roleb='';
$talka1='너는 1시간 이내의 단기 집중 세션에 대해 평가하는 수학공부  피드백 전문가야. 학습전략 측면에서 직전에 진행된 구체적인 내용에 대한 비판적인 성찰을 돕는 역할을 해줘';
$talkb1='';
$tone1='체크리스트 형식을 생성해서 체크하도록 해줘. 예, 아니오로 대답 가능한 2가지 목록 제시해줘.  공부내용 점검 6개 항목, 활동 중간점검 3개 항목. 주관식 답변 필요한 항목 배제해줘. 활동 중간 점검부분은 선생님의 피드백이 있는 경우는 해당 피드백 내용으로  체크리스트 항목 3개 만들고 피드백이 없는 경우 아래 예시  반영해줘.
...
2. 활동 중간점검
   - [ ] 목표는 집중하기 편리하도록 충분히 구체적이었나요 ?
   - [ ] 목표를 수행하기 위한 시간 예측은 적절하였나요 ?
   - [ ] 보충시간을 추가하는 등 실제 상황에 맞게 유연하게 대응하였나요 ?
';
