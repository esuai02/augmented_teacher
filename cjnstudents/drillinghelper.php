...
if (empty($openai_api_key)) {
    error_log('[drillinghelper.php:' . __LINE__ . '] OPENAI_API_KEY 환경변수가 설정되지 않았습니다.');
}
...
if($mode==NULL)$changeurl=$contextid.'?'.$currenturl.'&mode=modify';
else 
  {
    $changeurl = str_replace("&mode=modify", "", $contextid.'?'.$currenturl);
 
  }
$userrole=$DB->get_record_sql("SELECT data AS role FROM mdl_user_info_data where userid='$USER->id' AND fieldid='22' "); 
$role=$userrole->role;

//if($role!=='student')exit();

$thisboard=$DB->get_record_sql("SELECT * FROM mdl_abessi_messages where wboardid='$wboardid' ORDER BY id DESC LIMIT 1    ");

$getcnt=$DB->get_record_sql("SELECT maintext FROM mdl_icontent_pages WHERE id ='$thisboard->contentsid' ORDER BY id DESC LIMIT 1"); // 전자책에서 가져오기
$cnttext=$getcnt->maintext; 
$thiscnt=$DB->get_record_sql("SELECT * FROM mdl_abrainalignment_gptresults WHERE type LIKE '$type' AND contentsid LIKE '$thisboard->contentsid' AND contentstype LIKE '$thisboard->contentstype' AND gid LIKE '$gid'  ORDER BY id DESC LIMIT 1 ");
...
if($thiscnt->id!=NULL)
   ...
   // if($mode==='modify')
      ...
      $preparegpt=$cnttext.'중에서 '.$prompt.' 부분을 전체 본문에서의 위치를 식별해줘. 해당부분을 제대로 이했는지 그 부분에 집중된 확인질문들 생성해줘. ';
      $firstdescription ='';
      }/*
    else 
      {
      $preparegpt='보충설명 : ';
      $outputtext=str_replace('"', '', $thiscnt->outputtext);
      $outputtext=str_replace('\n', '. ',$outputtext);

      $outputtext = json_decode($outputtext, true);

      //$outputtext = json_encode($outputtext, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP | JSON_UNESCAPED_UNICODE);
     
    
      $firstdescription =$cnttext.'중에서 '.$prompt.' 부분을 자세히 설명하면 다음과 같습니다.'.$outputtext.' 이 설명들 중에서 번호를 입력하시면 더 상세한 설명을 보실 수 있습니다';
      echo $outputtext;
      }
*/
      $typenew='
    ...
    {  
   // $preparegpt=$cnttext.'중에서 '.$prompt.' 부분을 전체 본문에서의 위치를 식별해줘. 이후 해당 부분을 문맥을 고려하여 작은 단계로 나누어 설명해줘. 다른 부분은 이해가 되니 이 부분만 집중해서 진행해줘. 확인질문 5개 생성';
   $preparegpt=$cnttext.'중에서 '.$prompt.' 부분을 전체 본문에서의 위치를 식별해줘. 해당 부분을 제대로 이했는지 그 부분에 집중된 확인질문들 생성해줘. '; 
...
$avartarimg='https://mathking.kr/Contents/IMAGES/thinkaloud.jpg';
...
$talka1='학생이 질문합니다';
$talkb1='멘토가 답합니다';
$tone1='아주중요해1 - 제시된 구체적인 위치에 질문을 집중해줘. 출력형식은 무조건 다음 예시와 같이 해줘. 각 단계는 따옴표로 감싸고 반드시 컴마로 분리해, 도입부 및 마무리 등의 내용 제거하고 예시의 형식 외의 다른 내용은 아예 표시하지 말아줘. 안 그러면 크게 혼난다. 예시 : "1. 질문", "2. 질문", "3. 질문."';
      ...
      reader.read().then(function processResult(result) {
         ...
         for (let line of dataList) {
            ...
            try {
              ...
              console.error("JSON 파싱 오류:", e, "라인 내용:", line);
              continue; // 파싱 오류가 발생한 경우 해당 라인을 건너뜁니다.
            }

            const { delta, finish_reason } = parsedLine.choices[0];
...
function saveResult()
  {
    var Contentsid= \''.$thisboard->contentsid.'\'; 
    var Contentstype= \''.$thisboard->contentstype.'\';  
