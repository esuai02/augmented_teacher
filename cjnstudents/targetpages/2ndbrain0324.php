...
$nextlearning=$DB->get_record_sql("SELECT * FROM mdl_abessi_mission WHERE  userid='$studentid' AND complete=0 ORDER BY id DESC LIMIT 1");
 			
$checkgoal1= $DB->get_record_sql("SELECT * FROM mdl_abessi_progress WHERE userid='$studentid' AND plantype ='분기목표' AND hide=0 AND deadline > '$timecreated' ORDER BY id DESC LIMIT 1"); 
$checkgoal2= $DB->get_record_sql("SELECT  * FROM  mdl_abessi_today WHERE userid='$studentid' AND timecreated > '$wtimestart' AND type LIKE '주간목표'  ORDER BY id DESC LIMIT 1 ");
$checkgoal3= $DB->get_record_sql("SELECT  * FROM  mdl_abessi_today WHERE userid='$studentid' AND timecreated > '$halfdayago' AND (type LIKE '오늘목표' OR type LIKE '검사요청' ) ORDER BY id DESC LIMIT 1 ");
...
$note1=$DB->get_record_sql("SELECT * FROM mdl_abessi_messages WHERE  userid='$studentid'  AND (status LIKE  'flag'   OR status LIKE 'reply' OR status LIKE 'retry' OR status LIKE 'present' OR (status LIKE 'complete' AND depth LIKE 'NULL' AND teacher_check NOT LIKE '2')  OR  (status LIKE 'review' AND depth LIKE 'NULL' AND teacher_check NOT LIKE '2') ) AND hide=0 AND tlaststroke>'$hoursago6' AND contentstype=2  ORDER BY tlaststroke DESC LIMIT 1");
$note2=$DB->get_record_sql("SELECT * FROM mdl_abessi_messages WHERE  userid='$studentid'  AND status LIKE 'summary' AND timecreated > '$hoursago6'   ORDER BY id DESC LIMIT 1");
$note3=$DB->get_record_sql("SELECT * FROM mdl_abessi_messages WHERE  userid='$studentid'  AND status LIKE 'weekly' ORDER BY id DESC LIMIT 1");
...
elseif($checkgoal3->id==NULL && $missionlog->timecreated >$halfdayago) $ntodo=4; //오늘목표
...
elseif($note1->status==='retry')$ntodo=6; //재시도
elseif($note1->status==='present')$ntodo=7; //발표요청
elseif($note1->status==='reply')$ntodo=8; // 답변도착
elseif($note1->status==='review' && $note1->status==='complete' )$ntodo=9; // 학습완료, 복습예약
elseif($note1->status==='flag' && $note1->flag==1)$ntodo=10; //고민지점
elseif($note2->id !=NULL && $note2->nstroke<10)$ntodo=11; //요약하기
elseif($note3->nstroke<10 && $note3->timecreated > '$aweekago')$ntodo=12; //주간활동 설계
/*
...
if($ntodo==1) 
	...
	$contextid='https://mathking.kr/moodle/local/augmented_teacher/students/missionguide.html';
	$fburl='';			
	}   
elseif($ntodo==2) 
	{
...
elseif($ntodo==5) 
	{
	$fbtype='목표를 구체화해 주세요.';
	$fbtext='구체적인 목표를 입력하면 집중이 되어 공부 스트레스를 줄일 수 있습니다.';
	$contextid='https://mathking.kr/moodle/local/augmented_teacher/students/edittoday.php';
	$fburl='id='.$studentid;			
	}
elseif($ntodo==6) 
...
elseif($ntodo==12) 
	...
	$contextid='https://mathking.kr/moodle/local/augmented_teacher/whiteboard/board_brainstorm.php';
	$fburl='id='.$smnote2->wboardid;			
	}
else
	{
	$fbtype='오늘활동 점검하기';
	$fbtext='현재 특이사항이 없습니다. 정보를 업데이트 합니다.';
	$contextid='https://mathking.kr/moodle/local/augmented_teacher/students/today.php';
	$fburl='id='.$studentid.'&tb=604800';
	$ntodo=0;
	}
...
echo json_encode( array("fbtype"=>$fbtype,"fbtext"=>$fbtext,"contextid"=>$contextid,"fburl"=>$fburl) );
