...
if($type==='목표')
	{
	$flowitem1='목표를 입력할 때 성공을 위한 결심을 함께 한다.';
	$flowitem2='학습의 중간과정에서 목표를 성공하기 위해 의식적인 노력을 기울인다.';
	$flowitem3='목표설정 과정에서 실행 시 발생할 어려운 점들을 미리 고민하고 준비한다.';
	$flowitem4='오늘 목표를 실패하더라도 주간목표를 성공하기 위한 계획 재정비를 한다.';
	$flowitem5='주간목표를 성공하지 못하더라도 분기목표를 성공하기 위해 계획을 재정비한다.';
	...
	$rubric='<table align=center><tr><td><input style=margin-top:8px; type=checkbox onClick="Checkflow(1,\''.$ntype.'\',\''.$studentid.'\',\''.$flowitem1.'\', this.checked)"/></td><td>'.$flowitem1.'</td></tr><tr><td><input style=margin-top:8px; type=checkbox onClick="Checkflow(2,\''.$ntype.'\',\''.$studentid.'\',\''.$flowitem2.'\', this.checked)"/></td><td>'.$flowitem2.'</td></tr><tr><td><input style=margin-top:8px; type=checkbox onClick="Checkflow(3,\''.$ntype.'\',\''.$studentid.'\',\''.$flowitem3.'\', this.checked)"/></td><td>'.$flowitem3.'</td></tr><tr><td><input style=margin-top:8px; type=checkbox onClick="Checkflow(4,\''.$ntype.'\',\''.$studentid.'\',\''.$flowitem4.'\', this.checked)"/></td><td>'.$flowitem4.'</td></tr><tr><td><input style=margin-top:8px; type=checkbox onClick="Checkflow(5,\''.$ntype.'\',\''.$studentid.'\',\''.$flowitem5.'\', this.checked)"/></td><td>'.$flowitem5.'</td></tr></table>';
...
elseif($type==='순서')
	{
	$flowitem1='테스트를 응시하기 전 필요한 개념이 준비가 되어 있는지 확인을 하고 시작한다.';
	$flowitem2='테스트를 응시하기 전 필요한 경우 기존 테스트에 대한 부분 재시도 등을 통하여 상태를 점검한다.';
	$flowitem3='새로운 테스트를 시작하기 전 오답노트를 꼼꼼하게 마무리하는 편이다.';
	$flowitem4='이전 과정에 대한 복습이 필요하다고 판단되는 경우 계획에 없었던 복습을 진행하기도 한다.';
	$flowitem5='테스트를 마무리 한 다음 오답문항들을 전체적으로 검토하면서 취약 개념이나 단원에 대해 생각하고 분석한 다음 후속학습을 재설정한다.';
...
elseif($type==='기억')
	{
	$flowitem1='복습예약을 사용하지 않는다.';
	$flowitem2='복습예약을 하기 때문에 오답활동을 적당히 하는 경향이 있다.';
	$flowitem3='복습예약을 하는 문제도 그렇지 않은 문제와 같은 정도로 체화한다.';
	$flowitem4='복습실행 시 기억상태를 점검하고 새로운 복습예약 시 활동을 개선하는데 활용한다.';
	$flowitem5='복습실행 시 좀 전에 푼 문제와 같이 능숙하게 푸는 것을 목표로 복습예약 시 준비하는 과정을 지속적으로 개선하고 있다.';
...
elseif($type==='개념')
	{
	$flowitem1='도움이 된다고 느끼지만 습관화에 어려움이 있다.';
	$flowitem2='마무리 활동 시 규칙적으로 사용하고 있다.';
	$flowitem3='개념집착노트를 탭에 열어두고 수시로 업데이트 한다.';
	$flowitem4='개념집착노트를 활용하여 영역별 개념들에 대한 정리활동을 하고 있다.';
	$flowitem5='주제단위의 성찰활동을 위하여 개념집착노트를 활용한다.';
...
elseif($type==='발상')
	{
	$flowitem1='동작기억의 원리를 이해하고 있다.';
	$flowitem2='동작기억을 스스로 조절하여 발상하는 방법을 활용하고 있다.';
	$flowitem3='문제가 눈으로 전달되는 상태와 두뇌 속으로 완전히 옮긴 이후의 상태의 차이를 식별할 수 있다.';
	$flowitem4='숙련기준 학습을 통한 자동발상 공부법을 이해하고 있다.';
	$flowitem5='자동발상 공부법이 점진적으로 습관화되는 과정에 있다고 느낀다.';
...
elseif($type==='해석')
	{
	$flowitem1='해석상태와 풀이상태를 효과적으로 스위칭할 수 있다.';
	$flowitem2='지문 소거법을 통한 문제풀이를 활용할 수 있다.';
	$flowitem3='구체화되지 않은 부분을 식별하고 기억소환을 위한 몰입이 가능하다.';
	$flowitem4='사용하지 않은 부분에 대하여 선택과 집중을 통한 순환적 문제해석이 가능하다.';
	$flowitem5='내가 할 수 있는 해석의 끝에 도달한 것을 스스로 확인할 수 있다.';
...
elseif($type==='숙달')
	{
	$flowitem1='개념공부 시 기억저장 활동을 지속적으로 사용하고 있다.';
	$flowitem2='오답노트 시 기억저장 활동을 지속적으로 사용하고 있다.';
	$flowitem3='매주 정해진 루틴으로 개념복습을 위한 기억저장활동을 생활화하고 있다.';
	$flowitem4='기억저장 활동 시 단순반복이 아니라 차분히 의미를 되새기며 반복저장을 하고 있다.';
	$flowitem5='기억저장 활동시 복습이 필요없다는 확신이 든다.';
...
elseif($type==='효율')
	{
	$flowitem1='문제에 대한 해석이나 풀이에 중점을 두어 속도에 대해 고민할 여유가 부족하다.';
	$flowitem2='평소에 문제를 빠르게 푸는 것에 관심이 있는 편이다.';
	$flowitem3='표시된 고민지점을 집중적으로 연습하여 속도향상에 힘쓰고 있다.';
	$flowitem4='필기양을 최소화하면서도 정확하고 효율적인 풀이를 위해 노력하고 있다.';
	$flowitem5='평상시에 풀이가 이해되어도 더 쉬운 풀이를 찾기 위해 노력하고 있다.';
...
function Checkflow(Itemid,Ntype,Userid,Text, Checkvalue)
		{
		    var checkimsi = 0;
		    if(Checkvalue==true)
			{
