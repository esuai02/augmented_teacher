...
elseif($cnttype==8)
    {
      $thiscnt = $DB->get_record_sql("SELECT * FROM mdl_abrainalignment_gptresults WHERE id LIKE '$cntid' ORDER BY id DESC LIMIT 1");
      $outputtext = $thiscnt->outputtext;
      
      // Use str_getcsv to correctly parse the questions
      $questions = str_getcsv($outputtext);
      
      $papertest = '<div class="container"><table align=center width=90%><tr><td><h3>지면평가</h3></td><td>  ('.$stdname.')</td></tr></table><hr><br><br>';
      
      foreach ($questions as $index => $question) {
          // Remove any surrounding quotes from the question
          $question = trim($question, '"');
      
          // Determine if this is the last question for the large answer space
          $answerClass = ($index == count($questions) - 1) ? "large" : "";
          
          $papertest .= '<div class="question">';
         // $papertest .= '<div class="question-number">' . ($index + 1) . '.</div>';
          $papertest .= '<div class="question-text">' . htmlspecialchars($question) . '</div>';
          $papertest .= '<div class="answer-space ' . $answerClass . '"></div>';
          $papertest .= '</div>';
      }
      $papertest .= '</div>';
...
echo '<audio controls style="position:fixed;right:10%;bottom:0%;width:800px;height: 30px;"><source src="'.$cnttext->audiourl.'" type="audio/mpeg"></audio>';
$userrole=$DB->get_record_sql("SELECT data AS role FROM mdl_user_info_data where userid='$USER->id' AND fieldid='22' "); 
$role=$userrole->role;


$Hippocampustest='';
if($cnttext->reflections1!=NULL)$Hippocampustest='&nbsp;&nbsp;&nbsp;&nbsp; <a href="https://mathking.kr/moodle/local/augmented_teacher/LLM/print_papertest.php?cntid='.$cntid.'&cnttype='.$cnttype.'&studentid='.$studentid.'&print=1"><button>보충학습</button></a>';

if(strpos($cnttext->reflections1, '\tab') !== false)$Hippocampustest.='&nbsp;&nbsp;&nbsp;&nbsp; <a href="https://mathking.kr/moodle/local/augmented_teacher/LLM/anki.php?cntid='.$cntid.'&cnttype='.$cnttype.'&studentid='.$studentid.'&wboardid=$wboardid&print=0"><button>ANKI</button></a>';
//if($print==1)$
...
<div width=70% style="background-color: white;">
<table  id="content-to-capture"  width=80% ><tr><td valign=top  class="no-print" width=2%><br><br>&nbsp;&nbsp;&nbsp;</td><td valign=top>'.$papertest.'</td><td valign=top><br><br><a href="https://chat.openai.com/g/g-apWVmPMop-hogisim-haegyeolsa"target="_blank"><img class="no-print" src="https://mathking.kr/Contents/IMAGES/curiosity.png" width=30px></a></td></tr></table>
</div>
<div><table align=center><tr><td valign=top style="font-size:12px;"  class="no-print">  <button id="createwhiteboard" onclick="convertToImageAndUpload(\''.$studentid.'\',\''.$cntid.'\',\''.$print.'\')">화이트보드</button> <span id="print" onclick="window.print()"><img style="margin-top:10px;" src=https://www.mathking.kr/moodle/local/augmented_teacher/IMAGES/printer.png width=30></span>&nbsp;&nbsp;&nbsp;&nbsp;'.$Hippocampustest.'</td></tr></table></div><br><br><br><br><br><br><br></body></html> 
      ...
      canvas.toBlob(async blob => {
          ...
          try {
              ...
              if (result.success) {
                  ...
                  window.open(`https://mathking.kr/moodle/local/augmented_teacher/whiteboard/create_SPECWB.php?wboardid='.$wboardid.'&studentid='.$studentid.'&contentsid='.$cntid.'&contentstype=1&print='.$print.'&imageurl=${result.url}`);
              } else {
