...
// src/utils.ts
import { tool as createTool, generateId } from "ai";
...
function createStepFromChunk(chunk) {
  return match(chunk).returnType().when(
...
function createVoltagentErrorFromSdkError(sdkError, errorStage = "llm_stream") {
  const originalError = match(sdkError).returnType().with({ error: P.not(P.nullish) }, (e) => e.error).with(P.instanceOf(Error), (e) => e).otherwise(
    () => new Error(`An unknown error occurred during Vercel AI operation (stage: ${errorStage})`)
  );
  return match(originalError).returnType().with({ toolCallId: P.not(P.nullish), toolName: P.not(P.nullish) }, (e) => ({
...
function mapToStreamPart(part) {
  return match(part).returnType().with({ type: "text-delta" }, (p) => ({
    type: "text-delta",
    textDelta: p.text
...
var VercelAIProvider = class {
  ...
  async generateText(options) {
    ...
    try {
      const result = await generateText({
        ...
        model: options.model,
        tools: vercelTools,
        ...match2(options.maxSteps).with(P2.number, (maxSteps) => ({ stopWhen: stepCountIs(maxSteps) })).otherwise(() => ({})),
        ...match2(options.provider?.maxTokens).with(P2.number, (maxTokens) => ({ maxOutputTokens: maxTokens })).otherwise(() => ({})),
  ...
  async generateObject(options) {
    const vercelMessages = options.messages.map(this.toMessage);
    const onFinish = match2(options).with({ onStepFinish: P2.not(P2.nullish) }, (o) => {
      return async (result) => {
        const step = createStepFromChunk({
          type: "text",
          text: match2(result.object).with(P2.string, (s) => s).otherwise((o2) => safeStringify(o2)),
          usage: result.usage
        });
        if (step) {
  ...
  async streamObject(options) {
    const vercelMessages = options.messages.map(this.toMessage);
    const onFinish = match2(options).returnType().with({ onStepFinish: P2.not(P2.nullish) }, (o) => {
      return async (event) => {
    ...
    }).with({ onFinish: P2.not(P2.nullish) }, (o) => {
      return async (event) => {
        const finishResult = {
          object: event.object,
    ...
    const result = streamObject({
      ...
      abortSignal: options.signal,
      ...match2(options.provider?.maxTokens).with(P2.number, (maxTokens) => ({ maxOutputTokens: maxTokens })).otherwise(() => ({})),
      ...match2(onFinish).with(P2.not(P2.nullish), (handler) => ({ onFinish: handler })).otherwise(() => ({})),
      onError: /* @__PURE__ */ __name((sdkError) => {
...
function getUsageInfo(usage) {
  return match2(usage).with({ inputTokens: P2.number, outputTokens: P2.number, totalTokens: P2.number }, (u) => ({
