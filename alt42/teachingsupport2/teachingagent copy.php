...
아래 '나의 풀이 메시지함'에서 상세한 설명을 확인하세요!`;
                ...
                return `
                    <div class="history-item" data-id="${item.id}" style="display: flex; gap: 15px; background: ${bgColor}; border: 2px solid ${borderColor};">
                        ...
                        <div class="history-item-actions" style="display: flex; flex-direction: column; gap: 6px; min-width: 280px; max-width: 280px;">
                            <button class="btn btn-sm" onclick="acceptNewRequest(${item.id}, '${item.problemImage || ''}', ${isReRequest})"
                                    style="background: #1976d2; color: white; width: 100%; text-align: left; padding: 10px 12px; font-size: 13px; border-radius: 6px; font-weight: 500;">
                                ${isReRequest ? '🔄 재풀이 시작' : '✅ 표준풀이 시작하기'}
                            </button>

                            <button class="btn btn-sm" onclick="acceptNewRequestWithStyle(${item.id}, '${item.problemImage || ''}', ${isReRequest}, 'concise')"
                                    style="background: #42a5f5; color: white; width: 100%; text-align: left; padding: 10px 12px; font-size: 13px; border-radius: 6px;">
                                ⚡ 수식·기호·화살표 위주 간결 풀이
                            </button>

                            <button class="btn btn-sm" onclick="acceptNewRequestWithStyle(${item.id}, '${item.problemImage || ''}', ${isReRequest}, 'textbook')"
                                    style="background: #5c6bc0; color: white; width: 100%; text-align: left; padding: 10px 12px; font-size: 13px; border-radius: 6px;">
                                📖 시중 교재 해설지 스타일
                            </button>

                            <button class="btn btn-sm" onclick="acceptNewRequestWithStyle(${item.id}, '${item.problemImage || ''}', ${isReRequest}, 'detailed')"
                                    style="background: #26a69a; color: white; width: 100%; text-align: left; padding: 10px 12px; font-size: 13px; border-radius: 6px;">
                                📋 단계별 상세 설명 풀이
                            </button>

                            <button class="btn btn-sm" onclick="acceptNewRequestWithStyle(${item.id}, '${item.problemImage || ''}', ${isReRequest}, 'examples')"
                                    style="background: #66bb6a; color: white; width: 100%; text-align: left; padding: 10px 12px; font-size: 13px; border-radius: 6px;">
                                💡 예시를 넣어 설명하는 스타일
                            </button>

                            <button class="btn btn-sm" onclick="acceptNewRequestWithStyle(${item.id}, '${item.problemImage || ''}', ${isReRequest}, 'exam')"
                                    style="background: #ffa726; color: white; width: 100%; text-align: left; padding: 10px 12px; font-size: 13px; border-radius: 6px;">
                                🎯 실전풀이(시험장 버전)
                            </button>

                            <button class="btn btn-sm" onclick="acceptNewRequestWithStyle(${item.id}, '${item.problemImage || ''}', ${isReRequest}, 'concept')"
                                    style="background: #ab47bc; color: white; width: 100%; text-align: left; padding: 10px 12px; font-size: 13px; border-radius: 6px;">
                                🔑 핵심 개념 강조 풀이
                            </button>

                            <button class="btn btn-sm" onclick="openCustomSolutionModal(${item.id}, '${item.problemImage || ''}', ${isReRequest})"
                                    style="background: #7e57c2; color: white; width: 100%; text-align: left; padding: 10px 12px; font-size: 13px; border-radius: 6px;">
                                📝 다른 풀이 입력
                            </button>
                        </div>
                    </div>
        ...
        // 새로운 요청 수락하고 풀이 시작
        async function acceptNewRequest(interactionId, problemImage, isReRequest = false) {
            if (!problemImage) {
        ...
        // 스타일별 풀이 시작 (6가지 스타일)
        async function acceptNewRequestWithStyle(interactionId, problemImage, isReRequest = false, solutionStyle = 'default') {
            if (!problemImage) {
                alert('문제 이미지를 찾을 수 없습니다.');
                return;
            }
