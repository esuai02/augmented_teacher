...
// 'id' 또는 'interactionid' 파라미터 지원
$id = $_GET['id'] ?? $_GET['interactionid'] ?? 0;
// contentsid와 contentstype으로 조회 지원
...
try {
    ...
    // contentsid와 contentstype으로 조회 (우선순위 1)
    if ($contentsId && $contentsType !== null) {
        ...
        error_log("[get_interaction_data.php] contentsid: {$contentsId}, contentstype: {$contentsType} 로 조회");
    }
    
    // contentsid로만 조회 (우선순위 2)
    if (!$interaction && $contentsId) {
        ...
        error_log("[get_interaction_data.php] contentsid: {$contentsId} 로만 조회");
    }
    
    // id로 조회 (우선순위 3)
    if (!$interaction && $id) {
        $interaction = $DB->get_record('ktm_teaching_interactions', array('id' => $id));
        error_log("[get_interaction_data.php] id: {$id} 로 조회");
    }
    ...
    if ($format === 'section') {
        ...
        // faqtext 포함
        $faqtext = null;
        if (isset($interaction->faqtext) && !empty($interaction->faqtext)) {
