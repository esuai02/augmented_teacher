...
class RuleContentGenerator {
    ...
    private function generateRuleScenarioContent($rule, $context) {
        ...
        foreach ($rule['conditions'] as $index => $condition) {
            $scenarios[] = [
                'scenario_id' => "SCENARIO_{$rule['rule_id']}_{$index}",
                'description' => "조건 {$index + 1} 충족 시나리오",
                'condition' => $condition,
    ...
    private function generateRuleTestCaseContent($rule, $context) {
        return [
            'type' => 'rule_test_case',
            'rule_id' => $rule['rule_id'],
            'title' => "룰 테스트 케이스: {$rule['rule_id']}",
            'content' => [
                'test_cases' => [
                    [
                        'test_id' => "TEST_{$rule['rule_id']}_PASS",
                        'name' => '정상 케이스',
                    ...
                    [
                        'test_id' => "TEST_{$rule['rule_id']}_FAIL",
                        'name' => '실패 케이스',
    ...
    private function generateExecutionFlow($rule) {
        return [
            'step_1' => '조건 평가',
            'step_2' => '조건 충족 여부 확인',
            'step_3' => '룰 실행 (우선순위: ' . $rule['priority'] . ')',
            'step_4' => '액션 수행',
            'step_5' => '결과 반환 (신뢰도: ' . $rule['confidence'] . ')'
        ];
    }
    ...
    private function generateInteractionPoints($rule) {
        ...
        foreach ($rule['action'] as $index => $action) {
            if (strpos($action, 'question:') !== false || strpos($action, 'display_message:') !== false) {
                $points[] = [
                    'interaction_id' => "INTERACTION_{$rule['rule_id']}_{$index}",
                    'type' => strpos($action, 'question:') !== false ? 'question' : 'message',
                    'action' => $action,
                    'trigger' => "룰 {$rule['rule_id']} 실행 시",
                    'response_handling' => $this->generateResponseHandling($action)
                ];
            }
        }
    ...
    private function generateNextSteps($rule) {
        ...
        foreach ($rule['action'] as $action) {
            ...
            if (strpos($action, 'create_instance:') !== false) {
