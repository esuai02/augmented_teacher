...
$username_param = optional_param('username', '', PARAM_RAW);

// username이 있고 userid가 없으면 username으로 사용자 조회
if (!$studentid && $username_param) {
    $thisuser = $DB->get_record_sql("SELECT id FROM mdl_user WHERE username = ?", array($username_param));
    if ($thisuser && $thisuser->id) {
        $studentid = $thisuser->id;
        // 해당 id로 리다이렉트
        redirect(new moodle_url('https://mathking.kr/moodle/local/augmented_teacher/alt42/mentoring/buildingtrust.php', array('userid' => $studentid)));
    } else {
        echo '<script>alert("해당 username의 사용자를 찾을 수 없습니다.");</script>';
        // 다시 username 입력창을 띄우거나 에러처리
    }
}

// $studentid가 없고 username도 없으면 username 입력 창 출력
if (!$studentid && !$username_param) {
    echo '<div style="position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.5);display:flex;justify-content:center;align-items:center;">';
    echo '  <div style="background:#fff;padding:20px;border-radius:8px;text-align:center;">';
    echo '      <p>학생 username을 입력하세요</p>';
    echo '      <input type="text" id="username" placeholder="학생username">';
    echo '      <button onclick="getUsername()">확인</button>';
    echo '  </div>';
    echo '</div>';
    echo '<script>
        function getUsername(){
            ...
            if(uname){
                // 입력된 username을 쿼리 파라미터로 전달
                window.location.href = "https://mathking.kr/moodle/local/augmented_teacher/alt42/mentoring/buildingtrust.php" + "?username=" + encodeURIComponent(uname);
            } else {
                alert("학생username을 입력해 주세요.");
            }
        }
    </script>';
    exit();
...
function get_recipient_id($studentid, $recipientType) {
    global $DB;
...
<body class="bg-gray-100">
    ...
    <script>
        function openGptLink(gptlink) {
            window.open(gptlink+'?userid=<?= $studentid ?>', '_blank');
        }
        // 탭 기능 구현
