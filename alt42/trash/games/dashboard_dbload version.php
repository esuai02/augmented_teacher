    ...
    if ($action === 'save_game_result') {
        ...
        if ($game_id > 0) {
            ...
            if ($record) {
                ...
                echo json_encode(['success' => false, 'message' => '해당 게임을 찾을 수 없습니다.']);
            }
        } else {
...
<script>
    ...
    document.querySelectorAll('.play-button').forEach(button => {
        button.addEventListener('click', () => {
            const gameId = button.getAttribute('data-game-id');
            const url = `https://mathking.kr/moodle/local/augmented_teacher/alt42/games/playgame.php?game_id=${gameId}`;
