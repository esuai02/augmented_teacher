# -*- coding: utf-8 -*-
"""16 경우의 수와 확률 영역.md를 기반으로 OWL 파일 생성"""
...
def to_iri(name):
    ...
    name = name.replace('A', 'A').replace('B', 'B').replace('c', 'c')
    # 앞뒤 언더스코어 제거
    name = name.strip('_')
    # 연속된 언더스코어를 하나로
    while '__' in name:
...
def generate_description(topic_name, stage_name, diagnosis_text=""):
    """description 생성"""
    # stage_name에서 학년 정보 추출
    if "중등수학" in stage_name:
        level = "중등"
    elif "고등수학" in stage_name or "확률과 통계" in stage_name:
        level = "고등"
    else:
        level = "중등"  # 기본값
    ...
    # 주제별 상세 설명 추가
    if "경우의 수" in topic_name or "사건" in topic_name:
        base_desc += f" 경우의 수를 체계적으로 계산하고 적용할 수 있다."
    elif "확률" in topic_name:
        base_desc += f" 확률의 개념을 이해하고 계산할 수 있다."
    elif "순열" in topic_name:
        base_desc += f" 순열의 개념과 계산 방법을 이해하고 적용할 수 있다."
    elif "조합" in topic_name:
        base_desc += f" 조합의 개념과 계산 방법을 이해하고 적용할 수 있다."
    elif "이항정리" in topic_name:
        base_desc += f" 이항정리의 개념을 이해하고 전개식을 활용할 수 있다."
    elif "조건부확률" in topic_name:
        base_desc += f" 조건부확률의 개념을 이해하고 계산할 수 있다."
    
    return base_desc

# 마크다운 파일 파싱
md_file = "16 경우의 수와 확률 영역.md"
...
while i < len(lines):
    ...
    # 단계 감지
    stage_match = re.match(r'(\d+)단계\s*:\s*(.+)', line)
    if stage_match:
...
# OWL 파일 생성
owl_file = "16 경우의_수와_확률_ontology.owl"
with open(owl_file, 'w', encoding='utf-8') as f:
    ...
    f.write('    <rdfs:comment xml:lang="ko">대한민국 경우의 수와 확률 기반 탄력적 복습 온톨로지 (학년 순서 보장, 활동 포함, 상세 설명)</rdfs:comment>\n')
