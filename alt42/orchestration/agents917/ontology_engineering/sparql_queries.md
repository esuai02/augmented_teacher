...
WHERE {
    ?student at:performs ?activity .
    ?activity rdf:type at:LearningActivity .
}
```
...
WHERE {
    ?subclass at:isSubtypeOf ?superclass .
}
...
WHERE {
    ?student rdf:type at:Student .
    OPTIONAL { ?student at:hasAttribute ?level . ?level rdf:type at:MathLevel . }
    OPTIONAL { ?student at:hasAttribute ?confidence . ?confidence rdf:type at:MathConfidence . }
    OPTIONAL { ?student at:hasAttribute ?style . ?style rdf:type at:MathLearningStyle . }
    OPTIONAL { ?student at:hasGoal ?goal . }
    OPTIONAL { ?student at:hasRoutine ?routine . }
}
```
...
WHERE {
    ?student at:hasEmotion ?emotion .
    ?emotion at:leadsTo ?personaId .
    ?personaId at:affects ?persona .
    ?persona at:leadsTo ?routine .
    ?routine rdf:type at:SignatureRoutine .
}
```
...
WHERE {
    ?teacher at:provides ?feedback .
    ?feedback at:generates ?content .
    ?content at:affects ?student .
}
```
...
WHERE {
    ?entity1 at:contradicts ?entity2 .
    ?entity1 at:coOccursWith ?entity2 .
}
```
