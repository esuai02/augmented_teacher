...
```
IF (A, affects, B) AND (A, rdf:type, mk:EmotionPattern)
THEN (B, affectedBy, A)
```
...
```
IF (X, rdf:type, mk:Student) 
   AND NOT EXISTS (X, mk:hasAttribute, ?level WHERE ?level rdf:type mk:MathLevel)
...
```
IF (X, rdf:type, mk:Goal) 
   AND NOT EXISTS (X, mk:hasPlan, ?plan)
...
```turtle
# 전이성 속성 정의
mk:isPrerequisiteOf rdf:type owl:TransitiveProperty .

# 대칭성 속성 정의
mk:coOccursWith rdf:type owl:SymmetricProperty .

# 역관계 정의
mk:hasPrerequisite owl:inverseOf mk:isPrerequisiteOf .
```
