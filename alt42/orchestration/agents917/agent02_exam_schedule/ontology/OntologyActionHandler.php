<?php
/**
 * 온톨로지 액션 핸들러 (Agent02 전용)
 * File: agent02_exam_schedule/ontology/OntologyActionHandler.php
 * 
 * 룰 엔진의 온톨로지 액션을 처리하는 핸들러 - 시험 일정 관련
 */
...
class OntologyActionHandler {
    ...
    public function __construct($agentId = null, array $context = [], ?int $studentId = null) {
        ...
        try {
            ...
            error_log("[OntologyActionHandler] OntologyEngine 초기화 완료 [File: " . __FILE__ . ", Line: " . __LINE__ . "]");
        } catch (Exception $e) {
            error_log("[OntologyActionHandler] OntologyEngine 초기화 실패: " . $e->getMessage() . " [File: " . __FILE__ . ", Line: " . __LINE__ . "]");
            // OntologyEngine이 없어도 기본 동작은 가능하도록 null로 설정
    ...
    public function executeAction($action): array {
        try {
            // OntologyEngine이 초기화되지 않았는지 확인
            if ($this->ontologyEngine === null) {
                error_log("[OntologyActionHandler] OntologyEngine이 초기화되지 않았습니다. [File: " . __FILE__ . ", Line: " . __LINE__ . "]");
                return [
                    'success' => false,
                    'error' => 'OntologyEngine이 초기화되지 않았습니다.'
                ];
            }
