...
function showAgent01Modal() {
    ...
    // 현재 선택된 MBTI (URL -> localStorage -> phpData 순)
    let currentMBTI = 'INTJ';
    try {
        const params = new URLSearchParams(window.location.search || '');
        const fromUrl = params.get('mbti');
        const uid = (window.phpData && window.phpData.studentId) ? String(window.phpData.studentId) : '';
        const fromLS = uid ? (localStorage.getItem('alt42_mbti_' + uid) || '') : '';
        const fromPhp = (window.phpData && window.phpData.mbti) ? window.phpData.mbti : '';
        currentMBTI = (fromUrl || fromLS || fromPhp || 'INTJ').toUpperCase();
    } catch (_) {}
...
async function loadAgent01Data() {
    try {
        ...
        if (data && data.success) {
            ...
            // 최신 MBTI를 상단 표시에도 반영 (새로고침 후에도 유지되도록 DB에서 읽힘)
            try {
                const loadedMbti = data.data && data.data.mbti;
                if (loadedMbti) {
                    if (!window.phpData) window.phpData = {};
                    window.phpData.mbti = loadedMbti;
                }
            } catch (_e) {}
...
async function saveAgent01Data() {
    ...
    const mbtiType = (selectedMBTI.getAttribute('data-mbti') || '').toUpperCase();

    try {
        // 현재 사용자 ID 해석: URL ?userid => phpData => window.currentUserId
        let resolvedUserId = null;
        try {
            const params = new URLSearchParams(window.location.search || '');
            resolvedUserId = params.get('userid')
                || (window.phpData && window.phpData.studentId)
        ...
        if (!resolvedUserId) {
            alert('학생 ID를 찾을 수 없습니다. 페이지를 다시 열어주세요.');
            return;
        }

        // DB 없이 저장: localStorage + URL 파라미터로 유지
        try {
            localStorage.setItem('alt42_mbti_' + String(resolvedUserId), mbtiType);
        } catch (_) {}
        try {
            const url = new URL(window.location.href);
            url.searchParams.set('mbti', mbtiType);
            window.history.replaceState(null, '', url.toString());
        } catch (_) {}

        if (!window.phpData) window.phpData = {};
        window.phpData.mbti = mbtiType;
        ...
        console.log('✅ Agent 01 MBTI saved (localStorage/URL) and state updated successfully');
