 ...
 * File: alt42/orchestration/agents/agent22_module_improvement/ontology/test_load.php
 * 
 * 이 파일은 범용 OntologyActionHandler.php가 제대로 로드되고 사용할 수 있는지 테스트합니다.
 */
...
<head>
    <meta charset="utf-8">
    <title>Agent22 범용 OntologyActionHandler 로드 테스트</title>
    <style>
...
<body>
    <div class="container">
        <h1>🧪 Agent22 범용 OntologyActionHandler.php 로드 테스트</h1>
        ...
        $enginePath = __DIR__ . '/UniversalOntologyEngine.php';
        $configPath = __DIR__ . '/OntologyConfig.php';
        $fileLoaderPath = __DIR__ . '/OntologyFileLoader.php';
        ...
        // UniversalOntologyEngine.php 확인
        if (file_exists($enginePath)) {
            $engineSize = filesize($enginePath);
            echo "<div class='test-result success'>";
            echo "✅ <strong>UniversalOntologyEngine.php</strong> 파일 존재<br>";
            echo "크기: " . number_format($engineSize) . " bytes<br>";
            echo "</div>";
        } else {
            echo "<div class='test-result error'>";
            echo "❌ <strong>UniversalOntologyEngine.php</strong> 파일 없음<br>";
            echo "</div>";
        }
        
        // OntologyConfig.php 확인
        if (file_exists($configPath)) {
            $configSize = filesize($configPath);
            echo "<div class='test-result success'>";
            echo "✅ <strong>OntologyConfig.php</strong> 파일 존재<br>";
            echo "크기: " . number_format($configSize) . " bytes<br>";
            echo "</div>";
        } else {
            echo "<div class='test-result error'>";
            echo "❌ <strong>OntologyConfig.php</strong> 파일 없음<br>";
            echo "</div>";
        }
        
        // OntologyFileLoader.php 확인
        if (file_exists($fileLoaderPath)) {
            $loaderSize = filesize($fileLoaderPath);
            echo "<div class='test-result success'>";
            echo "✅ <strong>OntologyFileLoader.php</strong> 파일 존재<br>";
            echo "크기: " . number_format($loaderSize) . " bytes<br>";
            echo "</div>";
        } else {
            echo "<div class='test-result error'>";
            echo "❌ <strong>OntologyFileLoader.php</strong> 파일 없음<br>";
            echo "</div>";
        }
        ...
        try {
            // OntologyConfig.php 먼저 로드
            if (file_exists($configPath)) {
                require_once($configPath);
                echo "<div class='test-result success'>";
                echo "✅ <strong>OntologyConfig.php</strong> 로드 성공<br>";
                echo "클래스 존재: " . (class_exists('OntologyConfig') ? '✅ 예' : '❌ 아니오') . "<br>";
                echo "</div>";
            }
            
            // UniversalOntologyEngine.php 로드
            if (file_exists($enginePath)) {
                require_once($enginePath);
                echo "<div class='test-result success'>";
                echo "✅ <strong>UniversalOntologyEngine.php</strong> 로드 성공<br>";
                echo "클래스 존재: " . (class_exists('UniversalOntologyEngine') ? '✅ 예' : '❌ 아니오') . "<br>";
                echo "</div>";
            }
        ...
        try {
            // OntologyActionHandler 인스턴스 생성 (범용 핸들러는 agentId 필요)
            $testContext = [
        ...
        $servicePath = __DIR__ . '/../ui/agent_garden.service.php';
        if (file_exists($servicePath)) {
            ...
            if (strpos($serviceContent, 'OntologyActionHandler') !== false) {
                ...
                // 범용 핸들러 경로 확인
                if (preg_match("/agent22_module_improvement\/ontology\/OntologyActionHandler\.php/", $serviceContent)) {
                    echo "✅ 범용 핸들러 경로 사용 중<br>";
                }
