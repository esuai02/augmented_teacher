<?php
/**
 * Agent 14 - Current Position 데이터 매핑 분석 도구
 * view_reports.php에서 사용하는 데이터와 rules.yaml, data_access.php를 비교 분석
 * 
 * @file data_mapping_analysis.php
 * @location alt42/orchestration/agents/agent14_current_position/rules/
 */
...
if (!empty($rulesYamlContent)) {
    ...
    // 데이터 소스에서 필드 추출 (주석에서)
    preg_match_all('/(?:data_source|데이터 소스):\s*([a-z_]+(?:\s*,\s*[a-z_]+)*)/i', $rulesYamlContent, $matches);
    if (!empty($matches[1])) {
...
if (!empty($dataAccessContent)) {
    ...
    // 변수명으로 필드 추출 (예: $diaryRecord, $completion_rate 등)
    preg_match_all('/\$([a-z_]+)\s*=/i', $dataAccessContent, $matches);
    if (!empty($matches[1])) {
        foreach ($matches[1] as $var) {
            if (strpos($var, 'context') === false && strpos($var, 'DB') === false && 
                strpos($var, 'student') === false && strpos($var, 'diary') !== false) {
...
if (file_exists($viewReportsPath)) {
    ...
    if (!empty($matches[1])) {
        foreach ($matches[1] as $selectClause) {
            $fields = preg_split('/,\s*/', $selectClause);
            foreach ($fields as $field) {
                $field = trim($field);
...
// abessi_todayplans 테이블 구조 확인 (수학일기)
...
foreach ($rulesFields as $field) {
    ...
    // abessi_todayplans 테이블 확인
    if ($DB->get_manager()->table_exists(new xmldb_table('abessi_todayplans'))) {
        try {
            $columns = $DB->get_columns('abessi_todayplans');
            if (isset($columns[$field])) {
                $sampleData = $DB->get_record('abessi_todayplans', ['userid' => $studentid], $field, IGNORE_MISSING);
                if ($sampleData && isset($sampleData->$field)) {
                    $exists = true;
                    $tableName = 'abessi_todayplans';
                    $sampleValue = is_string($sampleData->$field) ? substr($sampleData->$field, 0, 50) : $sampleData->$field;
                }
            }
        } catch (Exception $e) {
            error_log("Error checking abessi_todayplans: " . $e->getMessage() . " [File: " . __FILE__ . ", Line: " . __LINE__ . "]");
        }
    }
...
function classifyDataType($fieldName, $tableName = '') {
    // survdata: 설문 응답 데이터 (사용자 입력)
    $survFields = [
        'calmness', 'pomodoro', 'inefficiency', 'weekly_goal', 'daily_plan', 
        'pace_anxiety', 'satisfaction', 'boredom', 'stress_level', 
        'positive_moment', 'problem_count', 'error_note', 'concept_study', 
        'difficulty_level', 'easy_problems', 'self_improvement', 
        'missed_opportunity', 'intuition_solving', 'forced_solving', 
        'questions_asked', 'unsaid_words', 'rest_pattern', 'long_problem', 
        'study_amount', 'plan1', 'plan2', 'plan3', 'plan4', 'plan5', 
        'plan6', 'plan7', 'plan8', 'plan9', 'plan10', 'plan11', 'plan12', 
        'plan13', 'plan14', 'plan15', 'plan16', 'status01', 'status02', 
        'status03', 'status04', 'status05', 'status06', 'status07', 
        'status08', 'status09', 'status10', 'status11', 'status12', 
    ...
    $sysFields = [
        ...
        'student_check', 'turn', 'hide', 'tbegin', 'due1', 'due2', 'due3', 
        'due4', 'due5', 'due6', 'due7', 'due8', 'due9', 'due10', 'due11', 
        'due12', 'due13', 'due14', 'due15', 'due16', 'tend01', 'tend02', 
        'tend03', 'tend04', 'tend05', 'tend06', 'tend07', 'tend08', 
        'tend09', 'tend10', 'tend11', 'tend12', 'tend13', 'tend14', 
        'tend15', 'tend16', 'userid', 'id'
    ];
    
    // gendata: 생성 데이터 (계산/추론)
    $genFields = [
        'calmnessGrade', 'pomodoroUsage', 'errorNoteCount', 'needsAttention',
        'completion_rate', 'progress_status', 'delay_index', 'risk_index',
        'emotion_curve', 'rhythm_score', 'comprehensive_diagnosis',
        'current_position', 'break_point', 'speed_interruption',
        'understanding_interruption', 'current_status', 'unit_by_unit_data',
        'difficulty_level_data', 'stage_by_stage_data', 'understanding_rate',
        'time_spent', 'error_type', 'bottleneck_identification',
        'smooth_section_identification', 'strength_weakness_analysis',
        'structural_summary', 'decision_logic', 'next_class_direction',
        'current_position_based_design', 'intervention_direction'
    ];
    ...
    if (in_array($fieldName, $survFields) || strpos($tableName, 'goinghome') !== false) {
        return 'survdata';
              ...
              strpos($tableName, 'todayplans') !== false) {
        return 'sysdata';
...
foreach ($dbFields as $dbField) {
    ...
    // 테이블명 제거하고 필드명만 추출
    $fieldName = preg_replace('/^(abessi_todayplans|alt42_goinghome|alt42_calmness|abessi_tracking|abessi_messages)\./', '', $dbField);
...
// 유사한 필드 매핑
$similarFields = [
    ['calmness', 'emotion_curve'],
    ['pomodoro', 'rhythm_score'],
    ['error_note', 'error_type'],
    ['completion_rate', 'progress_rate'],
    ['delay_minutes', 'delay_index'],
    ['plan1', 'unit_by_unit_data'],
    ['status01', 'emotion_distribution']
];
...
foreach ($rulesFields as $field) {
    ...
    foreach ($dbFields as $dbField) {
        ...
        if ($field === $dbFieldName || strpos($dbField, $field) !== false) {
            $exists = true;
            break;
        }
    }
    $dbExists[$field] = $exists;
}
...
<head>
    ...
    <title>데이터 매핑 분석 - Agent 14</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        ...
        <div class="header">
            <h1>📊 데이터 매핑 분석 리포트 - Agent 14 (Current Position)</h1>
                    ...
                    <tbody>
                        <?php foreach ($inDbNotInRules as $dbField): ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="badge badge-<?php echo $dataType === 'survdata' ? 'surv' : ($dataType === 'sysdata' ? 'sys' : ($dataType === 'gendata' ? 'gen' : 'info')); ?>">
                                        <?php echo $dataType; ?>
                                    </span>
                                </td>
        ...
        <!-- 6. Rules.yaml 필드별 상세 분석 -->
        <div class="section">
            <h2>📋 Rules.yaml 필드별 상세 분석</h2>
            <table class="data-table">
                <thead>
                    <tr>
                        <th>필드명</th>
                ...
                <tbody>
                    <?php foreach ($rulesFields as $field): ?>
                        <?php 
                        $dataType = classifyDataType($field);
                        $dbExist = $dbExists[$field] ?? false;
                        $dataAccessApplied = $dataAccessApplied[$field] ?? false;
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <?php else: ?>
                                    <span class="badge badge-error">✗ 없음</span>
                                <?php endif; ?>
                            </td>
                            <td>
                                <?php if ($dataAccessApplied): ?>
