#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# File: agent02_exam_schedule/rules/exam_schedule_rule_engine.py
# 확장된 룰 엔진 - 시험 일정 에이전트용
# 
# 추가 기능:
...
class ExamScheduleRuleEngine:
    """
    확장된 Rule Engine for Exam Schedule Agent
    ...
    def decide(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Ensure matched_rule is a valid dict
        if not matched_rule or not isinstance(matched_rule, dict):
            matched_rule = {
                'rule_id': 'default',
                'action': ['display_message: 시험 일정 정보를 분석 중입니다. 추가 정보가 필요할 수 있습니다.'],
                'confidence': 0.5,
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("Usage: python exam_schedule_rule_engine.py '<context_json>' [rules_file.yaml]", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        # Make decision
        engine = ExamScheduleRuleEngine(rules_file)
