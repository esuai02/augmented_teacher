<?php
/**
 * Centralized Error Handler for Student Manual System
 * File: alt42/orchestration/agents/studentmanual/includes/error_handler.php
 *
 * Provides standardized error handling and logging with file path and line number
 */

class StudentManualErrorHandler {
    /**
    ...
    public static function logError($message, $file = null, $line = null) {
        ...
        error_log("[StudentManual] Error in {$file}:{$line} - {$message}");
    }

    /**
     * Return standardized JSON error response with file and line information
     *
     * @param string $message Error message
    ...
    public static function handleDbError($e, $context = '') {
        ...
        $file = $e->getFile();
        $line = $e->getLine();
        self::logError("Database error{$contextInfo}: " . $e->getMessage() . " in {$file}:{$line}");
        return null;
    }
    ...
    public static function tableExists($DB, $tableName) {
        try {
            ...
            $file = __FILE__;
            $line = __LINE__;
            self::logError("Table check failed for {$tableName}: " . $e->getMessage() . " in {$file}:{$line}");
            return false;
        }
    }
    ...
    public static function safeGetRecord($DB, $table, $conditions, $fields = '*') {
        try {
            ...
            if (!self::tableExists($DB, $table)) {
                $file = __FILE__;
                $line = __LINE__;
                self::logError("Table '{$table}' does not exist in {$file}:{$line}");
                return null;
            }
     ...
     * @return bool True on success, false on error
     */
    public static function safeDeleteRecord($DB, $table, $conditions) {
        try {
            // Check table exists first
            if (!self::tableExists($DB, $table)) {
                $file = __FILE__;
            ...
            self::handleDbError($e, "Deleting records from {$table}");
            return false;
        }
    }
    ...
    public static function displayErrorPage($title, $message, $details = []) {
        ...
        <body>
            <div class="error-container">
                ...
                <div class="file-info">
                    <strong>파일:</strong> <?php echo htmlspecialchars($file); ?><br>
                    <strong>라인:</strong> <?php echo htmlspecialchars($line); ?>
                </div>
