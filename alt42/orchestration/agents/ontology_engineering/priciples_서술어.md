...
## 🧱 6️⃣ AlphaTutor용 기본 서술어 세트 (핵심 15개)

| 계층             | 서술어            | 의미            |
| -------------- | -------------- | ------------- |
| **Cognitive**  | `hasPart`      | 개념의 구성 관계     |
|                | `requires`     | 학습 선행 관계      |
|                | `extends`      | 개념 확장 관계      |
| **Affective**  | `causes`       | 원인-결과 (정서/상태) |
|                | `affects`      | 영향 관계 (인지·정서) |
|                | `reduces`      | 상태를 낮춤        |
|                | `enhances`     | 상태를 높임        |
| **Behavioral** | `leadsTo`      | 특정 행동 결과      |
|                | `resultsIn`    | 결과 유발         |
|                | `suggests`     | 다음 피드백 제안     |
|                | `recommends`   | 명시적 추천        |
|                | `supports`     | 보조/강화 관계      |
| **Meta**       | `isSubtypeOf`  | 계층 관계         |
|                | `contradicts`  | 논리 충돌 관계      |
|                | `coOccursWith` | 함께 발생하는 관계    |

> 🧠 이 정도 세트면 룰 기반 온톨로지 → 의미 기반 온톨로지로 자연스럽게 확장 가능해.

---

## ⚡ 7️⃣ 실전 예시 — 실제 대화에서 서술어 선택 과정

**입력 문장:**

> “그래프는 좀 이해됐는데, 문제 풀 때는 자꾸 실수해요.”
