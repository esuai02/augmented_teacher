            ...
            if (result.success) {
                ...
                // 일반 응답 구조 (result.data.response)
                    ...
                    if (responseData.ontology_strategy) {
                        ...
                        ontologyHtml += '<strong style="color: #1e40af; font-size: 1.1em;">📋 온톨로지 기반 전략</strong><br>';
                        
                        if (strategy['mk:hasMathLearningStyle']) {
                            ontologyHtml += `<div style="margin-top: 0.5rem;"><strong>학습 스타일:</strong> ${escapeHtml(strategy['mk:hasMathLearningStyle'])}</div>`;
                        }
                        if (strategy['mk:hasStudyStyle']) {
                            ontologyHtml += `<div style="margin-top: 0.5rem;"><strong>공부 스타일:</strong> ${escapeHtml(strategy['mk:hasStudyStyle'])}</div>`;
                        }
                        if (strategy['mk:hasMathConfidence'] !== undefined) {
                            ontologyHtml += `<div style="margin-top: 0.5rem;"><strong>수학 자신감:</strong> ${strategy['mk:hasMathConfidence']}/10</div>`;
                        }
                        if (strategy['mk:recommendsUnits'] && Array.isArray(strategy['mk:recommendsUnits'])) {
                            ontologyHtml += `<div style="margin-top: 0.5rem;"><strong>추천 단원:</strong> ${escapeHtml(strategy['mk:recommendsUnits'].join(', '))}</div>`;
                        }
                        if (strategy['mk:recommendsDifficulty']) {
                            ontologyHtml += `<div style="margin-top: 0.5rem;"><strong>추천 난이도:</strong> ${escapeHtml(strategy['mk:recommendsDifficulty'])}</div>`;
                        }
                    ...
                    if (responseData.ontology_procedure && Array.isArray(responseData.ontology_procedure)) {
                        ...
                        if (procedureSteps.length > 0) {
                            ...
                            ontologyHtml += '<strong style="color: #065f46; font-size: 1.1em;">📝 수업 절차 (온톨로지 기반)</strong><br>';
