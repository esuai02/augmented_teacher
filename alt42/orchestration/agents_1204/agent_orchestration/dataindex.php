...
if ($agentid === 'agent01_onboarding') {
    ...
    foreach ($agent01Tables as $tableName => $description) {
        ...
        if (!$exists) {
            // mdl_ 접두사 포함 시도
            $exists = $DB->get_manager()->table_exists(new xmldb_table('mdl_' . $tableName));
            if ($exists) {
                $tableName = 'mdl_' . $tableName;
            }
        }
        
        if ($exists && !in_array($tableName, $dbTables)) {
            $dbTables[] = $tableName;
            try {
                $columns = $DB->get_columns($tableName);
                foreach ($columns as $colName => $colInfo) {
...
foreach ($rulesFields as $field) {
    ...
    // alt42o_learning_assessment_results 테이블 확인 (16개 설문 질문: qa01~qa16, 영역별 점수)
    if (!$exists && $DB->get_manager()->table_exists(new xmldb_table('alt42o_learning_assessment_results'))) {
        try {
            $columns = $DB->get_columns('alt42o_learning_assessment_results');
            if (isset($columns[$field])) {
                $sampleData = $DB->get_record('alt42o_learning_assessment_results', ['userid' => $studentid], $field, IGNORE_MISSING);
                if ($sampleData && isset($sampleData->$field)) {
                    $exists = true;
                    $tableName = 'alt42o_learning_assessment_results';
                    $sampleValue = is_string($sampleData->$field) ? substr($sampleData->$field, 0, 50) : $sampleData->$field;
                }
            }
        } catch (Exception $e) {
            error_log("Error checking alt42o_learning_assessment_results: " . $e->getMessage() . " [File: " . __FILE__ . ", Line: " . __LINE__ . "]");
        }
    }
    
    // abessi_mbtilog 테이블 확인 (MBTI 정보)
    if (!$exists && $DB->get_manager()->table_exists(new xmldb_table('abessi_mbtilog'))) {
        try {
            ...
            if (isset($columns[$field])) {
                ...
                if ($sampleData && isset($sampleData->$field)) {
                    $exists = true;
                    $tableName = 'abessi_mbtilog';
                    $sampleValue = is_string($sampleData->$field) ? substr($sampleData->$field, 0, 50) : $sampleData->$field;
                }
            }
        } catch (Exception $e) {
            error_log("Error checking abessi_mbtilog: " . $e->getMessage() . " [File: " . __FILE__ . ", Line: " . __LINE__ . "]");
        }
    }
    
    // alt42_student_profiles 테이블 확인 (JSON 데이터)
...
<head>
    ...
    <style>
        * {
        ...
        #testPageSelector:hover {
            border-color: #059669 !important;
            box-shadow: 0 4px 12px rgba(16, 185, 129, 0.2);
            transform: translateY(-1px);
        }

        #testPageSelector:focus {
            border-color: #059669 !important;
            box-shadow: 0 0 0 3px rgba(16, 185, 129, 0.15);
        }

        #testPageSelector option {
            background: white;
            color: #374151;
            padding: 8px;
        }

        #testPageSelector option:hover {
            background: #d1fae5;
        }

        /* 페르소나 시스템 드롭다운 스타일 */
        #personaSystemSelector {
            border: 2px solid #8b5cf6 !important;
            background: linear-gradient(135deg, #ede9fe 0%, #f5f3ff 100%) !important;
            color: #5b21b6 !important;
        }

        #personaSystemSelector:hover {
            border-color: #7c3aed !important;
            box-shadow: 0 4px 12px rgba(139, 92, 246, 0.2);
            transform: translateY(-1px);
        }

        #personaSystemSelector:focus {
            border-color: #7c3aed !important;
            box-shadow: 0 0 0 3px rgba(139, 92, 246, 0.15);
        }

        #personaSystemSelector option {
            background: white;
            color: #374151;
            padding: 8px;
        }

        #personaSystemSelector option:hover {
            background: #ede9fe;
        }
...
<body>
    ...
    <div class="container" style="padding-top: 60px;">
        <div class="agent-selector-container">
            <div style="display: flex; gap: 1rem; align-items: flex-end;">
                ...
                <!-- 페르소나 시스템 바로가기 드롭다운 (중앙 33%) -->
                <div style="flex: 1;">
                    <label for="personaSystemSelector" style="display: block; margin-bottom: 0.5rem; font-weight: 600; color: #374151;">🎭 페르소나 룰 변수 입력:</label>
                    <select id="personaSystemSelector" onchange="openPersonaSystem()" style="width: 100%; padding: 0.75rem 1rem; border: 2px solid #8b5cf6; border-radius: 8px; font-size: 1rem; font-weight: 500; background: linear-gradient(135deg, #ede9fe 0%, #f5f3ff 100%); cursor: pointer; transition: all 0.2s; color: #5b21b6;">
                        <option value="">-- 페르소나 룰 변수 입력 선택 --</option>
                        <?php foreach ($allAgents as $agentId => $agentName):
                            // 페르소나 시스템 경로 확인
                            $personaChatPath = __DIR__ . '/../' . $agentId . '/persona_system/chat01.php';
                            $hasPersonaChat = file_exists($personaChatPath);

                            if ($hasPersonaChat):
                        ?>
                            <option value="../<?php echo $agentId; ?>/persona_system/chat01.php?tab=student"
                                    <?php echo $agentid === $agentId ? 'selected' : ''; ?>
                                    data-agent="<?php echo $agentId; ?>">
                                <?php echo htmlspecialchars($agentName); ?> - Persona
                            </option>
                ...
                <!-- 테스트 페이지 드롭다운 (우측 33%) -->
                <div style="flex: 1;">
                    <label for="testPageSelector" style="display: block; margin-bottom: 0.5rem; font-weight: 600; color: #374151;">🧪 테스트 페이지:</label>
                    <select id="testPageSelector" onchange="openTestPage()" style="width: 100%; padding: 0.75rem 1rem; border: 2px solid #10b981; border-radius: 8px; font-size: 1rem; font-weight: 500; background: linear-gradient(135deg, #d1fae5 0%, #ecfdf5 100%); cursor: pointer; transition: all 0.2s; color: #065f46;">
                        <option value="">-- 테스트 페이지 선택 --</option>
                        <?php foreach ($allAgents as $agentId => $agentName):
                            // 테스트 페이지 경로 확인
                            $testChatPath = __DIR__ . '/../' . $agentId . '/persona_system/test_chat.php';
                            $testPath = __DIR__ . '/../' . $agentId . '/persona_system/test.php';
                            $hasTestChat = file_exists($testChatPath);
                            $hasTest = file_exists($testPath);

                            if ($hasTestChat || $hasTest):
                                $testFile = $hasTestChat ? 'test_chat.php' : 'test.php';
                        ?>
                            <option value="../<?php echo $agentId; ?>/persona_system/<?php echo $testFile; ?>"
                                    <?php echo $agentid === $agentId ? 'selected' : ''; ?>
                                    data-agent="<?php echo $agentId; ?>">
                                <?php echo htmlspecialchars($agentName); ?> - Test
                            </option>
                        <?php endif; endforeach; ?>
                    </select>
                </div>
    ...
    <script>
        ...
        function openTestPage() {
            const testPageSelector = document.getElementById('testPageSelector');
            const selectedPath = testPageSelector.value;
            ...
            if (selectedPath) {
                ...
                // 선택 초기화 (다시 선택할 수 있도록)
                setTimeout(() => {
                    testPageSelector.selectedIndex = 0;
                }, 100);
            }
        }

        // 페르소나 시스템 열기 함수 (새 탭으로)
        function openPersonaSystem() {
            const personaSystemSelector = document.getElementById('personaSystemSelector');
            const selectedPath = personaSystemSelector.value;

            if (selectedPath) {
                // 새 탭으로 페르소나 시스템 페이지 열기
                window.open(selectedPath, '_blank');

                // 선택 초기화 (다시 선택할 수 있도록)
                setTimeout(() => {
                    personaSystemSelector.selectedIndex = 0;
                }, 100);
            }
        }

        // 에이전트 선택 변경 시 테스트 페이지와 페르소나 시스템 드롭다운도 동기화
        document.addEventListener('DOMContentLoaded', function() {
            const agentSelector = document.getElementById('agentSelector');
            const testPageSelector = document.getElementById('testPageSelector');
            const personaSystemSelector = document.getElementById('personaSystemSelector');

            if (agentSelector && testPageSelector) {
                agentSelector.addEventListener('change', function() {
                    const selectedAgent = this.value;

                    // 해당 에이전트의 테스트 페이지 옵션 찾기
                    const testOptions = testPageSelector.querySelectorAll('option');
                    testOptions.forEach(function(option) {
                        if (option.getAttribute('data-agent') === selectedAgent) {
                            option.selected = true;
                        }
                    });

                    // 해당 에이전트의 페르소나 시스템 옵션 찾기
                    if (personaSystemSelector) {
                        const personaOptions = personaSystemSelector.querySelectorAll('option');
