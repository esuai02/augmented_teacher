...
include_once(__DIR__ . '/orchestrator_bridge.php');
    ...
    public function getStateAnalysis(): array {
        ...
        if ($output) {
            ...
            $result['errors'][] = "Error [quantum_monitoring_dashboard.php:" . __LINE__ . "]: Failed to parse analysis output";
    ...
    public function getEntanglementMap(): array {
        ...
        if ($output) {
            ...
            $result['errors'][] = "Error [quantum_monitoring_dashboard.php:" . __LINE__ . "]: Failed to parse entanglement output";
            $result['raw_output'] = $output;
        }
...
try:
    ...
    # Get state analysis
    analysis = orchestrator.get_state_analysis(student_state=state)
    ...
    result = {
        ...
        'analysis': {
            ...
            'intervention_recommendation': analysis.get('intervention_recommendation', ''),
            'strengths': analysis.get('strengths', []),
            'weaknesses': analysis.get('weaknesses', []),
            'recommended_agents': analysis.get('recommended_agents', []),
            'state_summary': {
                'cognitive_clarity': analysis.get('state', {}).get('cognitive_clarity', 0),
                'emotional_stability': analysis.get('state', {}).get('emotional_stability', 0),
                'engagement_level': analysis.get('state', {}).get('engagement_level', 0),
                'concept_mastery': analysis.get('state', {}).get('concept_mastery', 0),
                'dropout_risk': analysis.get('state', {}).get('dropout_risk', 0)
            }
        },
        'data_interface_available': DATA_INTERFACE_AVAILABLE,
        'errors': []
...
try:
    ...
    entanglement_data = {}
    for agent_id, connections in ENTANGLEMENT_MAP.items():
        entanglement_data[str(agent_id)] = {
            str(k): float(v) for k, v in connections.items()
        }
...
PYTHON;
    ...
    private function collectAgentContexts(): array {
        ...
        // Agent 8: 침착도
        $contexts[8] = ['calm_score' => 0.6, 'calmness_level' => 3];

        // Agent 11: 문제 정확도
        $contexts[11] = ['accuracy_rate' => 0.75, 'total_problems' => 20];

        // Agent 12: 휴식 루틴
        $contexts[12] = ['rest_count' => 5, 'average_interval' => 55];

        // Agent 3: 목표
        $contexts[3] = ['goal_progress' => 0.6, 'goal_effectiveness' => 0.7];

        // Agent 9: 포모도로
        $contexts[9] = ['pomodoro_completion' => 0.8];

        // Agent 4: 참여도
        $contexts[4] = ['engagement_level' => 0.65, 'dropout_risk' => 0.2];

        // DB에서 실제 데이터 조회 시도
        try {
            // Calmness data
            $calm = $this->db->get_record_sql(
                "SELECT calm_score, calmness_level FROM {abessi_calm_log}
                 WHERE userid = ? ORDER BY timecreated DESC LIMIT 1",
                [$this->userid]
            );
            if ($calm) {
                $contexts[8] = [
                    'calm_score' => (float)$calm->calm_score,
                    'calmness_level' => (int)$calm->calmness_level
                ];
            }
        } catch (Exception $e) {
            // Use defaults
    ...
    private function runPythonCode(string $code): ?string {
        $tempFile = tempnam(sys_get_temp_dir(), 'quantum_dash_');
        if (!$tempFile) {
            $this->errors[] = "Error [quantum_monitoring_dashboard.php:" . __LINE__ . "]: Failed to create temp file";
            return null;
        }

        $tempFile .= '.py';
        if (file_put_contents($tempFile, $code) === false) {
            $this->errors[] = "Error [quantum_monitoring_dashboard.php:" . __LINE__ . "]: Failed to write temp file";
            return null;
        }
