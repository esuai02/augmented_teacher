...
<head> 
    ...
    <link rel="stylesheet" href="css/bias-card-session.css">
    <link rel="stylesheet" href="css/typing-effects.css">
    <link rel="stylesheet" href="css/bias-card-complete-library.css">
    <link rel="stylesheet" href="css/math-persona-system.css">
    ...
    <style>
        * {
        ...
        /* 시작 노드 특별 스타일 */
        .constellation-node.start-node {
            background: radial-gradient(circle, rgba(34, 197, 94, 0.8), rgba(34, 197, 94, 0.2));
            border: 3px solid #22c55e;
            animation: startPulse 3s infinite;
        }

        .constellation-node.start-node:hover {
            transform: scale(1.2);
            transition: all 0.3s ease;
        }
        ...
        /* 마스터리 노드 특별 스타일 (중앙) */
        .constellation-node.mastery-node {
            background: radial-gradient(circle, rgba(255, 215, 0, 0.9), rgba(147, 51, 234, 0.3));
        ...
        .constellation-node.mastery-node.unlocked {
            opacity: 1;
            cursor: pointer;
            animation: masteryGlow 2s infinite;
        }

        .constellation-node.mastery-node.completed {
            background: radial-gradient(circle, rgba(255, 215, 0, 1), rgba(255, 215, 0, 0.4));
            border: 4px solid #ffd700;
        ...
        @keyframes unlockPulse {
            0% { transform: scale(1); }
            50% { transform: scale(1.3); box-shadow: 0 0 30px rgba(34, 197, 94, 0.8); }
...
<body>
    ...
    <!-- 툴바 -->
    <div class="toolbar">
        <button class="tool-btn" onclick="if(window.updatedNavigationMap) window.updatedNavigationMap.open(); else { window.updatedNavigationMap = new UpdatedNavigationMap(); window.updatedNavigationMap.open(); }">
            🗺️ 우주 지도
        </button>
        <button class="tool-btn" onclick="window.biasCardCompleteLibrary?.open()">
            🃏 카드 도감
        </button>
        <button class="tool-btn math-persona-btn" onclick="openMathPatternGuide()" style="background: linear-gradient(135deg, #667eea, #764ba2); border: 2px solid #667eea;">
            📚 수학 인지관성 도감
        </button>
            ...
            <div class="journey-map">
                ...
                <div id="contentPanel">
                    <div style="text-align: center; color: #888;">
                        <p style="font-size: 1.5em; margin-bottom: 15px; color: #22c55e;">
                            🌟 하단의 초록빛 별에서 여정을 시작하세요
                        </p>
                        <p style="font-size: 1.1em; margin-bottom: 10px;">
                            정팔각형의 각 노드는 수학적 사고의 한 측면을 나타냅니다
                        </p>
                        <p style="font-size: 0.95em; opacity: 0.8;">
                            모든 여정을 완료하면 중앙의 황금빛 마스터리가 열립니다 👑
                        </p>
                    </div>
                </div>
            </div>
    ...
    <script src="js/bias-detection-system.js"></script>
    <script src="js/personalized-narrative-engine.js"></script>
    <script src="js/cosmic-intervention-system.js"></script>
    <script src="js/bias-card-system.js"></script>
    <script src="js/bias-card-integration.js"></script>
    <script src="js/bias-card-library.js"></script>
    <script src="js/bias-card-complete-library.js"></script>
    <script src="js/MathPersonaSystem.js"></script>
    <script src="js/bias-card-session.js"></script>
    ...
    <script>
        ...
        function createConstellation() {
            ...
            const radius = 180; // 더 넓은 반지름
            
            const nodePositions = [];
            
            // 0-7번 노드: 정팔각형 배치 (시작 노드는 하단)
            for (let i = 0; i < 8; i++) {
                // 하단(남쪽)부터 시계방향으로 배치
            ...
            // 8번 노드: 중앙에 최종 마스터리 노드
            nodePositions.push({ x: centerX, y: centerY });
            
            // 연결선 그리기 (여정 진행 경로)
            const connections = [
                // 시작점(0)에서 초기 진행 가능 노드들
                [0, 1], [0, 7],
                // 정팔각형 원형 연결
            ...
            nodePositions.forEach((pos, i) => {
                ...
                // 진행 상황에 따라 상태 업데이트
                if (window.journey?.journeyState.completedNodes.has(i)) {
                    node.classList.add('completed');
                    node.classList.remove('locked');
                }
                if (window.journey?.journeyState.unlockedNodes.has(i)) {
                    node.classList.remove('locked');
                    node.classList.add('unlocked');
                }
                ...
                node.onclick = () => {
                    console.log(`🌟 노드 ${i} 클릭됨`);
                    if (window.journey) {
                        window.journey.selectNode(i);
                    } else {
                        console.error('❌ journey 인스턴스가 없습니다!');
                        // journey가 없으면 생성
                        window.journey = new InteractiveJourney();
                        setTimeout(() => window.journey.selectNode(i), 100);
                    }
                };
        ...
        // 수학 인지관성 도감 열기
        function openMathPatternGuide() {
            // 기존 MathPersonaSystem 사용 시도
            if (window.mathPersonaSystem && typeof window.mathPersonaSystem.open === 'function') {
                window.mathPersonaSystem.open();
            } else {
                // 대체 방법: 새 창에서 도감 열기
                window.open('show_math_patterns.php', '_blank');
            }
        }
...
${window.userRole === 'teacher' ? '👩‍🏫 선생님 모드: 대기 시간 없이 진행 가능' : ''}
        ...
        window.addEventListener('DOMContentLoaded', () => {
            ...
            function initializeSystem() {
                // InteractiveJourney 클래스가 로드되었는지 확인
                if (typeof InteractiveJourney === 'undefined') {
                    console.log('⏳ InteractiveJourney 클래스 로딩 대기 중...');
                    setTimeout(initializeSystem, 100);
                    return;
                }
                
                // journey 인스턴스가 없으면 생성
                if (!window.journey) {
                    console.log('🚀 InteractiveJourney 인스턴스 생성');
                    window.journey = new InteractiveJourney();
                }
                ...
                // API Text Service로 환영 메시지 표시
                if (window.apiTextService) {
                    const avatarSpeech = document.getElementById('avatarSpeech');
                    window.apiTextService.generateText('welcome').then(text => {
                        window.apiTextService.typeText(avatarSpeech, text || "안녕! 오늘은 어떤 수학적 발견을 할까요? 🌟", {
                            speed: 40,
                ...
                // 편향 카드 시스템 연결
                if (window.biasCardScenarios) {
                    window.biasCardScenarios.onProblemStart = (nodeId) => {
                        console.log(`🃏 노드 ${nodeId}에서 편향 카드 시나리오 시작`);
                    };
                }
                
                // 네비게이션 맵 초기화
                if (!window.updatedNavigationMap && typeof UpdatedNavigationMap !== 'undefined') {
                    window.updatedNavigationMap = new UpdatedNavigationMap();
                    console.log('🗺️ Navigation Map 인스턴스 생성됨');
                }
                
                // Math Persona System 초기화
                if (!window.mathPersonaSystem && typeof MathPersonaSystem !== 'undefined') {
                    window.mathPersonaSystem = new MathPersonaSystem();
                    console.log('📚 Math Persona System 인스턴스 생성됨');
                }
                
                console.log('🌟 Shining Stars 시스템 준비 완료');
                console.log(`사용자 역할: ${window.userRole}`);
                console.log('🎮 Journey 인스턴스:', window.journey);
            }
