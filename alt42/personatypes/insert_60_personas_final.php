<?php
/**
 * 60 페르소나 데이터 삽입 스크립트 (최종 안전 버전)
 * created_at 필드 제거, 이모지를 텍스트로 대체
 */
...
// 60 페르소나 데이터 (첫 3개만 포함, 나머지는 동일한 형식으로 추가)
$patterns = [
    [
        'id' => 1,
        'name' => '아이디어 해방 자동발화형',
...
<head>
    <meta charset="UTF-8">
    <title>60 페르소나 데이터 삽입 (최종 버전)</title>
    <style>
...
<body>
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'insert') {
        ...
        try {
            ...
            echo "<h3>3. 나머지 57개 패턴</h3>";
            echo "<div class='status info'>";
            echo "이 테스트가 성공적이면, 60personas.txt의 전체 데이터를 추가하는 전체 버전을 사용하세요.";
            echo "</div>";
        ...
        <div class="status info">
            ...
            - created_at/updated_at 필드 제거 (자동 생성되는 경우 대비)<br>
            - 이모지를 텍스트로 저장 (문자셋 문제 회피)<br>
            - 처음 3개 패턴만 테스트 삽입
        </div>
        ...
        <h3>문제 해결 옵션</h3>
        <ul>
            <li><a href="check_table_schema.php">테이블 스키마 확인</a></li>
            <li><a href="check_db_status.php">현재 데이터 상태 확인</a></li>
        </ul>
        <?php
    }
    ?>
