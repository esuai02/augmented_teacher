<?php
// Script to update all card pages with "대화 시작" buttons

require_once __DIR__ . '/cards_data.php';

// Map of agent IDs to their folder names
$agentNameMap = [
    1 => '01_time_capsule',
...
foreach ($cards_data as $card) {
    $agentId = $card['id'];
    $agentNumId = intval(substr($agentId, 0, 2));
    ...
    if (file_exists($filePath)) {
        ...
        if (strpos($content, '대화 시작') === false) {
            ...
            $navPattern = '/(<nav class="category-nav">.*?<a href="#connections"[^>]*>[^<]*<\/a>)(.*?<\/nav>)/s';
            $replacement = '$1
            <a href="../../wxsperta_integrated.php?agent=' . $agentNumId . '" class="nav-item" style="background-color: #3498db; color: white;">대화 시작</a>$2';
            
            $updatedContent = preg_replace($navPattern, $replacement, $content);
            
            if ($updatedContent !== $content) {
                file_put_contents($filePath, $updatedContent);
                echo "Updated chat button in: {$filePath}\n";
            } else {
                echo "Could not update: {$filePath}\n";
            }
        } else {
            echo "Chat button already exists in: {$filePath}\n";
        }
    } else {
        // Generate new file with chat button included
        generateCardPage($card, $dirPath, $filePath, $agentNumId);
        echo "Generated new file with chat button: {$filePath}\n";
    }
}
        ...
        $content .= <<<HTML
                    ...
                    <div class="main-project">
                        <div class="project-header">
                            ...
                            <strong>{$projectNum}. {$project['name']}</strong>
                        </div>
...
echo "Chat button update complete!\n";