...
const modePrompts = {
  ...
  curriculum: {
    ...
    dataContext: {
      pomodoro: '포모도로 완료율에 따른 학습 강도 조절',
      activeness: '능동성 점수가 낮으면 더 강한 동기부여 메시지',
      calmness: '침착도가 낮으면 안정감 있는 톤으로 전환',
      score: '점수 추세에 따른 커리큘럼 속도 조정'
    }
  },
  ...
  custom: {
    ...
    dataContext: {
      pomodoro: '짧은 세션부터 시작하여 점진적 증가',
      activeness: '낮은 능동성 시 더 많은 격려와 지원',
      calmness: '불안감 감지 시 즉각적인 정서적 지원',
      score: '점수보다 향상도에 초점, 작은 개선도 축하'
    }
  },
  ...
  exam: {
    ...
    dataContext: {
      pomodoro: '시험 임박 시 고강도 집중 세션',
      activeness: '높은 능동성 유지를 위한 도전적 과제',
      calmness: '적정 긴장감 유지, 과도한 불안 시 조절',
      score: '목표 점수 대비 현재 위치 명확히 제시'
    }
  },
  ...
  mission: {
    ...
    dataContext: {
      pomodoro: '짧은 버스트 세션으로 집중력 유지',
      activeness: '능동성을 경험치로 변환하여 시각화',
      calmness: '스트레스 레벨에 따른 미션 난이도 조절',
      score: '점수를 게임 포인트로 변환하여 동기부여'
    }
  },
  ...
  reflection: {
    ...
    dataContext: {
      pomodoro: '학습 세션 후 성찰 시간 필수 포함',
      activeness: '능동성과 실행력 간의 관계 분석',
      calmness: '정서 상태와 학습 효율성 연결',
      score: '점수 변화의 근본 원인 탐색'
    }
  },
  ...
  selfled: {
    ...
    dataContext: {
      pomodoro: '자유로운 시간 블록 설계 허용',
      activeness: '주도성 지표로 활용하여 피드백',
      calmness: '스트레스 자가 관리 능력 향상',
      score: '자가 평가와 객관적 점수 비교 분석'
    }
  },

  // 7. 도제학습 중심모드 (cognitive)
  cognitive: {
    role: '인지적 도제 마스터',
    ...
    dataContext: {
      pomodoro: '깊은 사고를 위한 긴 세션 권장',
      activeness: '적극적 질문과 탐구 격려',
      calmness: '실수를 학습 기회로 전환',
      score: '과정 평가와 결과 평가 균형'
    }
  },
  ...
  timecentered: {
    ...
    dataContext: {
      pomodoro: '최적 세션 길이 데이터 기반 조정',
      activeness: '활동 시간대별 에너지 레벨 분석',
      calmness: '스트레스와 시간 압박 관계 관리',
      score: '시간 투자 대비 성과 분석'
    }
  },
  ...
  curiositycentered: {
    ...
    dataContext: {
      pomodoro: '탐구 주제별 자유로운 시간 할당',
      activeness: '호기심 수준을 능동성 지표로 활용',
      calmness: '탐구의 즐거움으로 스트레스 해소',
      score: '탐구 깊이와 질문 품질로 평가'
    }
  }
};

// 프롬프트 생성 함수
function generatePrompt(mode, studentData, mbtiType) {
  const modeConfig = modePrompts[mode];
  if (!modeConfig) {
    throw new Error(`Unknown mode: ${mode}`);
  }

  let prompt = modeConfig.corePrompt + '\n\n';

  // MBTI 적응
  if (mbtiType && modeConfig.mbtiAdaptation[mbtiType]) {
  ...
  if (studentData) {
    ...
    if (studentData.pomodoro !== undefined) {
      prompt += `- 포모도로 완료율: ${studentData.pomodoro}%\n`;
      if (studentData.pomodoro < 50) {
        prompt += `  → ${modeConfig.dataContext.pomodoro}\n`;
      }
    }
    
    if (studentData.activeness !== undefined) {
      prompt += `- 능동성: ${studentData.activeness}점\n`;
      if (studentData.activeness < 40) {
        prompt += `  → ${modeConfig.dataContext.activeness}\n`;
      }
    }
    
    if (studentData.calmness !== undefined) {
      prompt += `- 침착도: ${studentData.calmness}점\n`;
      if (studentData.calmness < 40) {
        prompt += `  → ${modeConfig.dataContext.calmness}\n`;
      }
    }
    
    if (studentData.score !== undefined) {
      prompt += `- 현재 점수: ${studentData.score}점\n`;
      prompt += `  → ${modeConfig.dataContext.score}\n`;
    }
  }
