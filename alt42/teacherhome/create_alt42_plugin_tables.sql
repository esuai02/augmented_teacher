...
CREATE TABLE IF NOT EXISTS mdl_alt42DB_plugin_settings_history (
    id INT AUTO_INCREMENT PRIMARY KEY,
    user_id INT NOT NULL COMMENT '사용자 ID (moodle user id)',
    plugin_id VARCHAR(50) NOT NULL COMMENT '플러그인 ID',
...
CREATE TABLE IF NOT EXISTS mdl_alt42DB_plugin_usage_stats (
    ...
    execution_data TEXT DEFAULT NULL COMMENT '실행 데이터 (JSON 형태)',
    timecreated INT(10) NOT NULL COMMENT '생성 시간',
    timemodified INT(10) NOT NULL COMMENT '수정 시간',
...
-- 초기 플러그인 데이터 삽입
INSERT INTO mdl_alt42DB_plugin_types (plugin_id, plugin_title, plugin_icon, plugin_description, timecreated, timemodified) VALUES
...
-- 테이블 생성 완료 메시지
SELECT 'ALT42 플러그인 설정 테이블 생성 완료!' as message;

-- 생성된 테이블 확인
SHOW TABLES LIKE 'mdl_alt42DB_%';

-- 각 테이블의 구조 확인
DESCRIBE mdl_alt42DB_plugin_types;
DESCRIBE mdl_alt42DB_user_plugin_settings;
DESCRIBE mdl_alt42DB_card_plugin_settings;
DESCRIBE mdl_alt42DB_plugin_settings_history;
DESCRIBE mdl_alt42DB_plugin_usage_stats;
