...
$role=$userrole->role; 
$qstnlist = $DB->get_records_sql("SELECT * FROM mdl_question WHERE mathexpression IS NOT NULL  AND chapter IS NULL ORDER BY timemodified DESC LIMIT 10");// 미변환
...
foreach(array_reverse($result2) as $value) 
	...
	$note=$DB->get_record_sql("SELECT * FROM mdl_abessi_messages WHERE userid LIKE '$studentid' AND contentstype LIKE '2' AND  contentsid LIKE '$qstnid' ORDER BY tlaststroke DESC LIMIT 1");
	$nstroke=$note->nstroke;

	if($note->id==NULL)
		{
		$wboardid='OVc4lRh'.$qstnid.'nx4HQkXq_user'.$studentid;
		$DB->execute("INSERT INTO {abessi_messages} (wboardid,userid,userto,userrole,status,active,contentstype,contentsid,timemodified,timecreated) VALUES('$wboardid','$studentid','2','student','complementary','0','2','$qstnid','$timecreated','$timecreated')");
		} 
    ...
    $htmlDom = new DOMDocument; @$htmlDom->loadHTML($questiontext); 
	$imageTags = $htmlDom->getElementsByTagName('img'); $extractedImages = array();
