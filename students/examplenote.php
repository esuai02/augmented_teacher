...
$nch = NULL;
if($cntid) {
    // 방법 1: mdl_abessi_curriculum 테이블에서 cntid가 어떤 cid의 cnt1, cnt2 등에 해당하는지 찾기 (가장 확실한 방법)
    // 데이터 타입 변환을 고려하여 문자열과 숫자 모두 비교
    $cntid_str = (string)$cntid;
    $cntid_int = (int)$cntid;
    
    $curriculums = $DB->get_records_sql("SELECT * FROM mdl_abessi_curriculum");
    foreach($curriculums as $curri) {
        $chapnum = isset($curri->nch) ? (int)$curri->nch : 0;
        for($i = 1; $i <= $chapnum; $i++) {
            $cntstr = 'cnt' . $i;
            if(isset($curri->$cntstr)) {
                $curriCntid = $curri->$cntstr;
                // 문자열과 숫자 모두 비교
                if($curriCntid == $cntid || $curriCntid == $cntid_str || $curriCntid == $cntid_int || (string)$curriCntid == $cntid_str) {
                    $cid = (int)$curri->id;
                    $nch = $i;
    ...
    // 방법 2: 방법 1에서 찾지 못했을 경우, mdl_icontent_pages에서 chapter 필드 확인
    if(!$cid || !$nch) {
        $firstPage = $DB->get_record_sql("SELECT chapter FROM mdl_icontent_pages WHERE cmid='$cntid' ORDER BY pagenum ASC LIMIT 1");
        if($firstPage && isset($firstPage->chapter) && $firstPage->chapter > 0) {
            $foundNch = (int)$firstPage->chapter;
            // nch를 찾았으면, 이 nch에 해당하는 cid 찾기
            if($foundNch && !$cid) {
                $curriculums = $DB->get_records_sql("SELECT * FROM mdl_abessi_curriculum");
                foreach($curriculums as $curri) {
                    $cntstr = 'cnt' . $foundNch;
...
foreach($result as $value)
    ...
    // cid와 nch 확인 및 링크 생성
    if($cid && $nch) {
        // cid와 nch가 모두 있으면 정상적으로 링크 생성
        $patternbankLink = ' <a style="text-decoration:none;color:#1956FF;font-size:20px;margin-left:10px;" href="https://mathking.kr/moodle/local/augmented_teacher/books/patternbank.php?cid='.$cid.'&nch='.$nch.'&studentid=0" target="_blank">문제은행</a>';
    } else {
        // cid나 nch가 없으면 디버깅 정보와 함께 링크 생성 (기본값 사용)
        $linkCid = $cid ? $cid : 63; // 기본값
        $linkNch = $nch ? $nch : 6;  // 기본값
        $patternbankLink = ' <a style="text-decoration:none;color:#1956FF;font-size:20px;margin-left:10px;" href="https://mathking.kr/moodle/local/augmented_teacher/books/patternbank.php?cid='.$linkCid.'&nch='.$linkNch.'&studentid=0" target="_blank">문제은행</a>';
        // 디버깅용 주석 (개발 중에만 사용)
        // $patternbankLink .= ' <!-- DEBUG: cid='.($cid ? $cid : 'NULL').', nch='.($nch ? $nch : 'NULL').', cntid='.$cntid.' -->';
    }
    
    $headerHtml='<table align=center><tr><td valign=top><div class="table-wrapper"><table width=100%><thead><tr><th width=30%><a href="https://mathking.kr/moodle/local/augmented_teacher/teachers/synergetic.php?id='.$USER->id.'&userid='.$studentid.'"><img style="margin-top:5px;" src=https://mathking.kr/moodle/local/augmented_teacher/IMAGES/circulation.png width=40></a> <span style="font-size:20px;margin-bottom:10px;"> 대표유형 노트 </span>'.$patternbankLink.'</th><th><a style="text-decoration:none;color:#1956FF;font-size:20px;" href="https://mathking.kr/moodle/local/augmented_teacher/students/examplenote.php?userid='.$studentid.'&cntid='.$cntid.'&title=%EC%A7%80%EB%A9%B4%ED%8F%89%EA%B0%80&viewtype=papertest"target="_blank">시험지 출력</a></th><th><a style="text-decoration:none;color:#1956FF;font-size:20px;" href="https://mathking.kr/moodle/local/augmented_teacher/books/mynote_test.php?'.$examnoteurl.'"target="_blank">온라인 시험</a></th><th><a style="text-decoration:none;color:#1956FF;font-size:20px;" href="https://mathking.kr/moodle/local/augmented_teacher/students/today.php?id='.$studentid.'&tb=604800"target="_blank">'.$stdname.'</a> '.$notetitle.'   </th></tr></thead></table><table><tr><td><hr></td><td><hr></td><td><hr></td></tr></table>'.$papertest.'</td></tr></table></div>';

    // papertest 모드인 경우: 표지 + 본문, 그 외 모드: 기존 헤더 + 본문
    if($viewtype==='papertest'){
        $coverPage='<div class="cover-page"><div class="cover-title">서술형 평가</div><button class="clear-btn" onclick="clearEasy()">전체 체크 해제</button><div class="cover-footer">'.$stdname.' | KAIST TOUCH MATH</div></div>';
