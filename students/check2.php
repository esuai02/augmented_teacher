...
if($eventid==201)    // 숨기기
	{  
	if($checkimsi==1)  // 여러번 체크하는 경우 고려하도록 추후 변경
		{
...
if($eventid==301) 
	...
	$DB->execute("UPDATE {quiz_attempts} SET  timestart='$timeupdated' WHERE id='$attemptid' "); 
...
if($eventid==40) // 의견달기
	...
	$DB->execute("INSERT INTO {abessi_cognitivetalk} (wboardid,creator,talkid,userid,type,hide,text,timemodified,timecreated ) VALUES('$wboardid','$wbcreator','$talkid','$userid','$type','0','$inputtext','$timecreated','$timecreated')");
