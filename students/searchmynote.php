...
if($searchtext1!=NULL || $searchtext2!=NULL || $searchtext3!=NULL)
	...
	foreach($results as $value)
		...
		if($searchcourseid==$courseid || $searchcourseid==0)
			{
			parse_str($value['redirect'], $params);
			$contentsid = isset($params['pageid']) ? $params['pageid'] : null;
			$thisboard=$DB->get_record_sql("SELECT * FROM mdl_abessi_messages WHERE   contentsid='$contentsid' AND contentstype='1'  ORDER BY id DESC LIMIT 1"); 
			...
			if($thisboard->url!=NULL)$linkurl='https://mathking.kr/moodle/local/augmented_teacher/books/mynote.php?'.$thisboard->url;
			elseif($value['redirect']!=NULL)$linkurl='https://mathking.kr/moodle/local/augmented_teacher/whiteboard/board_topic.php?id='.str_replace('jnrsorksqcrark','jnrsorksqcrark_user'.$studentid,$value['redirect']).'&studentid='.$studentid;
			...
			if($courseid==193)$Mynotes1.='<tr><td><a href="'.$linkurl.'" target="_blank"><div class="tooltip3">'.$displaytext.'<span class="tooltiptext3"><table style="" align=center width=90%><tr><td><img src='.$imgSrc.' width=600></td></tr></table></span></div>   </a></td><td></td><td> </td></tr>';
			elseif($courseid==201)$Mynotes2.='<tr><td><a href="'.$linkurl.'" target="_blank"><div class="tooltip3">'.$displaytext.'<span class="tooltiptext3"><table style="" align=center width=90%><tr><td><img src='.$imgSrc.' width=600></td></tr></table></span></div>   </a></td><td></td><td> </td></tr>';
			elseif($courseid==197)$Mynotes3.='<tr><td><a href="'.$linkurl.'" target="_blank"><div class="tooltip3">'.$displaytext.'<span class="tooltiptext3"><table style="" align=center width=90%><tr><td><img src='.$imgSrc.' width=600></td></tr></table></span></div>   </a></td><td></td><td> </td></tr>';
			}
		}
							...
							<div class="card">							
								...
								<p align=center><img src="https://mathking.kr/Contents/MATH MATRIX/MATH images/IMG/BESSI1617694317001.png" width=60%></p>
									<table align=center  class="table table-head-bg-info mt-8">
										...
										<td valign=bottom><span style="height:40;" id="alert_search" onclick="searchnote($(\'#squareInput1\').val(),$(\'#squareInput2\').val(),$(\'#squareInput3\').val(),$(\'#basic1\').val()); "><img src=https://mathking.kr/Contents/MATH%20MATRIX/MATH%20images/IMG/bessi21_1640176934.png width=40></span> <a href="https://mathking.kr/moodle/local/augmented_teacher/books/chapter.php?cid='.$chapterlog->cid.'&cntid='.$chapterlog->cntid.'&nch='.$chapterlog->nch.'&studentid='.$studentid.'&type=init"><img loading="lazy" style="margin-bottom:5px;" src=https://mathking.kr/moodle/local/augmented_teacher/IMAGES/circulation.png width=50></a></td>
										</tr> 
										</tbody>
									</table>
