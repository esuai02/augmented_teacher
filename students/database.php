...
// LEGACY: This eventid=2 section may be deprecated
// Current save endpoints:
//   - save_daily_goals.php: Saves daily goals to mdl_abessi_weeklyplans (plan1-7)
//   - save_weekly_goals.php: Saves weekly goals to mdl_abessi_weeklyplans (plan8-16)
// TODO: Verify if any active files still use this eventid=2 path
// TODO: If not used, remove this entire eventid=2 section after testing
...
if($type==='주간목표')
	{
	$score=$checkgoal2->score;
	$halfdayago=time()-86400;
	$aweekago=time()-604800;

	$exist1= $DB->get_record_sql("SELECT * FROM mdl_abessi_today WHERE userid='$userid' AND type LIKE '주간목표' AND timecreated <'$halfdayago' ORDER BY id DESC LIMIT 1");
	//$exist2= $DB->get_record_sql("SELECT * FROM mdl_abessi_today WHERE userid='$userid' AND type LIKE '분기목표'  ORDER BY id DESC LIMIT 1");
	...
	WHERE  mdl_quiz_attempts.timefinish > '$aweekago' AND mdl_quiz_attempts.userid='$userid' ORDER BY  mdl_quiz_attempts.id DESC ");
	...
	foreach($quizresult as $value2) 
		...
		if($qnum>15)  //$todayGrade  $ntodayquiz  $weekGrade  $nweekquiz
			{
			//$quizlist1.='<tr><td valign=top> '.$imgstatus.'&nbsp;'.date("m/d",$value2['timestart']).' </td><td valign=top> '.substr($quiztitle,0,60).'</a>  ('.$value2['attempt'].get_string('trial', 'local_augmented_teacher').')  </td><td valign=top><span class="" style="color: rgb(239, 69, 64);"> '.$quizgrade.'점</span></td><td valign=top> 완료</td></tr>';
			$quizlist1.='<tr><td valign=top>  </td><td valign=top> '.$quiztitle.' </td><td valign=top> </td><td valign=top> 완료</td></tr>';
			} 
		else
			{
			//$quizlist2.='<tr><td valign=top> '.$imgstatus.'&nbsp;'.date("m/d",$value2['timestart']).' </td><td valign=top> '.substr($quiztitle,0,60).'</a>  ('.$value2['attempt'].get_string('trial', 'local_augmented_teacher').')  </td><td valign=top><span class="" style="color: rgb(239, 69, 64);"> '.$quizgrade.'점</span></td><td valign=top> 완료</td></tr>';
			$quizlist2.='<tr><td valign=top>  </td><td valign=top> '.$quiztitle.' </td><td valign=top> </td><td valign=top> 완료</td></tr>';
			}
		}
		 
		// 주간 보고서 테이블 작성
		$weeklyreport1 = '<table align=center><tr><td style="font-size:24px;"><b> # 주간목표 : '.$exist1->text.'</b></td></tr><tr><td style="font-size:24px;"><hr></td></tr></table>';
		$weeklyreport2 .= '<table align=center><tr><td valign=top>  </td><td valign=top>연습문제</td><td valign=top> </td><td valign=top>  </td></tr>' . $quizlist2 .'</table>';
		$weeklyreport3 .= '<table align=center><tr><td valign=top>  </td><td valign=top>인증시험</td><td valign=top> </td><td valign=top>  </td></tr>' . $quizlist1 .'</table>';
		
		$weeklyreport= '<table align=center><tr><td valign=top>'.$weeklyreport1.'</td></tr>
		<tr><td valign=top>	<table><tr><td valign=top>'.$weeklyreport2.'</td><td width=5%></td><td valign=top>'.$weeklyreport3.'</td></tr></table> </td></tr></table>';
		...
		if($exist1->id!=NULL)
			...
			$record = new stdClass();
			$record->id = $exist1->id;
			$record->comment = $weeklyreport;
			$record->timemodified = $timecreated;

			$DB->update_record('abessi_today', $record);
			}
		// 결과를 JSON 형태로 반환
		ob_clean(); // Clear any accidental output
   echo json_encode(array("goaltype" =>$type));
	//echo json_encode( array("nextid" =>$nextid) );
	}
if($deadline==NULL)$deadline=$timecreated;

if($mindset==NULL)$mindset='';
...
if($checkgoal->id==NULL)
	{
  	$DB->execute("INSERT INTO {abessi_today} (text,userid,type,timemodified,timecreated) VALUES('수업없음','$userid','오늘목표','$timecreated','$timecreated')");  
	sleep(1);
	}
$DB->execute("UPDATE {abessi_today} SET type='$typetext',inspect='$statusvalue',checktime='$time',submit='$statusvalue',  timemodified='$timecreated'   WHERE userid='$userid' AND ( type LIKE '오늘목표' OR  type='검사요청' ) ORDER BY id DESC LIMIT 1  "); 

// JSON 응답 반환
ob_clean(); // Clear any accidental output
echo json_encode(array('status' => 'success', 'message' => '처리 완료'));
}
...
$DB->execute("UPDATE {abessi_schedule} SET  pinned='0' WHERE  userid='$userid' AND pinned='1'  "); 
...
if($eventid==33) // input weekly schedule (mobile)
	...
	$DB->execute("UPDATE {abessi_schedule} SET  pinned='0' WHERE  userid='$userid' AND pinned='1'  "); 
	$DB->execute("INSERT INTO {abessi_schedule} (editnew,lastday,start1,start2,start3,start4,start5,start6,start7,duration1,duration2,duration3,duration4,duration5,duration6,duration7,type,userid,pinned,timecreated) VALUES('0','$lastday','$start1','$start2','$start3','$start4','$start5','$start6','$start7','$duration1','$duration2','$duration3','$duration4','$duration5','$duration6','$duration7','임시','$userid','1','$timecreated')");

	//$showthis=$duration1; include("../showthis.php");
	} 
...
$randomdream= $_POST['randomdream'];
$randomdreamurl= $_POST['randomdreamurl'];
$timestamp = strtotime($deadline);
$bessi=$userid.$plantype.$deadline.$inputtext.$timecreated;  include("../whiteboard/debug.php");
if($randomdream==='stay') 
	{
	$prev=$DB->get_record_sql("SELECT * FROM mdl_abessi_progress WHERE userid='$userid' ORDER BY id DESC LIMIT 1");
	$randomdream=$prev->dreamchallenge;
	$randomdreamurl=$prev->dreamurl;
	}

$DB->execute("INSERT INTO {abessi_progress} (userid,plantype,deadline,memo,dreamchallenge,dreamurl,hide,timecreated) VALUES('$userid','$plantype','$timestamp','$inputtext','$randomdream','$randomdreamurl','0','$timecreated')"); 
...
ob_clean(); // Clear any accidental output
echo json_encode(array("passedeventid" => $eventid, "plantype" => $plantype));
} 
...
if($eventid==91) // 수납정보
	...
	$DB->execute("INSERT INTO {abessi_attendance} (userid,teacherid,type,subject,status,fee,text,complete,hide,doriginal,dchanged,timecreated) VALUES('$userid','$USER->id','$type','$subject','$status','$fee','$inputtext','0','0','$begintime','$timestamp','$timecreated')"); 
	} 
...
if($eventid==92) // 수납정보 업데이트
	...
	if($deposit==NULL)$deposit=$exitlog->fee;
...
if($eventid==94 && $role!=='student') // 당일 보강 추가
	{ 
	$tremain=$_POST['tremain'];
	if($tremain>=0)$tremain=0;
	else $tremain=round(-$tremain/60,1);
	$DB->execute("UPDATE {abessi_attendance} SET tupdate=tupdate+'$tremain'  WHERE userid LIKE '$userid' AND hide LIKE '0' ORDER BY id DESC LIMIT 1 ");  
	} 
if($eventid==40) // 학생 개인정보 변경
	{ 
	$institute = $_POST['institute']; // 학교 88
	$birthdate = $_POST['birthdate']; //출생년도 89
	$email1 = $_POST['email1']; //이메일 (부) 112
	$email2 = $_POST['email2']; //이메일 (모) 113
	...
	$mathlevel = $_POST['mathlevel']; // 수학 수준 114
	$additional = $_POST['additional']; // 추가 학습 115
	$progress = $_POST['progress']; // 학습 진행 116
	...
	$exit112= $DB->get_record_sql("SELECT * FROM mdl_user_info_data WHERE userid='$userid' AND fieldid=112 ORDER BY id DESC LIMIT 1"); 
	if($exit112->id==NULL) $DB->execute("INSERT INTO {user_info_data} (userid,fieldid,data) VALUES('$userid','112','$email1')"); 
   else $DB->execute("UPDATE {user_info_data} SET data='$email1'  WHERE userid='$userid' AND fieldid=112 ");  

	$exit113= $DB->get_record_sql("SELECT * FROM mdl_user_info_data WHERE userid='$userid' AND fieldid=113 ORDER BY id DESC LIMIT 1"); 
	if($exit113->id==NULL) $DB->execute("INSERT INTO {user_info_data} (userid,fieldid,data) VALUES('$userid','113','$phone2')"); 
   else $DB->execute("UPDATE {user_info_data} SET data='$email2'  WHERE userid='$userid' AND fieldid=113 ");  
	...
	$exit114= $DB->get_record_sql("SELECT * FROM mdl_user_info_data WHERE userid='$userid' AND fieldid=114 ORDER BY id DESC LIMIT 1"); 
	if($exit114->id==NULL) $DB->execute("INSERT INTO {user_info_data} (userid,fieldid,data) VALUES('$userid','114','$mathlevel')"); 
	else $DB->execute("UPDATE {user_info_data} SET data='$mathlevel'  WHERE userid='$userid' AND fieldid=114 ");  

	$exit115= $DB->get_record_sql("SELECT * FROM mdl_user_info_data WHERE userid='$userid' AND fieldid=115 ORDER BY id DESC LIMIT 1"); 
	if($exit115->id==NULL) $DB->execute("INSERT INTO {user_info_data} (userid,fieldid,data) VALUES('$userid','115','$additional')"); 
	else $DB->execute("UPDATE {user_info_data} SET data='$additional'  WHERE userid='$userid' AND fieldid=115 ");  
	
	$exit116= $DB->get_record_sql("SELECT * FROM mdl_user_info_data WHERE userid='$userid' AND fieldid=116 ORDER BY id DESC LIMIT 1");
	if($exit116->id==NULL) $DB->execute("INSERT INTO {user_info_data} (userid,fieldid,data) VALUES('$userid','116','$progress')"); 
	else $DB->execute("UPDATE {user_info_data} SET data='$progress'  WHERE userid='$userid' AND fieldid=116 ");  
	}
...
if($eventid==95) // 수강료 변경 (시수변경/과정변경)
	{  		 
	$type = $_POST['type'];
	$subject = $_POST['subject'];
	$begintime = $_POST['begintime'];
	...
	$DB->execute("INSERT INTO {abessi_attendance} (userid,teacherid,type,subject,status,text,complete,hide,doriginal,dchanged,timecreated) VALUES('$userid','$USER->id','$type','$subject','$status','$inputtext','0','0','$begintime','$timestamp','$timecreated')"); 
	} 
