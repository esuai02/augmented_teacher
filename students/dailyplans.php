...
for ($i = 1; $i <= 16; $i++) {
    ...
    $timeValue = isset($planinfo->$timeField) ? $planinfo->$timeField : '';
...
foreach ($topics as $topic) {
    ...
    if(strpos($displaytext, '도약') != false)$linkurl='https://mathking.kr/moodle/local/augmented_teacher/books/mynote.php?cmid='.$cntid.'&page=1&studentid='.$studentid.'&quizid='.$quizid;
    elseif(strpos($displaytext, '유형') !== false)$linkurl='https://mathking.kr/moodle/mod/quiz/view.php?id='.$cntid;

    elseif(strpos($displaytext, '정복') != false && $learningstyle !== '도제')$linkurl='https://mathking.kr/moodle/mod/quiz/view.php?id='.$cntid;
    elseif(strpos($displaytext, '마무리') != false && $learningstyle !== '도제')$linkurl='https://mathking.kr/moodle/mod/quiz/view.php?id='.$cntid;
    elseif(strpos($displaytext, '심유') != false && strpos($linkurl, 'checklist') != false)$linkurl='https://mathking.kr/moodle/mod/checklist/view.php?id='.$cntid.'&studentid='.$studentid;
    elseif(strpos($displaytext, '화수업') != false && strpos($linkurl, 'checklist') != false)$linkurl='https://mathking.kr/moodle/mod/checklist/view.php?id='.$cntid.'&studentid='.$studentid;
    ...
    $chapterlist .= '<tr><td>' . $nch . '</td><td>' . $insertButton . ' ' . $copyButton . ' ' . $linkIcon . '</td></tr>';
                    ...
                    <table class="table table-bordered" id="goalsTable">
                        ...
                        <tbody>
                            ...
                            for ($week = 1; $week <= $initialRows; $week++) {
                                ...
                                echo '<tr>
                                    <td><input type="time" class="form-control" name="time' . $week . '" value="' . htmlspecialchars($timeValue, ENT_QUOTES) . '"></td>
                        ...
                        <table width="100%">
                            <tr>
                                ...
                                for ($nch = 1; $nch <= $chapnum; $nch++) {
                                    $chname = 'ch' . $nch;
                                    $thischtitle = isset($curri->$chname) ? $curri->$chname : '챕터 ' . $nch;
                                    $title = isset($thischtitle) ? $thischtitle : '챕터 ' . $nch;
                                    $href = 'https://mathking.kr/moodle/local/augmented_teacher/students/dailyplans.php?id=' . $studentid . '&pid=' . $pid . '&cid=' . $cid . '&nch=' . $nch;
