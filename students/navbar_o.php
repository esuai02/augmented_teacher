...
$chapterlog = $DB->get_record_sql(
    "SELECT id, userid FROM mdl_abessi_chapterlog 
     WHERE userid = ? 
     ORDER BY id DESC LIMIT 1",
    array($studentid)
);

// 사용자 정보를 한 번의 쿼리로 가져오기
$user_info_query = "SELECT uid.userid, uid.fieldid, uid.data 
                    FROM mdl_user_info_data uid
                    WHERE (uid.userid = ? AND uid.fieldid IN (111, 107, 88, 89, 82, 90, 64))
                    OR (uid.userid = ? AND uid.fieldid = 22)";

$user_info_results = $DB->get_records_sql($user_info_query, array($studentid, $USER->id));
...
foreach ($user_info_results as $info) {
    if ($info->userid == $USER->id && $info->fieldid == 22) {
        $role = $info->data;
    } else if ($info->fieldid == 111) {
        $userdata = $info;
    } else if (in_array($info->fieldid, array(107, 88, 89, 82, 90, 64))) {
        $userdata2[$info->fieldid] = $info;
    }
}
...
$username = $DB->get_record_sql(
    "SELECT id, hideinput, lastname, firstname, timezone 
     FROM mdl_user 
     WHERE id = ? 
     ORDER BY id DESC LIMIT 1",
    array($studentid)
...
// 여러 정보를 한번에 가져오기 위한 복합 쿼리
$combined_query = "
    (SELECT 'mbtilog' as source, m.id, m.userid, m.type, m.mbti, m.timecreated, null as teacherid, null as weekquizave, null as ntodo, null as nforce, null as tlaststroke
     FROM mdl_abessi_mbtilog m
     WHERE m.userid = ? AND m.type = 'present' 
     ORDER BY m.id DESC LIMIT 1)
    UNION ALL
    (SELECT 'indicator' as source, i.id, i.userid, null as type, null as mbti, i.timecreated, i.teacherid, i.weekquizave, i.ntodo, i.nforce, i.tlaststroke
     FROM mdl_abessi_indicators i
     WHERE i.userid = ? 
     ORDER BY i.id DESC LIMIT 1)
    UNION ALL
    (SELECT 'mission' as source, ml.id, ml.userid, null as type, null as mbti, ml.timecreated, null as teacherid, null as weekquizave, null as ntodo, null as nforce, null as tlaststroke
     FROM mdl_abessi_missionlog ml
     WHERE ml.userid = ? AND ml.eventid = 17
     ORDER BY ml.id DESC LIMIT 1)
    UNION ALL
    (SELECT 'log' as source, l.id, l.userid, null as type, null as mbti, l.timecreated, null as teacherid, null as weekquizave, null as ntodo, null as nforce, null as tlaststroke
     FROM mdl_logstore_standard_log l
     WHERE l.userid = ?
     ORDER BY l.id DESC LIMIT 1)
    UNION ALL
    (SELECT 'readtime' as source, i.id, i.userid, null as type, null as mbti, i.timecreated, i.teacherid, null as weekquizave, null as ntodo, null as nforce, null as tlaststroke
     FROM mdl_abessi_indicators i
     WHERE i.userid = ? AND i.timecreated > ?
     ORDER BY i.id DESC LIMIT 1)
";

$combined_results = $DB->get_records_sql($combined_query, array($studentid, $studentid, $studentid, $studentid, $studentid, $halfdayago));
...
// 정보 처리
$teacherid0 = isset($readtime->teacherid) ? $readtime->teacherid : null;
if ($teacherid0 == NULL) $teacherid0 = $USER->id;

// 필요시 indicators 데이터 삽입
if ((empty($readtime) || $readtime->id == NULL) && ($USER->id == $studentid || $role !== 'student')) {
    $DB->execute("INSERT INTO {abessi_indicators} (userid, timemodified, timecreated) VALUES(?, ?, ?)", 
                array($studentid, $timecreated, $timecreated));
}

// 필요한 변수들 설정
$weeklyquizave = isset($engagement3->weekquizave) ? $engagement3->weekquizave : null;
$ntodo = isset($engagement3->ntodo) ? $engagement3->ntodo : null;
$nforce = isset($engagement3->nforce) ? $engagement3->nforce : null;
...
// 목표 관련 데이터 한번에 가져오기
$goal_query = "
    (SELECT 'today_latest' as source, t1.id, t1.userid, t1.type, t1.text, t1.score, t1.timecreated, t1.planscore
     FROM mdl_abessi_today t1
     WHERE t1.userid = ? AND (t1.type LIKE '오늘목표' OR t1.type LIKE '검사요청') AND t1.timecreated > ?
     ORDER BY t1.id DESC LIMIT 1)
    UNION ALL
    (SELECT 'today_earliest' as source, t2.id, t2.userid, t2.type, t2.text, t2.score, t2.timecreated, t2.planscore
     FROM mdl_abessi_today t2
     WHERE t2.userid = ? AND (t2.type LIKE '오늘목표' OR t2.type LIKE '검사요청') AND t2.timecreated > ?
     ORDER BY t2.id ASC LIMIT 1)
    UNION ALL
    (SELECT 'weekly' as source, t3.id, t3.userid, t3.type, t3.text, t3.score, t3.timecreated, t3.planscore
     FROM mdl_abessi_today t3
     WHERE t3.userid = ? AND t3.type LIKE '주간목표'
     ORDER BY t3.id DESC LIMIT 1)
    UNION ALL
    (SELECT 'last_weekly' as source, t4.id, t4.userid, t4.type, t4.text, t4.score, t4.timecreated, t4.planscore
     FROM mdl_abessi_today t4
     WHERE t4.userid = ? AND t4.type LIKE '주간목표' AND t4.timecreated < ? AND t4.timecreated > ?
     ORDER BY t4.id DESC LIMIT 1)
    UNION ALL
    (SELECT 'termplan' as source, p.id, p.userid, 'termplan' as type, p.memo as text, 0 as score, p.deadline as timecreated, 0 as planscore
     FROM mdl_abessi_progress p
     WHERE p.userid = ? AND p.plantype = '분기목표' AND p.hide = 0
...
$goal_results = $DB->get_records_sql($goal_query, array(
    $studentid, $halfdayago,
    $studentid, $halfdayago,
    $studentid,
    $studentid, $wtimestart1, $wtimestart2,
    $studentid
));
...
foreach ($goal_results as $result) {
    switch ($result->source) {
        case 'today_latest':
            $checkgoal = $result;
            break;
        ...
        case 'termplan':
            $termplan = $result;
            $termplan->deadline = $result->timecreated;
            $termplan->memo = $result->text;
            break;
    }
}

// 데이터 처리
$tgoal = isset($checkgoal2->timecreated) ? $checkgoal2->timecreated : $timecreated;
$tlastinput = isset($checkgoal->timecreated) ? $checkgoal->timecreated : null;
$tcomplete0 = isset($engagement3->timecreated) ? $engagement3->timecreated + $hours * 3600 : null;
$tcomplete = date("h:i A", $tcomplete0 ? $tcomplete0 : $timecreated);

$ratio1 = isset($checkgoal->score) ? $checkgoal->score : null;
$ratio2 = isset($wgoal->score) ? $wgoal->score : null;
$lastWeekPlanScore = isset($lastwgoal->planscore) ? $lastwgoal->planscore : null;

if ($role === 'student') {
    $tabtitle = isset($wgoal->text) ? $wgoal->text : '';
} else {
    ...
    $tabtitle = $username->lastname . '(' . (isset($wgoal->text) ? $wgoal->text : '') . ')';
}

$dreamdday = isset($termplan->deadline) ? round(($termplan->deadline - $timecreated) / 86400 + 1, 0) : null;
$EGinputtime = isset($termplan->deadline) ? date("m/d", $termplan->deadline) : null;
$termMission = isset($termplan->memo) ? $termplan->memo : null;

// 참여도 시간 계산
$teng1 = isset($engagement1->timecreated) ? (INT)((time() - $engagement1->timecreated) / 60) : PHP_INT_MAX;
$teng2 = isset($engagement2->timecreated) ? (INT)((time() - $engagement2->timecreated) / 60) : PHP_INT_MAX;
$teng3 = isset($engagement3->tlaststroke) ? (INT)((time() - $engagement3->tlaststroke) / 60) : PHP_INT_MAX;
...
if ($nday == 1) {
    $tstart = $schedule->start1; 
    $hours = $schedule->duration1;
} else if ($nday == 2) {
    $tstart = $schedule->start2; 
    $hours = $schedule->duration2;
} else if ($nday == 3) {
    $tstart = $schedule->start3; 
    $hours = $schedule->duration3;
} else if ($nday == 4) {
    $tstart = $schedule->start4; 
    $hours = $schedule->duration4;
} else if ($nday == 5) {
    $tstart = $schedule->start5; 
    $hours = $schedule->duration5;
} else if ($nday == 6) {
    $tstart = $schedule->start6; 
    $hours = $schedule->duration6;
} else if ($nday == 7) {
    $tstart = $schedule->start7; 
    $hours = $schedule->duration7;
}

$lastday = isset($schedule->lastday) ? $schedule->lastday : null;

$id=$finish->id;  
$mark=$finish->mark;

$mid=7;
$tremain=(INT)((($tgoal+$hours*3600)-$timecreated)/60);
...
else $tleft=$hours.'시간 중 '.(-$tremain).'분 보충수업 진행하였습니다. '; 
...
$imgstatus= '<a href="https://mathking.kr/moodle/local/augmented_teacher/whiteboard/board_onair.php?userid='.$studentid.'&mode=1" accesskey="w"><img loading="lazy" src=https://mathking.kr/Contents/MATH%20MATRIX/MATH%20images/IMG/MXBESSI1624721323001.png width=80></a>&nbsp;&nbsp;&nbsp;&nbsp;'.$mark4.'&nbsp;<img loading="lazy" src='.$imgtoday.' width=35><img loading="lazy" src="https://mathking.kr/Contents/Moodle/Visual%20arts/Preloader2.gif" width=60 ><img loading="lazy" src='.$imgtoday2.' width=35>&nbsp;&nbsp;'.$mark5. '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'; 
 
$messageicon1='<a href="https://mathking.kr/moodle/local/augmented_teacher/teachers/synergetic.php?id='.$USER->id.'&userid='.$studentid.'"><img style="margin-bottom:0px;" src=https://mathking.kr/moodle/local/augmented_teacher/IMAGES/circulation.png width=50></a> <a href="https://mathking.kr/moodle/local/augmented_teacher/books/chapter.php?cid='.$chapterlog->cid.'&cntid='.$chapterlog->cntid.'&nch='.$chapterlog->nch.'&studentid='.$studentid.'&type=init"><img style="margin-bottom:0px;" src=https://mathking.kr/Contents/IMAGES/timefolding.png width=38></a>';
...
if($role!=='student')$messageicon2='<span onClick="showMoment(\''.$studentid.'\')" accesskey="m"><img loading="lazy" src=https://mathking.kr/Contents/IMAGES/message.png height=30></span>';
...
if($institute==NULL || $birthyear==NULL) $schinfo='정보 미입력<br>('.$lastaccess.'분 전)'; 
else $schinfo=$institute.' '.$ngrade.'<br>('.$lastaccess.'분 전)';
...
$randomdreamdisplay='  <div class="gradient-bg shadow-none rounded-md">
  <a  style="color:white;"
    href="'.$termplan->dreamurl.'"
    target="_blank" 
    rel="noopener noreferrer">🌟랜덤꿈 &nbsp;
    '.$termplan->dreamchallenge.'
  </a>
</div> ';
if($termMission==NULL || $termplan->deadline<$timecreated)$goaldisplay='<a style="color:white;" href="https://mathking.kr/moodle/local/augmented_teacher/students/roadmap.php?id='.$studentid.'">분기목표를 설정해 주세요</a>'; 
else $goaldisplay= ' <div class="gradient-bg shadow-none rounded-md"> <span style="color:#EEFF00;"  class="gradient-text tracking-transition">🌟분기목표 : '.$termMission.'</span>(<a style="color:white;" href=" https://mathking.kr/moodle/local/augmented_teacher/students/weeklyplans.php?id='.$studentid.'&cid='.$chapterlog->cid.'&pid='.$termplan->id.'"target="_blank">'.$EGinputtime.'</a>)</div>';

if(($tstart!=NULL && $hours!=NULL && (strtotime($tstart)-$timecreated)/60>60)||$tstart==NULL)
	{
	$goaldisplay= ' &nbsp; <span style="color:white;background-color:red;border-radius:10px;padding:10px;"> ❕ 수업 가능한 시간이 아닙니다. 시간표 변경을 원한다면 선생님과 상의해 주세요.</span>';
	//exit;
	}
		...
		<div class="main-header" style="background-color:#177dff;color:white;">
			...
			<nav class="navbar navbar-header navbar-expand-lg" data-background-color="blue">
				...
				<div class="container-fluid ">
					 <table width=100%><tr><td align=right style="white-space: nowrap;"><span align=right>	'.$messageicon1.'	</span>	&nbsp; </td><td  style="white-space: nowrap; font-size: 16px; color: yellow; overflow: hidden; text-overflow: ellipsis; ">'.$goaldisplay.'</td><td style="white-space: nowrap; font-size: 16px; color: yellow; overflow: hidden; text-overflow: ellipsis; ">'.$randomdreamdisplay.'</td>
					<td style=" font-size:16;color:white;white-space: nowrap;" align=right>'.$imgstatus.' </td><td align=right>'.$messageicon2.'</td></tr></table>		 
				</div>
			</nav>
			<!-- End Navbar -->
		</div>
...
<li><a href="https://mathking.kr/moodle/report/log/user.php?mode=today&course=1&&id='.$studentid.'" target="_blank">활동로그</a></li>
					...
					<ul class="nav">
						...
						<li class="nav-item">
									...
									<i><img src=https://mathking.kr/Contents/IMAGES/pomodorologo.png width=20></i>
									<p style="color:blue;">수학일기</p>									 
									</a>	
						</li>	
						...
						<li class="nav-item">
									<a href="https://mathking.kr/moodle/local/augmented_teacher/books/domaindrilling.php?domain=120&studentid='.$studentid.'"target="_blank">
									<i></i>
									<p>수학특강</p>									 
									</a>	
						</li>	
...
<style>
.gradient-bg {
  background: linear-gradient(to right, #2563eb, #2563eb); /* from-purple-800 to-blue-600 */
...
.gradient-text {
  font-size: 1rem;
  font-weight: 700;
  background: linear-gradient(to right, yellow, #fef08a, white);
...
.gradient-bg:hover .tracking-transition {
